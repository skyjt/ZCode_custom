...
AIbuddy uses direct API providers without a product account. Configure providers in the desktop Model settings page; the CLI reads the same Personal Provider Config under `~/.aibuddy/v2/`. Use `/model` to choose an available model. The `login`, `logout`, `/login` and `/logout` commands have been removed.
...
- `npm run build` produces `dist/aibuddy.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
aibuddy plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.aibuddy/cli/plugins`:
...
This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and AIbuddy Guide content plugins are default-enabled and appear as `browser-use@zcode-plugins-official`, `document-skills@zcode-plugins-official`, `skill-creator@zcode-plugins-official`, and `aibuddy-guide@zcode-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@zcode-plugins-official`, `android-emulator@zcode-plugins-official`, and `restore-legacy-sessions@zcode-plugins-official`, are discovered by aibuddy but stay disabled until the user enables them.

```sh
aibuddy plugins list
aibuddy plugins enable ios-simulator
aibuddy plugins disable browser-use
aibuddy plugins enable restore-legacy-sessions
aibuddy plugins disable ios-simulator
```
...
MCP config can live directly in `.aibuddy-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current aibuddy plugin surface:
...
Example `.aibuddy-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${AIBUDDY_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${AIBUDDY_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${AIBUDDY_PLUGIN_DATA}",
...
Only environment variables with the `AIBUDDY_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting aibuddy-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

aibuddy reads MCP servers from the main JSON config. The default user config path is `~/.aibuddy/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits aibuddy's environment plus any `env` overrides.
...
aibuddy reads hooks from the same main JSON config file as MCP, usually `~/.aibuddy/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
