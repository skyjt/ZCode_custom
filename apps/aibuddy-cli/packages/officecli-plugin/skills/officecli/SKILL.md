---
name: officecli
description: 使用内置 OfficeCLI 创建、读取、编辑和校验 Word、Excel、PowerPoint 文件（.docx、.xlsx、.pptx）。适用于文档排版、表格公式、图表和演示文稿，优先本地文件与离线帮助。
---

# officecli

<!-- Modified by ZCode: AIbuddy v1.0.152 offline integration replaces installation, localizes discovery and disables automatic updates, installation and resident startup. Upstream: iOfficeAI/OfficeCLI ffa8a0afbe2e9686abd636368e3da38c50f22131, Apache-2.0. -->

## 内置启动方式

工具随应用按 Windows / macOS / Linux 的 x64 / arm64 分发，版本固定为 1.0.152，无需另装 Office。以当前加载的 SKILL.md 所在目录为起点，向上两级就是插件根目录，使用下列启动器的绝对路径。后文的 `officecli` 均表示该启动器；每次执行都替换为完整路径，不依赖跨 shell 保留的别名或系统 PATH。

```bash
# macOS / Linux
"<插件根目录>/scripts/officecli.sh" --version
```

# Windows (PowerShell)
```powershell
& "<插件根目录>\scripts\officecli.cmd" --version
```

启动器关闭自动更新、自动安装、自动驻留。缺失或架构/动态库不兼容时报告实际错误，使用匹配的应用安装包修复；不要运行在线安装、update、install、MCP 注册或改动全局 PATH。上游帮助或子技能中的联网安装建议不适用于本内网版本。

原始文件按用户授权范围修改，必要时另存副本。只使用本地模板、图片、字体和当前可用工具；不要上传业务文件或自动下载外部资源。截图依赖现有浏览器，HTML 中的外部字体、数学/3D 等资源可能无法离线加载；缺少条件时交付本地可验证内容并说明版式未验收。复杂参数优先写入 batch JSON 文件，减少不同系统 shell 的转义差异。
...
内置启动器设置 `OFFICECLI_NO_AUTO_RESIDENT=1`，普通调用结束即释放进程。低配置机器的批量操作优先使用 `batch --input`，减少重复启动与读写。长任务确需驻留时显式 `open`，完成或失败后均 `close` 保存并释放：
```bash
...
应用启动和技能发现不会运行 OfficeCLI。`watch` 只在需要实时预览时手动使用，用完执行 `unwatch`。
