...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    readonly credentialStore?: SharedAIbuddyCredentialStore;
    ...
    readonly onBuiltinRefreshResult?: (event: AIbuddyBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 AIbuddy Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedAIbuddyCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[AIBUDDY_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          aibuddyBuiltinFilePath: bundledFile,
          aibuddyBuiltinActiveFilePath: paths.aibuddyBuiltinFilePath,
          aibuddyBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.aibuddyBuiltinFilePath),
              "aibuddy-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeAIbuddyEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadAIbuddyBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: AIBUDDY_VERSION,
                platform: resolveAIbuddyBuiltinClientPlatform(),
      ...
      : {}),
    onAIbuddyBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckAIbuddyBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.aibuddyBuiltinRevision !== account.basedOnAIbuddyBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  const basedOnAIbuddyBuiltinRevision = input.basedOnAIbuddyBuiltinRevision.trim();
  if (!basedOnAIbuddyBuiltinRevision) {
