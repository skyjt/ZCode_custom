...
} from "@aibuddy/adapters/storage";
import { createNodeLoggerFactory } from "@aibuddy/adapters/logging";
import { createConfig, resolvePath } from "@aibuddy/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@aibuddy/adapters/exec";
import { createNodeFileSystemAdapter } from "@aibuddy/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@aibuddy/adapters/http";
import { createJimpImageProcessorAdapter } from "@aibuddy/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@aibuddy/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@aibuddy/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@aibuddy/adapters/context";
import { createNodeSkillAdapter } from "@aibuddy/adapters/skills";
import { createMcpAdapter } from "@aibuddy/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@aibuddy/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveAIbuddyRuntimeEnv } from "@aibuddy/shared";
import {
...
import { resolveAIbuddyCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveAIbuddyBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadAIbuddyAgentProfiles } from "../subagents.js";
...
export async function createAIbuddyApp(options: AIbuddyAppOptions): Promise<AIbuddyApp> {
  if (!options?.providerRegistry) {
    throw new Error("createAIbuddyApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("aibuddy").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveAIbuddyRuntimeEnv(options.env ?? process.env) === "development",
    );
    const aibuddySubagentProfileOutcome = await loadAIbuddyAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const aibuddySubagentProfiles = aibuddySubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: aibuddySubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: aibuddySubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...aibuddySubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          aibuddySubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("AIbuddy runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("AIbuddy runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).AIBUDDY_MAILBOX_ROOT ?? "~/.aibuddy/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveAIbuddyBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveAIbuddyCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("aibuddy-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("aibuddy-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("AIbuddy app startup failed", error, {
