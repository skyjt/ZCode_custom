...
import type { SharedAIbuddyCredentialStore } from "@aibuddy/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@aibuddy/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  if (config)
    return {
      aibuddyBuiltinRevision: config.revision,
  ...
  const filePath = env[AIBUDDY_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${AIBUDDY_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `AIbuddy Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedAIbuddyCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.aibuddyBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedAIbuddyCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
