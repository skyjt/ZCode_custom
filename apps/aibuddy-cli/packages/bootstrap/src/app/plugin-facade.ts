...
import type { PluginLoadOutcome } from "@aibuddy/contracts";
...
import type {
  AIbuddyApp,
  AIbuddyAppOptions,
  AIbuddyPluginSetResult,
  AIbuddyPluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listAIbuddyPlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<AIbuddyPluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<AIbuddyPluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
