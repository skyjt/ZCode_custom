import type { AIbuddyToolExecResource, BackgroundBashOutputResult } from "@aibuddy/shared";
import type { AiSdkModelAdapter } from "@aibuddy/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { AIbuddyModelOption } from "@aibuddy/shared";
import type { EffectiveModelSelectionResult } from "@aibuddy/shared/model-selection";
export type { AIbuddyModelOption } from "@aibuddy/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { AIbuddyInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@aibuddy/contracts";
...
export type AIbuddyAppRuntimeConfigInput = AgentRuntimeConfig;

export interface AIbuddyAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: AIbuddyAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: AIbuddyToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@aibuddy/contracts").TurnBackgroundAttribution;
...
export interface AIbuddyPluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface AIbuddyPluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: AIbuddyInstalledPluginData | null;
}
...
export interface AIbuddyApp {
  ...
  getCurrentModelOption?(): AIbuddyModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): AIbuddyModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@aibuddy/shared").AIbuddySessionSubagentsResult>;
  ...
  listModels(): AIbuddyModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<AIbuddyPluginSetResult>;
  uninstallPlugin(plugin: string): Promise<AIbuddyPluginUninstallResult>;
  /**
...
export interface RunAIbuddyProtocolAgentOptions {
...
export interface ListAIbuddySessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
