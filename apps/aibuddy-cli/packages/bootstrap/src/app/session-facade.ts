import { updateUiLocaleInFileConfig, type ConfigResult } from "@aibuddy/adapters/config";
import type { AgentRuntime } from "@aibuddy/core";
import { resolveLocale } from "@aibuddy/i18n";
import { normalizeModelSelection, type ModelSelection } from "@aibuddy/provider";
import {
...
import type { PrepareUserExecutionBoundary, AIbuddyApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<AIbuddyApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<AIbuddyApp["updateTargetStatus"]>
      >,
