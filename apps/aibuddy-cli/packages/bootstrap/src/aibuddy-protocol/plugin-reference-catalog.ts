...
import {
  ...
  aibuddyPluginsReferenceCatalogParamsSchema,
  aibuddyPluginsResolveSuggestedReferenceParamsSchema,
  type AIbuddyPluginReferenceCatalogEntry,
  type AIbuddyPluginDiagnostic as SharedPluginDiagnostic,
  type AIbuddyPluginsReferenceCatalogResult,
  type AIbuddyPluginsResolveSuggestedReferenceResult,
} from "@aibuddy/shared";
import type { PluginReferenceCatalogEntry } from "@aibuddy/contracts";
import { buildPluginReferenceCatalog } from "@aibuddy/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(aibuddyPluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<AIbuddyPluginsResolveSuggestedReferenceResult> {
  const params = parseParams(aibuddyPluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  const readState = () => {
    const outcome = resolveAIbuddyPlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: aibuddyProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateAIbuddyPluginMarketplace({
  ...
  const overview = getAIbuddyPluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getAIbuddyPluginsOverview({ workingDirectory: workspacePath });
