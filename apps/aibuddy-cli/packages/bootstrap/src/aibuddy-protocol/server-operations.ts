/* eslint-disable max-lines -- AIbuddy Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../aibuddy-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@aibuddy/adapters/exec";
import { inputIntentMetadata } from "../aibuddy-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@aibuddy/core";
import {
  CoreErrorType,
...
import {
  ...
  aibuddyProtocolErrorCodes,
  aibuddyProtocolMethods,
  aibuddySessionCancelBackgroundTaskParamsSchema,
  aibuddySessionCompactParamsSchema,
  aibuddySessionCloseParamsSchema,
  aibuddySessionCreateParamsSchema,
  aibuddySessionEventsParamsSchema,
  aibuddySessionForkParamsSchema,
  aibuddySessionGoalParamsSchema,
  aibuddySessionListParamsSchema,
  aibuddySessionMessagesParamsSchema,
  aibuddySessionReadParamsSchema,
  aibuddySessionRuntimePreferencesResultSchema,
  aibuddySessionResumeParamsSchema,
  aibuddySessionSendParamsSchema,
  aibuddySessionSetModeParamsSchema,
  aibuddySessionSetModelParamsSchema,
  aibuddySessionSetThoughtLevelParamsSchema,
  aibuddySessionStopParamsSchema,
  aibuddySessionSubscribeParamsSchema,
  aibuddySessionSubagentsParamsSchema,
  aibuddyTaskTokenUsageParamsSchema,
  aibuddyUsageStatsParamsSchema,
  aibuddyWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type AIbuddySessionCreateParams,
  type AIbuddyDeliveryKind,
  type IntegratedTerminalShellSelection,
  type AIbuddySessionRuntimePreferencesScope,
  type AIbuddySessionRuntimePreferencesResult,
  type AIbuddyModelContextBudgetStrategy,
  type AIbuddyProtocolTrace,
  type AIbuddySessionEvent,
  type AIbuddySessionHistoryTarget,
  type AIbuddySessionResumeParams,
  type AIbuddySessionPersistence,
...
import {
  ...
  type AIbuddyProtocolSessionRecord,
  type AIbuddyProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceAIbuddyApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../aibuddy-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<AIbuddyProtocolSessionRecord["app"]["readTarget"]>>>
>;

type AIbuddySessionRecordParams = (
  | AIbuddySessionCreateParams
  | (AIbuddySessionResumeParams & {
      mode?: AIbuddySessionCreateParams["mode"];
      model?: AIbuddySessionCreateParams["model"];
      parentSessionId?: AIbuddySessionCreateParams["parentSessionId"];
      thoughtLevel?: AIbuddySessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: AIbuddySessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: AIbuddyModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: AIbuddyProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<AIbuddyProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  AIbuddyProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: AIbuddyDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: AIbuddyProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: AIbuddyProtocolSessionRecord,
  deliveryKind?: AIbuddyDeliveryKind,
): AIbuddyProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: AIbuddyProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const aibuddy = asRecord(meta.aibuddy);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(aibuddy.parentToolUseId) ??
    stringValue(aibuddy.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: AIbuddySessionEvent,
  toolInputTransmissions: AIbuddyProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): AIbuddySessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("AIbuddy Protocol session event filtered", {
      event: "aibuddy_protocol.session_event.filtered",
      eventId: String(event.id),
      module: "bootstrap.aibuddy_protocol",
  ...
  // tool_input_delta 等高频模型中间态。AIbuddy Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<AIbuddySessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: AIbuddySessionEvent[] = [];
  const toolInputTransmissions: AIbuddyProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("AIbuddy Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "aibuddy_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<AIbuddySessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<AIbuddyProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<AIbuddySessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: AIbuddyProtocolSessionRecord;
  sessionId: SessionId;
  createParams: AIbuddySessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("AIbuddy Protocol imported history persisted", {
    event: "aibuddy_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
  trace: AIbuddyProtocolTrace | undefined,
  project: (
    record: AIbuddyProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(aibuddySessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("AIbuddy Protocol session/create started", {
    event: "aibuddy_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("AIbuddy Protocol session/create skipped unsupported thought level", {
          event: "aibuddy_protocol.session_create.thought_level_skipped",
          module: "bootstrap.aibuddy_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 AIbuddy session。
    ...
    context.logger?.info("AIbuddy Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "aibuddy_protocol.session_create.completed",
    ...
    context.logger?.warn("AIbuddy Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "aibuddy_protocol.session_create.failed",
      module: "bootstrap.aibuddy_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: AIbuddyProtocolAgentServerContext,
  params: AIbuddySessionResumeParams,
  ...
  context.logger?.debug("AIbuddy Protocol session resume started", {
    activeBeforeWait,
    event: "aibuddy_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("AIbuddy Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "aibuddy_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.aibuddy_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      aibuddyProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("AIbuddy Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "aibuddy_protocol.session.resume_completed",
    module: "bootstrap.aibuddy_protocol",
...
export async function resumeSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "aibuddy_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "aibuddy_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.aibuddy_protocol",
      sessionId: session.id,
...
export async function listSessions(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(aibuddySessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("AIbuddy Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "aibuddy_protocol.session.persisted_missing",
      module: "bootstrap.aibuddy_protocol",
...
export async function getUsageStats(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddyUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionReadParamsSchema, rawParams);
...
export async function readMessages(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(aibuddySessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("AIbuddy Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("AIbuddy Protocol session/send accepted", {
...
export async function compactSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("AIbuddy Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("AIbuddy Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("AIbuddy Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("AIbuddy Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("AIbuddy Protocol goal paused"));
    }
...
export async function forkSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: AIbuddyProtocolAgentServerContext,
  record: AIbuddyProtocolSessionRecord,
  fork: Awaited<ReturnType<AIbuddyProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: AIbuddyProtocolAgentServerContext,
  parentRecord: AIbuddyProtocolSessionRecord,
  forkRecord: AIbuddyProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("AIbuddy Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("AIbuddy Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("AIbuddy Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("AIbuddy Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("AIbuddy Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("AIbuddy Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("AIbuddy Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("AIbuddy Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(aibuddySessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("AIbuddy Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(aibuddySessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: AIbuddyProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(aibuddySessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: AIbuddySessionPersistence,
  expectedPersistence?: AIbuddySessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(aibuddyWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceAIbuddyApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: AIbuddyProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: AIbuddyProtocolAgentServerContext,
  _record: AIbuddyProtocolSessionRecord,
...
async function resolveForkTarget(
  context: AIbuddyProtocolAgentServerContext,
  record: AIbuddyProtocolSessionRecord,
  target: AIbuddySessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(aibuddyTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: AIbuddyProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: aibuddyProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "aibuddy_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "aibuddy_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // AIbuddyProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("AIbuddy Protocol runtime preferences request started", {
    event: "aibuddy_protocol.runtime_preferences.request_started",
    module: "bootstrap.aibuddy_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      aibuddyProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      aibuddySessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("AIbuddy Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "aibuddy_protocol.runtime_preferences.response_received",
      module: "bootstrap.aibuddy_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "aibuddy_protocol.runtime_preferences.request_failed",
      module: "bootstrap.aibuddy_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("AIbuddy Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("AIbuddy Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "AIbuddy Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_AIBUDDY_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: AIbuddyProtocolAgentServerContext,
  params: AIbuddySessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: AIbuddyProtocolTrace,
): Promise<AIbuddyProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("AIbuddy Protocol createRecord MCP config", {
    event: "aibuddy_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: AIbuddyProtocolSessionRecord | undefined;
  const app = await createWorkspaceAIbuddyApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.aibuddy/.agents 的 enabled MCP，
      ...
      // AIbuddy Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // AIbuddy Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: AIbuddyProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): AIbuddySessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isAIbuddySessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isAIbuddySessionMode(
  value: unknown,
): value is NonNullable<AIbuddySessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("AIbuddy Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "aibuddy_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "aibuddy_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "aibuddy_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "aibuddy_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "aibuddy_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: AIbuddyStateUpdatedNotification = {
    patch,
    reason,
