...
import {
  ...
  aibuddyUserInputResponseSchema,
  type AIbuddyPermissionOption,
  type AIbuddyPermissionResponse,
  type AIbuddyUserInputQuestion,
...
import type { AIbuddyProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        aibuddyProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → AIbuddyPermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: AIbuddyPermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 aibuddyPermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        aibuddyProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): AIbuddyUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): AIbuddyUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): AIbuddyUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): AIbuddyUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "aibuddy_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
