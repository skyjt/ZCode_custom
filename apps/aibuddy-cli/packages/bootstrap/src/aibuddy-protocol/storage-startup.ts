...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@aibuddy/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  aibuddyProtocolNotifications,
  aibuddyStorageStartupStateSchema,
  type AIbuddyStorageStartupState,
} from "@aibuddy/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: AIbuddyStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = aibuddyStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: aibuddyProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { aibuddyStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@aibuddy/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = aibuddyStoragePathReadySchema.parse(JSON.parse(line));
