import type { McpServerConfig } from "@aibuddy/contracts";
import type { AIbuddyProtocolMcpServer } from "@aibuddy/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: AIbuddyProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // AIbuddy Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
