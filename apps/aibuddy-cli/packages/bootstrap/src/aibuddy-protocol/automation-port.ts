...
import {
  ...
  aibuddyAutomationCheckTaskBindingResultSchema,
  aibuddyAutomationCreateResultSchema,
  aibuddyAutomationDeleteResultSchema,
  aibuddyAutomationListResultSchema,
  aibuddyAutomationUpdateResultSchema,
  aibuddyProtocolMethods,
  type AIbuddyAutomationProtocol,
} from "@aibuddy/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => AIbuddyProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              aibuddyProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              aibuddyProtocolMethods.automationList,
              {},
              aibuddyAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          aibuddyProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        aibuddyProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        aibuddyProtocolMethods.automationDelete,
        { automationId: input.id },
        aibuddyAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: AIbuddyAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: AIbuddyAutomationProtocol): CronAutomation {
  return {
