...
import type { McpTelemetryTracker } from "@aibuddy/adapters";
import type { WorkspaceHookPolicyProvider } from "@aibuddy/core";
import type { AccountProviderConfigSnapshot } from "@aibuddy/provider";
import {
  aibuddyProtocolErrorCodes,
  type AIbuddyDeliveryKind,
  type ModelSelection,
  type AIbuddyModelContextBudgetStrategy,
  type AIbuddyProtocolMessage,
  type AIbuddyProtocolMethod,
  type AIbuddyProtocolNotification,
  type AIbuddyProtocolRequest,
  type AIbuddyProtocolRequestId,
  type AIbuddyProtocolTrace,
  type AIbuddySessionMode,
  type AIbuddySessionPersistence,
  type AIbuddyWorkspaceRef,
} from "@aibuddy/shared";
import type { AIbuddyApp, AIbuddyAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../aibuddy-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../aibuddy-protocol-v4/v4-gateway.js";
...
export interface AIbuddyProtocolAgentDependencies {
  createAIbuddyApp(options?: Omit<AIbuddyAppOptions, "providerRegistry">): AIbuddyApp | Promise<AIbuddyApp>;
  /**
...
export type AIbuddyProtocolAgentResolvedDependencies = AIbuddyProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface AIbuddyProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface AIbuddyProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface AIbuddyProtocolSessionRecord {
  app: AIbuddyApp;
  ...
  protocolEventSequences: Map<string, AIbuddyProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, AIbuddyProtocolToolInputTransmissionState>;
...
export interface AIbuddyProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface AIbuddyProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: AIbuddyProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: AIbuddyProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: AIbuddyProtocolMessage): message is AIbuddyProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: AIbuddyProtocolMessage,
): message is AIbuddyProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: AIbuddyProtocolMessage,
): message is { id: AIbuddyProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: AIbuddyProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): AIbuddyProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: AIbuddyDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("AIbuddy Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "aibuddy_protocol.session.require_missing",
...
export function createProtocolLogger(deps: AIbuddyProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("aibuddy").child({
    module: "bootstrap.aibuddy_protocol",
  });
}
