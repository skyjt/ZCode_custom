...
import {
  createAIbuddyProcessResourceSampler,
  type AIbuddyProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: AIbuddyProtocolAgentServer,
  send: (message: AIbuddyProtocolNotification) => void,
  logger: Logger,
): AIbuddyProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createAIbuddyProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: aibuddyProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "aibuddy_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
