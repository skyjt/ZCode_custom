import type { OffPeakPort, OffPeakTaskSummary } from "@aibuddy/contracts";
import {
  aibuddyOffPeakCreateResultSchema,
  aibuddyOffPeakListResultSchema,
  aibuddyProtocolMethods,
  type AIbuddyOffPeakTaskProtocolSnapshot,
} from "@aibuddy/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            aibuddyProtocolMethods.offPeakList,
            {},
            aibuddyOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        aibuddyProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: AIbuddyOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
