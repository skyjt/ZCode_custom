...
import { workspaceHookPolicySchema } from "@aibuddy/contracts";
import type { WorkspaceHookPolicyProvider } from "@aibuddy/core";
import {
  aibuddyWorkspaceHookTrustGrantParamsSchema,
  aibuddyWorkspaceHookTrustGrantReasonCodeSchema,
  aibuddyWorkspaceHookTrustGrantResultSchema,
  type AIbuddyWorkspaceHookTrustGrantReasonCode,
  type AIbuddyWorkspaceHookTrustGrantResult,
} from "@aibuddy/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = aibuddyWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return aibuddyWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): AIbuddyWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): AIbuddyWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = aibuddyWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
