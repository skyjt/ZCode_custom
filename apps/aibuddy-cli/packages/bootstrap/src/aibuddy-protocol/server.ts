...
import {
  aibuddyPluginsCancelOperationParamsSchema,
  aibuddyProtocolMethods,
  aibuddyWorkspaceCancelGenerateTextParamsSchema,
  aibuddyWorkspaceHookTrustGrantParamsSchema,
} from "@aibuddy/shared";
import type { BrowserControlPort } from "@aibuddy/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@aibuddy/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  AIbuddyProtocolError,
  AIbuddyProtocolMessage,
  AIbuddyProtocolMethod,
  AIbuddyProtocolNotification,
  AIbuddyProtocolRequest,
  AIbuddyProtocolRequestId,
  AIbuddyProtocolResponse,
} from "@aibuddy/shared";
import {
...
} from "../aibuddy-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type AIbuddyProtocolClientRequestOptions,
  type AIbuddyProtocolAgentDependencies,
  type AIbuddyProtocolAgentServerContext,
  type AIbuddyProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@aibuddy/contracts";

export type { AIbuddyProtocolAgentDependencies, AIbuddyProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type AIbuddyProtocolOutboundMessage = AIbuddyProtocolNotification | AIbuddyProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, AIbuddyProtocolSessionRecord>;
}): Promise<void> {
...
interface AIbuddyProtocolPostResponseBatch {
  readonly messages: readonly AIbuddyProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class AIbuddyProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<AIbuddyProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: AIbuddyProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: AIbuddyProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    AIbuddyProtocolRequestId,
    AIbuddyProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: AIbuddyProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createAIbuddyApp);
    const resolvedDeps = {
      ...deps,
      createAIbuddyApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, AIbuddyProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: AIbuddyProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("AIbuddy Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "aibuddy_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: AIbuddyProtocolRequestId): AIbuddyProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: AIbuddyProtocolRequestId): AIbuddyProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: AIbuddyProtocolMessage,
  ): Promise<AIbuddyProtocolError | AIbuddyProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("AIbuddy Protocol notification ignored", {
        event: "aibuddy_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: AIbuddyProtocolRequest,
  ): Promise<AIbuddyProtocolError | AIbuddyProtocolResponse> {
  ...
  private async dispatchRequest(request: AIbuddyProtocolRequest) {
      ...
      case aibuddyProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: aibuddyWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? aibuddyWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(aibuddyPluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(aibuddyWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: AIbuddyProtocolRequestId, result: unknown): AIbuddyProtocolResponse {
    return { id, result };
  }

  private fail(
    id: AIbuddyProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): AIbuddyProtocolError {
  ...
  private requestClient<T>(
    method: AIbuddyProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: AIbuddyProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No AIbuddy Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: AIbuddyProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: AIbuddyProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
