...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@aibuddy/contracts";
import {
  aibuddyMcpListParamsSchema,
  aibuddyMcpListResultSchema,
  type AIbuddyMcpListResult,
} from "@aibuddy/shared";
import {
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<AIbuddyMcpListResult> {
  const params = parseParams(aibuddyMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.aibuddy_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.aibuddy` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return aibuddyMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return aibuddyMcpListResultSchema.parse({
