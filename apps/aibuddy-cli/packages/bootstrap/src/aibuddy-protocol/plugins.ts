import {
  aibuddyPluginsConfigureParamsSchema,
  aibuddyPluginsResetConfigParamsSchema,
  aibuddyPluginsInstallParamsSchema,
  aibuddyPluginsMarketplaceAddParamsSchema,
  aibuddyPluginsMarketplaceRemoveParamsSchema,
  aibuddyPluginsMarketplaceUpdateParamsSchema,
  aibuddyPluginsOverviewParamsSchema,
  aibuddyPluginsListParamsSchema,
  aibuddyPluginsSetEnabledParamsSchema,
  aibuddyPluginsUninstallParamsSchema,
  aibuddyPluginsUpdateParamsSchema,
  aibuddyPluginsValidateParamsSchema,
  aibuddyPluginsDescribeParamsSchema,
  aibuddyPluginsRestoreBuiltinParamsSchema,
  type AIbuddyAvailablePluginSummary,
  type AIbuddyInstalledPluginSummary,
  type AIbuddyPluginComponentGroup,
  type AIbuddyPluginDiagnostic,
  type AIbuddyPluginInfo,
  type AIbuddyPluginMarketplaceSummary,
  type AIbuddyPluginsConfigureResult,
  type AIbuddyPluginsDescribeResult,
  type AIbuddyPluginsInstallResult,
  type AIbuddyPluginsListResult,
  type AIbuddyPluginsMarketplaceMutationResult,
  type AIbuddyPluginsOverviewResult,
  type AIbuddyPluginsRestoreBuiltinResult,
  type AIbuddyPluginsSetEnabledResult,
  type AIbuddyPluginsUninstallResult,
  type AIbuddyPluginsValidateResult,
} from "@aibuddy/shared";
import type { PluginDiagnostic, PluginMetadata } from "@aibuddy/contracts";
import {
  addAIbuddyPluginMarketplace,
  configureAIbuddyPlugin,
  describeAIbuddyPlugin,
  getAIbuddyPluginsOverview,
  installAIbuddyMarketplacePlugin,
  removeAIbuddyPluginMarketplace,
  resolveAIbuddyPlugins,
  resetAIbuddyPluginConfig,
  ...
  validateAIbuddyPlugin,
} from "../plugins.js";
import { listInstalledPluginRecords } from "@aibuddy/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@aibuddy/adapters/config";
import { parseParams, type AIbuddyProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 AIbuddyPluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): AIbuddyPluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): AIbuddyPluginDiagnostic {
  return {
...
export async function listPlugins(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyPluginsListResult> {
  const params = parseParams(aibuddyPluginsListParamsSchema, rawParams);
...
): Promise<AIbuddyPluginsSetEnabledResult> {
  const params = parseParams(aibuddyPluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setAIbuddyPluginEnabled({
...
export async function getPluginsOverview(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyPluginsOverviewResult> {
  const params = parseParams(aibuddyPluginsOverviewParamsSchema, rawParams);
  const overview = getAIbuddyPluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(aibuddyPluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(aibuddyPluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(aibuddyPluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<AIbuddyPluginsInstallResult> {
  const params = parseParams(aibuddyPluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installAIbuddyMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyPluginsUninstallResult> {
  const params = parseParams(aibuddyPluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallAIbuddyMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(aibuddyPluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: AIbuddyInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installAIbuddyMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 AIbuddyPluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: AIbuddyPluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installAIbuddyMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(aibuddyPluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(aibuddyPluginsConfigureParamsSchema, rawParams);
  await configureAIbuddyPlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(aibuddyPluginsResetConfigParamsSchema, rawParams);
  await resetAIbuddyPluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyPluginsValidateResult> {
  const params = parseParams(aibuddyPluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyPluginsDescribeResult> {
  const params = parseParams(aibuddyPluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeAIbuddyPlugin({
      logger: context.logger,
  ...
  const components: AIbuddyPluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: AIbuddyPluginMarketplaceSummary["refreshFailure"];
}): AIbuddyPluginMarketplaceSummary {
  return {
    id: input.id,
    name: input.name,
    source: input.source,
...
function toAvailablePluginSummary(input: {
  ...
  listing?: AIbuddyAvailablePluginSummary["listing"];
}): AIbuddyAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: AIbuddyInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: AIbuddyInstalledPluginSummary["listing"];
}): AIbuddyInstalledPluginSummary {
  return {
    id: input.id,
    name: input.name,
    marketplace: input.marketplace,
