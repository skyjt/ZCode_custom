...
import { createInMemorySessionEventStore } from "@aibuddy/adapters/storage";
import type { ModelSelection } from "@aibuddy/contracts";
import {
  aibuddyProviderTestModelConnectivityParamsSchema,
  aibuddyWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(aibuddyWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(aibuddyProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceAIbuddyApp(
  context: AIbuddyProtocolAgentServerContext,
  ...
  return context.deps.createAIbuddyApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createAIbuddyApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<AIbuddyProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: AIbuddyProtocolAgentServerContext,
  record: Pick<AIbuddyProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
