...
import type { AIbuddySessionEndedSubagent, AIbuddySessionRunningSubagent } from "@aibuddy/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: AIbuddySessionRunningSubagent[];
  ended: AIbuddySessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): AIbuddySessionRunningSubagent["status"] | undefined {
...
): AIbuddySessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): AIbuddySessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: AIbuddySessionRunningSubagent[] = [];
  const ended: AIbuddySessionEndedSubagent[] = [];
...
function encodeCursor(item: AIbuddySessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly AIbuddySessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: AIbuddySessionEndedSubagent[]; nextCursor?: string } {
