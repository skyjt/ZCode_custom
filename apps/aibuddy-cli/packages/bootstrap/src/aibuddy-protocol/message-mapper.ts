import type { AIbuddyMessagePart, AIbuddyMessageWithParts, AIbuddyToolState } from "@aibuddy/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@aibuddy/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): AIbuddyMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): AIbuddyMessagePart {
  const base = {
...
function mapToolState(state: ToolState): AIbuddyToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // AIbuddy protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
