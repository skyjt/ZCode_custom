import type { Logger, McpPort } from "@aibuddy/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@aibuddy/adapters/mcp";
import type { SqliteSessionStore } from "@aibuddy/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@aibuddy/telemetry";
...
import type { AIbuddyProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<AIbuddyProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<AIbuddyProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`AIbuddy Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `aibuddy_protocol.${resource}.shutdown.failed`,
