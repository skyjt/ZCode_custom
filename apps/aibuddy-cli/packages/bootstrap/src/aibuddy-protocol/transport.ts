...
import { aibuddyProtocolMessageSchema, aibuddyProtocolMethods } from "@aibuddy/shared";
import type { Logger } from "@aibuddy/contracts";

type AIbuddyProtocolOutgoingMessage =
  | AIbuddyProtocolError
  | AIbuddyProtocolNotification
  | AIbuddyProtocolRequest
  | AIbuddyProtocolResponse;

type AIbuddyProtocolMessageHandler = (
  message: AIbuddyProtocolMessage,
) => Promise<AIbuddyProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface AIbuddyProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: AIbuddyProtocolMessageHandler;
  ...
  ) => readonly AIbuddyProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: AIbuddyProtocolRequestId) => {
    readonly messages: readonly AIbuddyProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class AIbuddyProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: AIbuddyProtocolNdjsonConnectionOptions) {
  ...
  send(message: AIbuddyProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("AIbuddy Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("AIbuddy Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): AIbuddyProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("AIbuddy Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "aibuddy_protocol.parse.failed",
    ...
    const parsed = aibuddyProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid AIbuddy Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: AIbuddyProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === aibuddyProtocolMethods.sessionStop ||
        message.method === aibuddyProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: AIbuddyProtocolMessage): Promise<void> {
    if (this.terminal) return;
