import {
  ...
  type AIbuddyActiveToolCall,
  type AIbuddyContextUsageBreakdownItem,
  type AIbuddyDeliveryKind,
  type AIbuddyPendingPermission,
  type AIbuddySessionContextUsage,
  type AIbuddySessionEvent,
  type AIbuddySessionGoal,
  type AIbuddySessionGoalVerification,
  type AIbuddySessionGoalVerificationTimeline,
  type AIbuddySessionGoalStats,
  type AIbuddySessionInfo,
  type AIbuddySessionKind,
  type AIbuddySessionProjection,
  type AIbuddySessionRuntimeState,
  type AIbuddySessionSettingsState,
  type AIbuddySessionStateSnapshot,
  type AIbuddySessionTodoGroup,
...
import type { AIbuddyApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: AIBUDDY_PROTOCOL_NAME,
      version: AIBUDDY_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<AIbuddyApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 aibuddy-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("aibuddy-artifact://") ? artifactUri : undefined;
}
...
): Promise<AIbuddySessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<AIbuddyApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as AIbuddySessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is AIbuddySessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = aibuddyApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const aibuddyMeta = asRecord(meta.aibuddy);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = aibuddyApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): AIbuddySessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<AIbuddyApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: AIbuddyDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): AIbuddySessionContextUsage | undefined {
...
): AIbuddySessionGoalVerification[] {
...
): AIbuddySessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: AIbuddySessionContextUsage["cache"] | undefined,
): AIbuddySessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): AIbuddySessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): AIbuddyPendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 aibuddyPendingPermissionSchema（shared/src/aibuddy-protocol/index.ts:1139）和
  // aibuddyPermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 aibuddyPermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): AIbuddyActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): AIbuddySessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): AIbuddySessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): AIbuddySessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, AIbuddySessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, AIbuddySessionTodoGroup>,
  input: {
  ...
  const group: AIbuddySessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): AIbuddySessionEvent["type"] {
  switch (type) {
