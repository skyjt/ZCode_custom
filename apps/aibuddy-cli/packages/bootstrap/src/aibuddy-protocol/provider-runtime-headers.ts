...
import {
  ...
  aibuddyProviderRuntimeHeadersResponseSchema,
  type AIbuddyWorkspaceRef,
} from "@aibuddy/shared";
import type { AIbuddyAppOptions } from "../app/types.js";
import {
...
): NonNullable<AIbuddyAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          aibuddyProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: aibuddyProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
