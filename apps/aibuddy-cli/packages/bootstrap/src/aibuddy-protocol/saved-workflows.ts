...
// 解析器与序列化器只从 @aibuddy/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.aibuddy/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@aibuddy/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  AIBUDDY_WORKFLOWS_RUNS_MAX_LIMIT,
  aibuddyWorkflowsDeleteParamsSchema,
  aibuddyWorkflowsGetParamsSchema,
  aibuddyWorkflowsListParamsSchema,
  aibuddyWorkflowsMoveParamsSchema,
  aibuddyWorkflowsRunsParamsSchema,
  aibuddyWorkflowsUpdateMetaParamsSchema,
  type AIbuddySavedWorkflowRun,
  type AIbuddySavedWorkflowScope,
  type AIbuddyWorkflowsDeleteResult,
  type AIbuddyWorkflowsGetResult,
  type AIbuddyWorkflowsListResult,
  type AIbuddyWorkflowsMoveResult,
  type AIbuddyWorkflowsRunsResult,
  type AIbuddyWorkflowsUpdateMetaResult,
} from "@aibuddy/shared";
import type { JournalStorePort } from "@aibuddy/dynamic-workflow";
...
function scopeOf(params: { scope?: AIbuddySavedWorkflowScope }): AIbuddySavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<AIbuddyWorkflowsListResult> {
  const params = parseParams(aibuddyWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyWorkflowsGetResult> {
  const params = parseParams(aibuddyWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(aibuddyWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(aibuddyWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(aibuddyWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(AIBUDDY_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: AIbuddySavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, AIbuddySavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, AIbuddySavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: AIbuddyProtocolAgentServerContext,
  rawParams: unknown,
): Promise<AIbuddyWorkflowsMoveResult> {
  const params = parseParams(aibuddyWorkflowsMoveParamsSchema, rawParams);
