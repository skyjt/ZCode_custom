...
import { createNodeModelSelectionFacade } from "@aibuddy/provider-node";
import { createNodeLoggerFactory } from "@aibuddy/adapters/logging";
import {
  createMcpAdapterConnectionPool,
  createMcpTelemetryTracker,
  type McpConnectionPool,
  type McpTelemetryTracker,
} from "@aibuddy/adapters/mcp";
import {
  aibuddyProtocolNotifications,
  type AIbuddyMcpResourceSample,
  type AIbuddyMcpTelemetryEvent,
} from "@aibuddy/shared";
import type { SqliteSessionStore } from "@aibuddy/adapters/storage";
import { traceContextToLogContext, createRootTraceContext } from "@aibuddy/contracts";
import type { McpPort, ModelSelection } from "@aibuddy/contracts";
import type { PresentationSurface } from "@aibuddy/core";
import type { RunAIbuddyProtocolAgentOptions, AIbuddyAppOptions } from "./app/types.js";
import { createAIbuddyApp } from "./app/create-app.js";
import {
...
} from "./aibuddy-protocol/storage-startup.js";
...
import { installAIbuddyProtocolAiSdkWarningLogger } from "./aibuddy-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./aibuddy-protocol/official-mcp-auth-port.js";
import {
  createOfficialMcpTrustedOriginRegistry,
  OFFICIAL_MCP_DEV_TRUSTED_ORIGINS_ENV,
  AIBUDDY_WORKSPACE_IDENTITY_ENV,
  resolveRuntimeAIbuddyEndpointOrigin,
} from "@aibuddy/shared";
import { AIbuddyProtocolAgentServer } from "./aibuddy-protocol/server.js";
import { AIbuddyProtocolNdjsonConnection } from "./aibuddy-protocol/transport.js";
import { cleanupProtocolRuntime } from "./aibuddy-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./aibuddy-protocol/resource-sampler.js";
import { acquireProtocolStartupResource } from "./aibuddy-protocol/startup-resource.js";
import type { AIbuddyProcessResourceSampler } from "./process-resource-sampler.js";
import { prepareAIbuddyTelemetryEnv, shutdownAIbuddyTelemetry } from "./telemetry-bootstrap.js";

function applyProtocolPresentationSurface(
  options: Omit<AIbuddyAppOptions, "providerRegistry">,
  presentationSurface: PresentationSurface,
): Omit<AIbuddyAppOptions, "providerRegistry"> {
  return {
    ...options,
    runtimeConfig: {
...
function applyProtocolProviderRegistry(
  options: Omit<AIbuddyAppOptions, "providerRegistry">,
  providerRegistry: AIbuddyAppOptions["providerRegistry"],
...
export async function runAIbuddyProtocolAgent(
  options: RunAIbuddyProtocolAgentOptions = {},
): Promise<void> {
  ...
  const traceContext = createRootTraceContext({
    attributes: {
      entrypoint: "aibuddy_protocol",
    },
  });
  const logger = loggerFactory.createLogger("aibuddy").child({
    ...traceContextToLogContext(traceContext),
    module: "bootstrap.aibuddy_protocol",
  });
  installAIbuddyProtocolAiSdkWarningLogger(logger);
  const startupTimer = new StartupTimer(
    logger,
    {
      ...traceContextToLogContext(traceContext),
      module: "bootstrap.aibuddy_protocol",
      startupKind: "aibuddy_protocol_agent",
    },
    startupStartedAt,
  );
  startupTimer.start("AIbuddy Protocol agent startup started", {
    context: { version: options.version },
    event: "aibuddy_protocol.startup.started",
    stage: "start",
  });

  let sessionStore: SqliteSessionStore | undefined;
  let serverForCleanup: AIbuddyProtocolAgentServer | undefined;
  ...
  let mcpResourceSink: ((samples: AIbuddyMcpResourceSample[]) => void) | undefined;
  let mcpTelemetrySink: ((event: AIbuddyMcpTelemetryEvent) => void) | undefined;
  let processResourceSampler: AIbuddyProcessResourceSampler | undefined;
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "aibuddy_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    logger.info("Worker Provider Registry 已就绪", {
      ...
      event: "aibuddy_protocol.provider_registry.ready",
      module: "bootstrap.aibuddy_protocol",
    ...
    const telemetryEnv = await acquireProtocolStartupResource({
      signal: options.lifecycle?.signal,
      logger,
      disposeLate: () => shutdownAIbuddyTelemetry(),
      create: () =>
        prepareAIbuddyTelemetryEnv(runtimeEnv, {
          cliVersion: options.version,
          productVersion: options.env?.AIBUDDY_APP_VERSION,
          runtimeSurface,
        }),
    });
    const telemetryDeviceMid = telemetryEnv.AIBUDDY_TELEMETRY_DEVICE_MID;
    mcpTelemetryTracker =
    ...
    // 与下面 trustedOrigins 的 resolveAIbuddyApiOrigin 必须是同一个表达式，否则两侧判定分叉。
    const resolveAIbuddyApiOrigin = (): string =>
      resolveRuntimeAIbuddyEndpointOrigin(options.env ?? process.env);
    const workspaceIdentity = (options.env ?? process.env)[AIBUDDY_WORKSPACE_IDENTITY_ENV]?.trim();
    const officialMcpAuth = {
      ...
      resolveAIbuddyApiOrigin,
      ...(workspaceIdentity ? { workspaceIdentity } : {}),
      // 信任判定只看一条：目标 origin 等于当前 AIbuddy API origin（https）。pluginId 不参与。
    ...
    options.lifecycle?.signal.throwIfAborted();
    const server = (serverForCleanup = new AIbuddyProtocolAgentServer({
      createAIbuddyApp: (appOptions = {}) =>
        createAIbuddyApp({
          ...
          env: {
            ...telemetryEnv,
            ...appOptions.env,
            ...(telemetryDeviceMid ? { AIBUDDY_TELEMETRY_DEVICE_MID: telemetryDeviceMid } : {}),
          },
          ...
          onToolExecResource: (params) =>
            connection.send({ method: aibuddyProtocolNotifications.toolExecResource, params }),
        }),
      cwd: options.cwd,
      env: options.env,
      loggerFactory,
    ...
    const connection = new AIbuddyProtocolNdjsonConnection({
      signal: options.lifecycle?.signal,
    ...
    mcpResourceSink = (samples) =>
      connection.send({
        method: aibuddyProtocolNotifications.mcpResourceSamples,
        params: samples,
      });
    mcpTelemetrySink = (event) => {
      ...
      connection.send({
        method: aibuddyProtocolNotifications.mcpTelemetry,
        params: event,
      });
    };
    connection.start();
    ...
    startupTimer.complete("AIbuddy Protocol agent startup completed", {
      event: "aibuddy_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("AIbuddy Protocol agent startup failed", error, {
      event: "aibuddy_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
    ...
    logger.info("AIbuddy Protocol agent shutdown completed", {
      ...traceContextToLogContext(traceContext),
      event: "aibuddy_protocol.shutdown.completed",
      module: "bootstrap.aibuddy_protocol",
      status: "completed",
    });
  }
...
function resolveProtocolRuntimeSurface(
  ...
  return env.AIBUDDY_TELEMETRY_RUNTIME_SURFACE?.trim() === "remote_workspace_host"
