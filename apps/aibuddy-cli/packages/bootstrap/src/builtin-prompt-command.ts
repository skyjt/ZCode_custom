...
interface ResolveAIbuddyBuiltinPromptCommandOptions {
  workingDirectory?: string;
}

export function resolveAIbuddyBuiltinPromptCommand(
  input: string,
  options: ResolveAIbuddyBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running AIbuddy's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future AIbuddy agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".aibuddy", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.aibuddy/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .aibuddy/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future AIbuddy agents would otherwise miss.",
