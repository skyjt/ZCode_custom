...
import type { Logger, SkillContent, SkillDiagnostic, SkillLoadOutcome } from "@aibuddy/contracts";
import { resolveAIbuddyPlugins } from "./plugins.js";
import { collectDisabledPaths } from "./skill-command-overrides.js";

export interface ListAIbuddySkillsOptions {
...
export interface InspectAIbuddySkillOptions extends ListAIbuddySkillsOptions {
  name: string;
}

export interface AIbuddySkillInspection {
  diagnostics: SkillDiagnostic[];
  skill: SkillContent;
}

export async function listAIbuddySkills(
  options: ListAIbuddySkillsOptions = {},
...
export async function inspectAIbuddySkill(
  options: InspectAIbuddySkillOptions,
): Promise<AIbuddySkillInspection> {
  const discovery = createSkillDiscovery(options);
  if (!discovery.enabled) {
...
function createSkillDiscovery(options: ListAIbuddySkillsOptions):
  | {
      enabled: false;
      workingDirectory: string;
    }
