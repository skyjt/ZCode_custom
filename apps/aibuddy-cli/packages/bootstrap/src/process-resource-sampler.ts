...
import {
  AIBUDDY_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type AIbuddyProcessResourceSample,
} from "@aibuddy/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const AIBUDDY_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = AIBUDDY_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateAIbuddyProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: AIbuddyProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: AIbuddyProcessResourceSample["platform"];
  arch?: AIbuddyProcessResourceSample["arch"];
...
export interface AIbuddyProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createAIbuddyProcessResourceSampler(
  options: CreateAIbuddyProcessResourceSamplerOptions,
): AIbuddyProcessResourceSampler {
  const platform = options.platform ?? (process.platform as AIbuddyProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as AIbuddyProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: AIbuddyProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, AIBUDDY_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
