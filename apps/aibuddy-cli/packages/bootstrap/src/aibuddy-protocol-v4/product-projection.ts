import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@aibuddy/shared/aibuddy-protocol-v4";
...
import {
  ...
  isAIbuddyFileStreamingToolInputPreviewTool,
  parseAIbuddyBackgroundTaskNotificationText,
  resolveAIbuddyBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseAIbuddyBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = aibuddyBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isAIbuddyFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < AIBUDDY_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveAIbuddyBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @aibuddy/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
