...
import { getModelUsageTotalTokens, SessionEventType } from "@aibuddy/contracts";
import { parseAutomationRunId } from "@aibuddy/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const aibuddy = recordValue(meta.aibuddy);
  return (
    ...
    optionalString(aibuddy.parentToolCallId) ??
    optionalString(aibuddy.parentToolUseId)
  );
}
