...
import { localTtftNow, localTtftFactsSchema } from "@aibuddy/shared/aibuddy-protocol-v4";
import {
...
import type { ConversationSnapshot } from "@aibuddy/shared/aibuddy-protocol-v4";
import { SessionEventType, isFileSystemPortError } from "@aibuddy/contracts";
import type { AIbuddyWorkspaceRef } from "@aibuddy/shared";
import { extractMarkdownArtifactImageRefs } from "@aibuddy/shared";
import type {
  CommandAck,
  AttachmentRef,
  CommandEnvelope,
...
export interface V4GatewayHost {
  ...
  emitLocalTtftFacts?(facts: import("@aibuddy/shared").LocalTtftFacts): void;
...
function artifactRefBelongsToSession(ref: string, sessionId: string): boolean {
  return ref.startsWith(`aibuddy-artifact://${encodeURIComponent(sessionId)}/`);
}
...
function toShareStatFault(error: unknown): unknown {
  if (readAIbuddyAttachmentFaultCode(error)) return error;
  if (isFileSystemPortError(error) && MISSING_ATTACHMENT_FS_CODES.has(error.code)) {
    return new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotFound, {
      cause: error,
    });
  }
  return error;
...
export class ConversationV4Gateway {
   ...
   * effect 之前，而 CLI 严格串行派发请求（`aibuddy-protocol/transport.ts`，只有 session/stop
  ...
  async conversationAttachmentRead(
    ...
    if (!this.host.readSessionAttachment) {
      throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.readUnsupported);
    }
    const existingReady = this.readyFlights.get(params.sessionId);
    const publisher = existingReady
      ? await existingReady
    ...
    if (row?.kind !== "userInput") {
      throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized);
    }
  ...
  async conversationAttachmentStat(
    ...
    if (row?.kind !== "userInput") {
      throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    }
    const attachment = row.attachments?.[params.attachmentIndex];
    if (!attachment || (attachment.ref !== params.ref && attachment.previewRef !== params.ref)) {
      throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    ...
    if (result.totalBytes > PROTOCOL_V4_LIMITS.attachmentStatMaxBytes) {
      throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatTooLarge);
    }
    ...
    const payload = this.host.readSessionAttachment!(sessionId, {
      ...
      .then((result) => {
        ...
        if (
          ...
          throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.previewNotMedia);
        }
        if (result.bytes.byteLength > maxBytes) {
          throw new AIbuddyAttachmentFaultError(AIBUDDY_ATTACHMENT_FAULT_CODES.previewTooLarge);
        }
        const current = this.binaryReadCache.get(key);
        if (current) {
  ...
  private emitReservation<F extends RoutedTopicFrame>(
    ...
    if (
      ...
      const observations: import("@aibuddy/shared").LocalTtftFacts[] = [];
