...
import { AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@aibuddy/contracts";
import { AIBUDDY_CUA_OFFICIAL_PLUGIN_ID, isAIbuddyCuaInternalFeatureEnabled } from "@aibuddy/shared";
...
export interface ResolveAIbuddyPluginsOptions {
...
export interface ListAIbuddyPluginsOptions extends ResolveAIbuddyPluginsOptions {}

export interface SetAIbuddyPluginEnabledOptions extends ResolveAIbuddyPluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetAIbuddyPluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface AIbuddyMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface AIbuddyAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface AIbuddyInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface AIbuddyPluginsOverviewData {
  marketplaces: AIbuddyMarketplaceSummaryData[];
  availablePlugins: AIbuddyAvailablePluginData[];
  installedPlugins: AIbuddyInstalledPluginData[];
  restorableBuiltins: AIbuddyAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface AIbuddyMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: AIbuddyMarketplaceSummaryData[];
}

export interface AddAIbuddyMarketplaceOptions extends ResolveAIbuddyPluginsOptions {
...
export interface RemoveAIbuddyMarketplaceOptions extends ResolveAIbuddyPluginsOptions {
  marketplace: string;
}

export interface UpdateAIbuddyMarketplaceOptions extends ResolveAIbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallAIbuddyMarketplacePluginOptions extends ResolveAIbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallAIbuddyMarketplacePluginOptions extends ResolveAIbuddyPluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`aibuddy plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateAIbuddyMarketplacePluginOptions extends ResolveAIbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateAIbuddyPluginPathOptions extends ResolveAIbuddyPluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface AIbuddyPluginUpdateData extends AIbuddyPluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveAIbuddyPluginsOptions {
  pluginId: string;
}

interface ConfigureAIbuddyPluginOptions extends ResolveAIbuddyPluginsOptions {
...
interface ResetAIbuddyPluginConfigOptions extends ResolveAIbuddyPluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateAIbuddyPluginOptions extends ResolveAIbuddyPluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeAIbuddyPluginOptions extends ResolveAIbuddyPluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface AIbuddyPluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveAIbuddyPlugins(options: ResolveAIbuddyPluginsOptions = {}): PluginLoadOutcome {
...
export function getAIbuddyPluginsOverview(
  options: ResolveAIbuddyPluginsOptions = {},
): AIbuddyPluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveAIbuddyPlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  const catalogs: Array<{
    summary: AIbuddyMarketplaceSummaryData;
  ...
  const restorableBuiltins: AIbuddyAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isAIbuddyCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
...
export function listAIbuddyPlugins(options: ListAIbuddyPluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveAIbuddyPlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setAIbuddyPluginEnabled(
  options: SetAIbuddyPluginEnabledOptions,
): Promise<SetAIbuddyPluginEnabledResult> {
...
export async function addAIbuddyPluginMarketplace(
  options: AddAIbuddyMarketplaceOptions,
): Promise<AIbuddyMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeAIbuddyPluginMarketplace(
  options: RemoveAIbuddyMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateAIbuddyPluginMarketplace(
  options: UpdateAIbuddyMarketplaceOptions,
): Promise<AIbuddyMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installAIbuddyMarketplacePlugin(
  options: InstallAIbuddyMarketplacePluginOptions,
): Promise<AIbuddyPluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallAIbuddyMarketplacePlugin(
  options: UninstallAIbuddyMarketplacePluginOptions,
): Promise<AIbuddyInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
...
/**
 * `aibuddy plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateAIbuddyMarketplacePlugin(
  options: UpdateAIbuddyMarketplacePluginOptions,
): Promise<AIbuddyPluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateAIbuddyPluginMarketplace({
    ...options,
  ...
  const installed = await installAIbuddyMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `aibuddy plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateAIbuddyPluginPath(
  options: ValidateAIbuddyPluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const aibuddyCuaPluginId = AIBUDDY_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === aibuddyCuaPluginId &&
    !isAIbuddyCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires AIBUDDY_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureAIbuddyPlugin(options: ConfigureAIbuddyPluginOptions): Promise<void> {
...
export async function resetAIbuddyPluginConfig(
  options: ResetAIbuddyPluginConfigOptions,
...
export async function validateAIbuddyPlugin(
  options: ValidateAIbuddyPluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeAIbuddyPlugin(
  options: DescribeAIbuddyPluginOptions,
...
function resolvePluginContext(options: ResolveAIbuddyPluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallAIbuddyMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveAIbuddyPluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.aibuddy/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".aibuddy", "config.json");
