...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@aibuddy/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@aibuddy/core";
import type { Logger, PluginMetadata } from "@aibuddy/contracts";
import {
  createAgentStateId,
...
interface LoadAIbuddyAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadAIbuddyAgentProfilesResult {
...
export async function loadAIbuddyAgentProfiles(
  input: LoadAIbuddyAgentProfilesInput,
): Promise<LoadAIbuddyAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".aibuddy", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .aibuddy/agents/*.md 是仓库内容，不能通过 frontmatter
...
): LoadAIbuddyAgentProfilesResult {
  const diagnostics: AgentProfileParseDiagnostic[] = [];
