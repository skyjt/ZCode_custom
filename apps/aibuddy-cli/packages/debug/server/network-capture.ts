...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          AIBUDDY_HTTP_PROXY: url,
          AIBUDDY_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.aibuddyCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.AIBUDDY_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.AIBUDDY_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.AIBUDDY_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.AIBUDDY_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.AIBUDDY_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
