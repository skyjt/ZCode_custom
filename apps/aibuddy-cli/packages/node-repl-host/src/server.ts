...
import { JsInputJsonSchema } from "@aibuddy/contracts/tools/node-repl";
// 值导入必须走 @aibuddy/core/repl 这条深路径：barrel 会把 core 的整张图拖进 bundle
// （tool handlers → @aibuddy/dynamic-workflow → typescript，实测 21.7MB 且求值即崩
...
} from "@aibuddy/core/repl";
import { createComputerUseRuntime, type ComputerUseRuntime } from "@aibuddy/aibuddy-cua";
import { z } from "zod";
...
const WORKER_KIND = "aibuddy-node-repl-call";
export const NODE_REPL_MCP_PROCESS_TITLE = "aibuddy-node-repl-mcp";
const pluginRoot = process.env.AIBUDDY_PLUGIN_ROOT ?? process.cwd();
...
const cuaDocumentationRoot = resolve(
  process.env.AIBUDDY_CUA_PLUGIN_ROOT ?? pluginRoot,
  "docs",
);
...
function buildRequestMeta(meta: Record<string, unknown> | undefined): NodeReplRequestMeta {
  ...
  // 安全边界：顶层 MCP _meta 是第三方可扩展字段，不能成为 AIbuddy session 路由凭据。
...
export function captureComputerUseRuntimeFromEnvironment(
  ...
  const socketPath = env.AIBUDDY_CUA_PERMISSION_BROKER_SOCKET?.trim();
  if (!socketPath) return undefined;
  return createComputerUseRuntime({
    brokerSocketPath: socketPath,
    refreshMarkerPath: env.AIBUDDY_CUA_PERMISSION_BROKER_REFRESH_MARKER?.trim(),
  });
}
