...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "aibuddy.agent.turn.duration"),
      agent_step: duration(meter, "aibuddy.agent.step.duration"),
      tool_execution: duration(meter, "aibuddy.tool.execution.duration"),
      command_execution: duration(meter, "aibuddy.command.execution.duration"),
      context_compaction: duration(meter, "aibuddy.context.compaction.duration"),
      detached_operation: duration(meter, "aibuddy.detached.operation.duration"),
      model_call: duration(meter, "aibuddy.model.call.duration"),
      model_attempt: duration(meter, "aibuddy.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("aibuddy.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("aibuddy.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "aibuddy.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "aibuddy.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "aibuddy.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("aibuddy.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "aibuddy.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "aibuddy.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("aibuddy.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("aibuddy.telemetry.abandoned.count", {
