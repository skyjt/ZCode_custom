...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@aibuddy/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 AIbuddy Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "aibuddy.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "aibuddy.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "aibuddy.execution.turn_id": safeId(correlation.turnId),
    "aibuddy.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "aibuddy.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "aibuddy.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "aibuddy.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "aibuddy.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "aibuddy.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "aibuddy.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
