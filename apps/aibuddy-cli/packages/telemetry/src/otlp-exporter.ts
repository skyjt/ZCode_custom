...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@aibuddy/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@aibuddy/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("aibuddy.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("aibuddy.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "aibuddy.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "aibuddy.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("aibuddy.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("aibuddy.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "aibuddy.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("aibuddy.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("aibuddy.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("aibuddy.model.attempt.tokens"),
    cardinalityView("aibuddy.model.attempt.stream_stall.count"),
    cardinalityView("aibuddy.telemetry.creation_drop.count"),
    cardinalityView("aibuddy.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "aibuddy.build.commit_id": resource.buildCommitId,
    "aibuddy.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "aibuddy.product.version": resource.productVersion,
    "aibuddy.runtime.distribution": resource.runtimeDistribution,
    "aibuddy.runtime.surface": resource.runtimeSurface,
    "aibuddy.telemetry.schema_owner": "cli",
    "aibuddy.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
