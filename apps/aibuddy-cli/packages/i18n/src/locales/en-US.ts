...
export const enUS: AIbuddyCopy = {
  ...
  cli: {
    ...
    help: (version) => `aibuddy ${version}

Usage:
  aibuddy [command] [options]

With no command, aibuddy opens the full-screen TUI.

Commands:
  app-server Run the AIbuddy Protocol stdio app server
...
`,
  ...
  tui: {
    ...
    loginRequired: {
      help: "Configure an API provider in AIbuddy Settings, then use /model to select it.",
      message:
        "No available models. Configure an API provider with a Base URL, API key and model ID.",
      status:
        "No available models. Configure an API provider with a Base URL, API key and model ID.",
    ...
    terminal: {
      ...
      starting: "Starting AIbuddy... Ctrl+C to exit",
    },
    transcript: {
      compact: {
