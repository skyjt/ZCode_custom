...
不传 command 时，aibuddy 会打开全屏 TUI。

命令:
  app-server 运行 AIbuddy Protocol stdio app server
...
`,
  ...
  tui: {
    ...
    loginRequired: {
      help: "在 AIbuddy 设置中配置 API 供应商，再输入 /model 选择模型。",
      message: "没有可用模型，请配置 API 供应商的 Base URL、API Key 和模型 ID。",
      status: "没有可用模型，请配置 API 供应商的 Base URL、API Key 和模型 ID。",
      title: "需要配置模型",
    },
    model: {
    ...
    terminal: {
      starting: "正在启动 AIbuddy… Ctrl+C 退出",
