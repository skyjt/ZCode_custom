...
import type { McpServerConfig, RuntimeConfigPatch } from "@aibuddy/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 AIbuddyConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".aibuddy" ? dirname(configDirectory) : configDirectory;
}
