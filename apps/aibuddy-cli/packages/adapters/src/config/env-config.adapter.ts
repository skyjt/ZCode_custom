// Env Config Adapter - Parse the intentionally small AIBUDDY_* environment surface.
...
const DEFAULT_PREFIX = "AIBUDDY_";

/**
 * Parse AIBUDDY_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.AIBUDDY_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
