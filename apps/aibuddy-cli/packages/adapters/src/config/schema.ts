/* eslint-disable max-lines -- aibuddy-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
// 上游已发布的插件 ID 参与配置迁移，产品改名不能改变其匹配值。
...
export const AIbuddyConfigFileSchema = z
  .object({
...
export type AIbuddyConfigFile = z.infer<typeof AIbuddyConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. AIbuddy does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；AIbuddy 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = AIbuddyConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: AIbuddyConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: AIbuddyConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: AIbuddyConfigFile["skills"],
