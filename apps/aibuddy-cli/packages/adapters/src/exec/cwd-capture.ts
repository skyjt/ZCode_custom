...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `aibuddy-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__aibuddy_status=$?",
    `if [ "$__aibuddy_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__aibuddy_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__aibuddy_status=%ERRORLEVEL%"',
    `if "%__aibuddy_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__aibuddy_status%",
  ].join("\r\n");
}
