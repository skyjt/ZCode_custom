...
import { windowsPathToGitBashPath } from "@aibuddy/contracts";
import type { ExecutionShellDialect } from "@aibuddy/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_AIBUDDY_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_AIBUDDY_SHELL_INIT_SNAPSHOT",
  ];
}
