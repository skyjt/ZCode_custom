...
import { AIBUDDY_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type AIbuddyMcpResourceSample } from "@aibuddy/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: AIbuddyMcpResourceSample["arch"];
  platform: AIbuddyMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: AIbuddyMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const groups = new Map<string, AIbuddyMcpResourceSample>();
