...
import type { AIbuddyMcpTelemetryEvent } from "@aibuddy/shared";
import {
...
type McpTelemetryEvent = AIbuddyMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: AIbuddyMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: AIbuddyMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as AIbuddyMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as AIbuddyMcpTelemetryEvent["platform"]);
