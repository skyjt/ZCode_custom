...
import { acquireFileLock } from "@aibuddy/shared/node";
import { AIBUDDY_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@aibuddy/shared";
import type { SharedAIbuddyCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === AIBUDDY_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
