...
import { sanitizeAIbuddyRuntimeEnv } from "@aibuddy/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeAIbuddyRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.aibuddy/server/node 启动，但登录环境 PATH 不含该目录，
