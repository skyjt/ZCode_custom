...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@aibuddy/contracts";
import { LogLevel, LogLevelName } from "@aibuddy/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.AIBUDDY_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.AIBUDDY_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".aibuddy", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeAIbuddyRuntimeEnv(env[AIBUDDY_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `aibuddy-${formatLocalLogDate(new Date())}.jsonl`;
}
