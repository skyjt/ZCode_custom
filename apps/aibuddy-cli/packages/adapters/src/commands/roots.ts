...
import type { CustomCommandRoot, CustomCommandSource } from "@aibuddy/contracts";

const COMMANDS_DIR = "commands";
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const PRIORITY_STEP = 10;
const AIBUDDY_DIR = ".aibuddy";
...
export interface CustomCommandRootResolutionOptions {
  ...
  includeAIbuddyCommands?: boolean;
}
...
export async function resolveDefaultCustomCommandRoots(
  ...
  const includeAIbuddy = options.includeAIbuddyCommands ?? true;
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.aibuddy` 命令需要同时可见。
  // 同一级别 `.aibuddy` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, AIBUDDY_DIR, COMMANDS_DIR), scope, "aibuddy", nextPriority()),
