 ...
 */
import type { McpOfficialProvenance, AIbuddyOfficialMcpAuthConfig } from "@aibuddy/contracts";
 ...
 */
export function parseAIbuddyOfficialAuth(
  value: unknown,
  mcpKey: string,
): AIbuddyOfficialMcpAuthConfig | undefined {
  if (value === undefined) return undefined;
  if (!isRecord(value)) {
    throw new Error(`MCP server ${mcpKey}: auth must be an object`);
  }
  // 精确值匹配，区分大小写；aibuddy-official、zcode_official_auth 等别名一律拒绝。
