...
import {
  ...
  AIBUDDY_INLINE_PLUGIN_MARKETPLACE,
  AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE,
} from "@aibuddy/contracts";
import {
  directoryExists,
  fileExists,
  ...
  for (const [eventName, matcherConfigs] of Object.entries(hooksRoot)) {
    if (!SUPPORTED_HOOK_EVENTS.has(eventName)) {
      input.diagnostics.push({
        ...
        message: `Plugin hook event is not supported by this AIbuddy runtime: ${eventName}`,
        path: input.sourcePath,
        pluginId: input.loaded.id,
        severity: "warning",
      });
...
function materializeCommandMetadataRoot(
  ...
  for (const [rawName, rawMetadata] of Object.entries(spec)) {
    ...
    // AIbuddy 的 custom command loader 只扫描 markdown 根目录，因此把低风险命令内容
...
function scanOfficialCache(
  ...
  const cacheRoot = join(storageRoot, "cache", AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE);
  try {
    const roots: string[] = [];
...
function findManifest(rootPath: string): string | null {
  const aibuddyPath = join(rootPath, AIBUDDY_MANIFEST_PATH);
  if (fileExists(aibuddyPath)) {
    return aibuddyPath;
  }

  // 上游市场仍发布旧格式；本地优先读取 AIbuddy 清单，兼容读取已发布插件。
  const upstreamPath = join(rootPath, UPSTREAM_MANIFEST_PATH);
