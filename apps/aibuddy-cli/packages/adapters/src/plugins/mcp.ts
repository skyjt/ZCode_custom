...
import { findOfficialMcpReservedHeaders } from "@aibuddy/shared";
import type {
  McpOAuthConfig,
  McpServerConfig,
...
import { AIBUDDY_PLUGIN_ID_ENV_KEY } from "@aibuddy/shared";
import type { LoadedPlugin } from "./types.js";
import { isNotFoundError, isPluginOptionValue, isRecord, resolveInside } from "./helpers.js";
import { buildOfficialProvenance, parseAIbuddyOfficialAuth } from "./mcp-official-auth.js";
...
function resolveMcpServerConfig(
  ...
  // AIbuddy 官方市场同时包含随应用装载的 Builtin Plugin 与按需安装的 CDN Plugin；后者运行时
  // source 为 `cache`，因此必须按 marketplace 身份归类，不能只看 loader source。
  const source: McpServerRuntimeSource = {
    kind: context.loaded.marketplace === AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE ? "builtin" : "plugin",
  };
  ...
  const officialAuth = parseAIbuddyOfficialAuth(server.auth, identity.mcpKey);
  ...
  if (type === "stdio") {
    ...
    const env = resolveStringRecord(
      {
        ...
        AIBUDDY_PLUGIN_DATA: context.dataPath,
        AIBUDDY_PLUGIN_ROOT: context.loaded.rootPath,
        AIBUDDY_PROJECT_DIR: context.workingDirectory,
        // 上游插件进程仍读取这些公开变量；只保留插件边界的兼容别名。
    ...
    // registry，不是可序列化配置），不能让第三方伪造 official aibuddy-cua 身份后获得只应定向注入给
    // 内置插件的 broker 凭据。manifest env spread 之后覆写，确保 user/manifest 无法覆盖。
    env[AIBUDDY_PLUGIN_ID_ENV_KEY] = context.loaded.id;
    env.ZCODE_PLUGIN_ID = context.loaded.id;
    return {
      type: "stdio",
