...
function rebuildOfficialMarketplaceSync(storageRoot: string): Record<string, unknown> {
  ...
  const merged = {
    ...(bundledManifest ?? {}),
    ...(cdnManifest ?? {}),
    name: AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE,
...
function assertOfficialManifest(manifest: Record<string, unknown>): void {
  if (manifest.name !== AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
    throw new Error(
      `Official marketplace manifest must be named ${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}`,
    );
  }
}

function partitionPath(storageRoot: string, fileName: string): string {
  return join(storageRoot, "marketplaces", AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE, fileName);
}
