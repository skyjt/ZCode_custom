...
import type { PluginDiagnostic, PluginManifest, PluginStoreListing } from "@aibuddy/contracts";
import { isOfficialMarketplaceId, AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE } from "@aibuddy/contracts";
import { DEFAULT_PLUGIN_MARKETPLACES, sanitizeAIbuddyRuntimeEnv } from "@aibuddy/shared";
...
const AIBUDDY_MANIFEST_PATH = join(".aibuddy-plugin", "plugin.json");
const UPSTREAM_MANIFEST_PATH = join(".zcode-plugin", "plugin.json");
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeAIbuddyRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 AIbuddy 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export async function addMarketplace(input: {
  ...
  try {
    ...
    if (
      input.trustedId === AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE &&
      loaded.manifest.name !== AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE
    ) {
      throw new Error(
        `Official marketplace source must provide ${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE}, received ${loaded.manifest.name}`,
      );
    }
    ...
    } else if (loaded.manifest.name !== AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallMarketplacePlugin(input: {
  ...
  storageRoot: string;
  removeCache?: boolean;
  /** `aibuddy plugins uninstall --keep-data`：删安装缓存但保留 data/<plugin-id> 用户数据目录。 */
  keepData?: boolean;
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-plugin-src-"));
  const args = ["clone"];
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.AIBUDDY_GIT_BINARY?.trim() || "git";
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this AIbuddy runtime: ${key}`,
