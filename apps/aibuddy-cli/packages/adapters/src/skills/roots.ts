...
import type { SkillRoot, SkillSource } from "@aibuddy/contracts";
...
export async function resolveDefaultSkillRoots(
  ...
  const includeAIbuddy = options.includeAIbuddySkills ?? true;
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.aibuddy` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.aibuddy` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, AIBUDDY_DIR, SKILLS_DIR), scope, "aibuddy", nextPriority()),
