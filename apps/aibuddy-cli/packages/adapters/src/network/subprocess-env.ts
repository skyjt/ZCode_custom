import {
  ...
  AIBUDDY_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readAIbuddyToolEnvPassthroughEnv,
} from "@aibuddy/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [AIBUDDY_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [AIBUDDY_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, AIBUDDY_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readAIbuddyToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const aibuddyProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [AIBUDDY_HTTP_PROXY_ENV_KEY], platform),
  );
  if (aibuddyProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, aibuddyProxy, platform);
    }
    return;
  }
}
