import { isRetiredProductCredentialKey } from "@aibuddy/shared";
...
import {
  createAIbuddyCredentialCipher,
  type AIbuddyCredentialCipher,
} from "./credential-cipher.js";

const AIBUDDY_DATA_BASE_DIR_ENV_KEY = "AIBUDDY_DATA_BASE_DIR";
const credentialChangeListeners = new Map<string, Set<() => void | Promise<void>>>();

export const SHARED_AIBUDDY_CREDENTIAL_KEYS = {
  ...
  aibuddyJwtToken: "zcodejwttoken",
} as const;

export interface SharedAIbuddyCredentialStoreOptions {
  baseDir?: string;
  cipher?: AIbuddyCredentialCipher;
...
export interface SharedAIbuddyCredentialStore {
...
export function createSharedAIbuddyCredentialStore(
  options: SharedAIbuddyCredentialStoreOptions = {},
): SharedAIbuddyCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedAIbuddyCredentialsPath(options);
  const cipher = options.cipher ?? createAIbuddyCredentialCipher({ env });
  ...
  return {
    ...
    async load(key: string): Promise<string | null> {
      if (isRetiredProductCredentialKey(validateCredentialKey(key))) return null;
    ...
    async loadMany(keys: readonly string[]): Promise<Record<string, string | null>> {
      ...
      return Object.fromEntries(
        validatedKeys.map((key) => {
          if (isRetiredProductCredentialKey(key)) return [key, null];
    ...
    async save(key: string, value: string): Promise<void> {
      const validatedKey = validateWritableCredentialKey(key);
    ...
    async saveMany(entries: Readonly<Record<string, string>>): Promise<void> {
      const encryptedEntries = Object.entries(entries).map(
        ([key, value]) =>
          [
            validateWritableCredentialKey(key),
            cipher.encrypt(validateCredentialValue(value)),
          ] as const,
      );
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
    ...
    async saveZaiLoginCredentials(_payload: ZaiLoginCredentialPayload): Promise<void> {
      throw new Error("Product login has been removed. Configure an API provider instead.");
    },
  };
}

export function loadSharedAIbuddyCredentialSync(
  key: string,
  options: SharedAIbuddyCredentialStoreOptions = {},
): string | undefined {
  if (isRetiredProductCredentialKey(validateCredentialKey(key))) return undefined;
  const filePath = resolveSharedAIbuddyCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createAIbuddyCredentialCipher({ env: options.env });
...
export function resolveSharedAIbuddyCredentialsPath(
  options: SharedAIbuddyCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[AIBUDDY_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".aibuddy", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared AIbuddy credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared AIbuddy credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
...
function validateWritableCredentialKey(key: string): string {
  const validated = validateCredentialKey(key);
  if (isRetiredProductCredentialKey(validated)) {
    throw new Error("Product login has been removed. Configure an API provider instead.");
  }
  return validated;
}
