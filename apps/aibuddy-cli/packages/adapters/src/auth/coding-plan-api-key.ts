import type { HttpClientPort, HttpClientRunOptions, TraceContext } from "@aibuddy/contracts";
import { resolveBigModelApiOrigin } from "@aibuddy/shared";

const ZAI_API_HOST = "https://api.z.ai";
const JSON_CONTENT_TYPE = "application/json";
const AIBUDDY_API_KEY_NAME = "aibuddy-api-key";
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === AIBUDDY_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: AIBUDDY_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
