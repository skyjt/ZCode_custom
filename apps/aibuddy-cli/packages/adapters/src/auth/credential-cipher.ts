...
const CREDENTIAL_SECRET_ENV_KEY = "AIBUDDY_CREDENTIAL_SECRET";

export interface AIbuddyCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface AIbuddyCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createAIbuddyCredentialCipher(
  options: AIbuddyCredentialCipherOptions = {},
): AIbuddyCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedAIbuddyCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedAIbuddyCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `aibuddy-credential-fallback:${platform()}:${homedir()}:${username}`;
}
