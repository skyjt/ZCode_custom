...
import {
  AIBUDDY_CUA_OFFICIAL_MCP_NAMESPACE_NAME as AIBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME,
  AIBUDDY_CUA_OFFICIAL_PLUGIN_ID,
} from "@aibuddy/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@aibuddy/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(AIBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === AIBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(AIBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === AIBUDDY_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === AIBUDDY_CUA_OFFICIAL_PLUGIN_ID,
  );
