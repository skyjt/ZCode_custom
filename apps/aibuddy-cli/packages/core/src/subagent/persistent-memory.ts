...
import type { FileSystemPort, Logger, TraceContext } from "@aibuddy/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".aibuddy", "agent-memory", agentKey)
    : join(workspace, ".aibuddy", "agent-memory-local", agentKey);
}
