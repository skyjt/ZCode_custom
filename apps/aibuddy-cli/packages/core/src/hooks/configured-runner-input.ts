...
import { resolveWorkspaceHookTimeoutMs } from "@aibuddy/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "aibuddy-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 AIbuddy camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    AIBUDDY_PROJECT_DIR: input.cwd || workingDirectory,
    AIBUDDY_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      AIBUDDY_PLUGIN_DATA: plugin.dataPath,
      AIBUDDY_PLUGIN_ID: plugin.id,
      AIBUDDY_PLUGIN_NAME: plugin.name,
      AIBUDDY_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.AIBUDDY_PLUGIN_DATA = plugin.dataPath;
    replacements.AIBUDDY_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|AIBUDDY_PLUGIN_DATA|AIBUDDY_PLUGIN_ROOT|AIBUDDY_PROJECT_DIR|AIBUDDY_SESSION_ID|AIBUDDY_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "AIBUDDY_SKILL_DIR") {
