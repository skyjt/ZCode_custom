...
import type { ModelCatalogEntry, ModelSelection } from "@aibuddy/contracts";
import {
  AIBUDDY_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@aibuddy/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(AIBUDDY_MODEL_REASONING_SEPARATOR, searchFrom);
