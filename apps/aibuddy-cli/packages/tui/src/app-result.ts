import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@aibuddy/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getAIbuddyCopy(result.locale ?? input.fallback.locale).tui;
