...
const cliBundle = resolve(dist, "aibuddy.cjs");
const seaBlobForTarget = (target) => resolve(dist, `aibuddy-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "aibuddy-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/aibuddy.cjs",
        output: `dist/aibuddy-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "aibuddy-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        AIBUDDY_STORAGE_DIR: storageRoot,
      };
      delete env.AIBUDDY_BFS_BINARY;
      delete env.AIBUDDY_RG_BINARY;
      delete env.AIBUDDY_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/aibuddy.cjs. Run `pnpm build` first.");
  }
