...
export const seaTuiAssetPrefix = "aibuddy-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @aibuddy/contracts 依赖仓库根的 @aibuddy/shared；
    // 只扫描 apps/aibuddy-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@aibuddy/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @aibuddy/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@aibuddy/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@aibuddy/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @aibuddy 白名单漏掉 i18n
