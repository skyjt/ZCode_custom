import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@aibuddy/adapters/config";
import type { SessionEvent } from "@aibuddy/contracts";
import type { AIbuddyAppOptions } from "@aibuddy/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@aibuddy/i18n";
import type { TuiRequestPermission } from "@aibuddy/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createAIbuddyApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<AIbuddyAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createAIbuddyApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createAIbuddyApp"]>>>;
    try {
