import type { TuiSessionMetadata } from "@aibuddy/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listAIbuddySkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listAIbuddySkills;
  return await listAIbuddySkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listAIbuddySessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listAIbuddySessions;
  const sessions = await listAIbuddySessionsForTui({
