...
import type { listAIbuddyPlugins } from "@aibuddy/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@aibuddy/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@aibuddy/bootstrap");
export type PluginListOutcome = ReturnType<typeof listAIbuddyPlugins>;
 ...
 * 避免 `aibuddy plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addAIbuddyPluginMarketplace",
  getPluginsOverview: "getAIbuddyPluginsOverview",
  installPlugin: "installAIbuddyMarketplacePlugin",
  listPlugins: "listAIbuddyPlugins",
  removeMarketplace: "removeAIbuddyPluginMarketplace",
  setPluginEnabled: "setAIbuddyPluginEnabled",
  uninstallPlugin: "uninstallAIbuddyMarketplacePlugin",
  updateMarketplace: "updateAIbuddyPluginMarketplace",
  updatePlugin: "updateAIbuddyMarketplacePlugin",
  validatePluginPath: "validateAIbuddyPluginPath",
...
/** `aibuddy plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@aibuddy/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by aibuddy. Use: user, project");
  }
