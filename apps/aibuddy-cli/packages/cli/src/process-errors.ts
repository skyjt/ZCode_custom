...
import {
  AIBUDDY_PROCESS_DIAGNOSTIC_PREFIX,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, AIBUDDY_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: AIbuddyProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${AIBUDDY_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[aibuddy] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
