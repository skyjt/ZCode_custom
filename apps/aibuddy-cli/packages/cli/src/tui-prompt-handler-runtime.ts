...
// 品牌类型（SessionId）和 createAIbuddyApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createAIbuddyApp ?? bootstrapModule?.createAIbuddyApp;
  if (!createAppFactory) throw new Error("AIbuddy app factory is unavailable.");
