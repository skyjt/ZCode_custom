...
import type { PlaywrightChromiumModule } from "@aibuddy/adapters/browser";
...
const ASSET_PREFIX = "aibuddy-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "aibuddy-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "aibuddy", "sea-assets");
  }
  if (platform() === "win32") {
    return join(
