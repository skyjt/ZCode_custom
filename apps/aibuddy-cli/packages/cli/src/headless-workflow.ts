...
import { createDenyPermissionBroker } from "@aibuddy/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  AIbuddyAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `aibuddySessionEventTypeSchema`（`shared/src/aibuddy-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `aibuddySessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createAIbuddyApp` 注入点边界）。
 *
