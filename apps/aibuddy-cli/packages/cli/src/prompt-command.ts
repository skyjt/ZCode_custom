...
import { formatJson, type PresentationSurface } from "@aibuddy/core";
import type { RunContext, GlobalOptions } from "@aibuddy/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createAIbuddyApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createAIbuddyApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createAIbuddyApp ?? bootstrapModule?.createAIbuddyApp;
    if (!createApp) throw new Error("AIbuddy app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareAIbuddyTelemetryEnv ?? bootstrapModule?.prepareAIbuddyTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry =
        deps.shutdownAIbuddyTelemetry ?? bootstrapModule?.shutdownAIbuddyTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.AIBUDDY_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `aibuddy -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveAIbuddyCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedAIbuddySlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listAIbuddyCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadAIbuddyCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: aibuddy hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
