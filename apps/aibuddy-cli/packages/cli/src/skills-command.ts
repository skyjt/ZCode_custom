...
type BootstrapModule = typeof import("@aibuddy/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listAIbuddySkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: aibuddy skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listAIbuddySkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@aibuddy/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectAIbuddySkill;
...
function formatHumanSkillInspection(
  inspection: AIbuddySkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: AIbuddySkillInspection, cwd: string): string =>
  formatJson({
    cwd,
