...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@aibuddy/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.AIBUDDY_STORAGE_DIR?.trim() || join(homedir(), ".aibuddy");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "aibuddy-clipboard-"));
