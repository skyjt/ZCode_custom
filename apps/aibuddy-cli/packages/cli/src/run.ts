...
import { createNodeLoggerFactory } from "@aibuddy/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@aibuddy/core";
import { color, formatJson, supportsColor } from "@aibuddy/core";
import { getAIbuddyCopy, isUiLocale, type UiLocale } from "@aibuddy/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@aibuddy/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getAIbuddyCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "aibuddy_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("aibuddy doctor", colors)}\n`);
...
const runAIbuddyProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 AIbuddy agent transport closed。
    ...
    const runProtocolAgent =
      deps.runAIbuddyProtocolAgent ?? (await loadBootstrapModule()).runAIbuddyProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("aibuddy").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runAIbuddyProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
