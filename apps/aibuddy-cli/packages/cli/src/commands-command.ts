...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listAIbuddyCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: aibuddy commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listAIbuddyCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@aibuddy/bootstrap"));
  return (await bootstrap()).inspectAIbuddyCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: AIbuddyCustomCommandInspection,
  options: GlobalOptions,
): string {
