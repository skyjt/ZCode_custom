...
import {
  ...
  normalizeAIbuddyRuntimeEnv,
  sanitizeAIbuddyRuntimeEnv,
  sanitizeAIbuddyRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeAIbuddyRuntimeEnv(env),
    ...buildAIbuddyToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildAIbuddyToolEnvPassthroughEnv(env);
  sanitizeAIbuddyRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[AIBUDDY_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): AIbuddyRuntimeEnv {
  const explicit = normalizeAIbuddyRuntimeEnv(env[AIBUDDY_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.AIBUDDY_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.AIBUDDY_BETA === "1" || env.AIBUDDY_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])aibuddy-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.AIBUDDY_STORAGE_DIR = join(homedir(), ".aibuddy-beta");
}
