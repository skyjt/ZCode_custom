...
import type { SetAIbuddyPluginEnabledResult, AIbuddyPluginInstallData } from "@aibuddy/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: aibuddy plugins <command> [options]
...
Scopes: user (default), project. \`aibuddy plugin\` is an alias of \`aibuddy plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart aibuddy to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: AIbuddyPluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetAIbuddyPluginEnabledResult; error?: string }> = [];
