...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@aibuddy/shared/runtime-tool-runtime";
...
const assetPrefix = "aibuddy-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".aibuddy-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.AIBUDDY_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".aibuddy");
