...
interface AIbuddyBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createAIbuddyApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface AIbuddyContractsModule {
...
interface AIbuddyProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadAIbuddyModules();
    ...
    const app = await bootstrap.createAIbuddyApp({
...
async function loadAIbuddyModules(): Promise<{
  bootstrap: AIbuddyBootstrapModule;
  contracts: AIbuddyContractsModule;
  provider: AIbuddyProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<AIbuddyBootstrapModule>,
      import(contractsPath) as Promise<AIbuddyContractsModule>,
      import(providerConfigPath) as Promise<Omit<AIbuddyProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<AIbuddyProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built aibuddy-cli packages. Run `pnpm --filter @aibuddy/bootstrap^... build && pnpm --filter @aibuddy/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
