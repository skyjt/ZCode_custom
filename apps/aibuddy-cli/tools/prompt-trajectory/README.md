# @aibuddy/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting aibuddy-cli prompt assembly.
...
```bash
pnpm --filter @aibuddy/bootstrap^... build
pnpm --filter @aibuddy/bootstrap build

pnpm --filter @aibuddy/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/aibuddy-prompt-trajectory/basic-live

pnpm --filter @aibuddy/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @aibuddy/prompt-trajectory derive -- \
  --out /tmp/aibuddy-prompt-trajectory/basic-live

pnpm --filter @aibuddy/prompt-trajectory model-io -- \
  --input ~/.aibuddy/cli/debug/model-io-<session>.jsonl \
  --out /tmp/aibuddy-prompt-trajectory/model-io-session
```
...
uses the same aibuddy model config resolution as the CLI. The upstream request is
...
`model-io` reads a real AIbuddy `model-io-*.jsonl` file and turns the main
