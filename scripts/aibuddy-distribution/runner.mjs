...
const agentEntry = join(root, "agent", "aibuddy.cjs");

function usage() {
  return `Usage:
  aibuddy --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  aibuddy --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      AIBUDDY_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      AIBUDDY_AGENT_SERVER_COMMAND: process.execPath,
      AIBUDDY_SERVER_HOST: options.host,
      AIBUDDY_SERVER_WORKSPACE: options.workspace,
      AIBUDDY_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      AIBUDDY_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("AIbuddy Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: aibuddy --web [options] (aibuddy --web --help for details)\n");
    }
