...
const defaultEntryPath = join(repoRoot, "apps/aibuddy-cli/packages/cli/dist/aibuddy.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.AIBUDDY_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "aibuddy.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `aibuddy.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `aibuddy.bytecode-runtime-${runtimeHash}.cjs`;
