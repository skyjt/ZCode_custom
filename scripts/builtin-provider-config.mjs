...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.AIBUDDY_ENV !== undefined) value = parsed.AIBUDDY_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid AIBUDDY_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.AIBUDDY_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/aibuddy-builtin.json",
  );
  try {
    ...
    const { decodeAIbuddyBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/aibuddy-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeAIbuddyBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "aibuddy-builtin.json"), config.content, "utf8");
  return config;
}
