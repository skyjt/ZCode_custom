...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    AIBUDDY_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @aibuddy/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@aibuddy/rpc", "@aibuddy/web", "@aibuddy/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/aibuddy-cli"]);
