...
import {
  ...
  stageTuiRuntime,
} from "./aibuddy-distribution/assets.mjs";
import { installScriptSource } from "./aibuddy-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "aibuddy");
const defaultBaseUrl = (await loadEndpointEnv()).AIBUDDY_DIST_BASE_URL?.trim() || "";
const packageDirName = "aibuddy";
const usage = `Usage:
  pnpm build:aibuddy
  node scripts/build-aibuddy.mjs --skip-build
  node scripts/build-aibuddy.mjs --version 3.3.3-dev.1
  node scripts/build-aibuddy.mjs --out-dir dist/aibuddy
  node scripts/build-aibuddy.mjs --base-url http://host/aibuddy/deps/aibuddy/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/aibuddy.
...
function run(command, args, options = {}) {
  console.log(`[aibuddy] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[aibuddy] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@aibuddy/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@aibuddy/server", "build"]);
  run("pnpm", ["--filter", "@aibuddy/web", "build"]);
}

async function stageAIbuddyPackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "aibuddy-cli", "packages", "cli", "dist", "aibuddy.cjs");
  const agentProvider = resolve(root, "apps/aibuddy-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "aibuddy-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "aibuddy.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/aibuddy-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "aibuddy.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "aibuddy.mjs");
  await cp(resolve(root, "scripts/aibuddy-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure AIBUDDY_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve AIbuddy version.");
  }
  ...
  console.log(`[aibuddy] release directory: ${outDir}`);
  console.log(`[aibuddy] tarball: ${tarball}`);
  console.log(`[aibuddy] sha256: ${sha256}`);
}
