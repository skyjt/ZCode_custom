...
export function buildNativeSearchToolsWindows({
  ...
  const workDir = mkdtempSync(join(tmpdir(), "aibuddy-native-search-build-"));
  ...
  console.log("==> AIbuddy native search build");
  ...
  try {
    ...
    run(
      ...
      [
        ...
        `-DAIBUDDY_BUILD_PATH=${toCmakePath(workDir)}`,
        `-DAIBUDDY_UGREP_OUTPUT_DIR=${toCmakePath(cmakeOutputDir)}`,
        `-DAIBUDDY_UGREP_SOURCE_DIR=${toCmakePath(sources.ugrep)}`,
        `-DAIBUDDY_PCRE2_SOURCE_DIR=${toCmakePath(sources.pcre2)}`,
        `-DAIBUDDY_ZLIB_SOURCE_DIR=${toCmakePath(sources.zlib)}`,
        `-DAIBUDDY_BZIP2_SOURCE_DIR=${toCmakePath(sources.bzip2)}`,
        `-DAIBUDDY_ZSTD_SOURCE_DIR=${toCmakePath(sources.zstd)}`,
        `-DAIBUDDY_BROTLI_SOURCE_DIR=${toCmakePath(sources.brotli)}`,
      ],
      { env, quiet },
    );
    run(
