// composer parity：`#` 会话候选的数据源从旧 aibuddySessionStore/taskQueryCache/remote*
...
import type { AIbuddyProvider, AIbuddyTaskMeta } from "@aibuddy/shared";
...
function compareSessionTasks(
  left: AIbuddyTaskMeta,
  right: AIbuddyTaskMeta,
...
function getSessionLabel(task: AIbuddyTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: AIbuddyTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: AIbuddyTaskMeta, provider: AIbuddyProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, AIbuddyTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.aibuddyAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.aibuddyAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.aibuddyAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.aibuddyAgentService,
        },
      ];
    }
