 ...
 * Supports the unified AIbuddy Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToAIbuddyAgentMcpServer,
...
import { isAIbuddyAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@aibuddy/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "aibuddyagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 aibuddyAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isAIbuddyAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "aibuddyagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!aibuddyAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(aibuddyAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: AIbuddyAgentMcpServer[]): {
  mode: "status";
  mcpServers: AIbuddyAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: AIbuddyMcpServer[]): string {
  return servers
    .filter((server) => server.source === "aibuddyagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: AIbuddyPluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, AIbuddyAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<AIbuddyMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForAIbuddy("aibuddy");
  ...
  async function handleSave(form: FormState, prev?: AIbuddyMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: AIbuddyMcpServer) {
  ...
  function handleEdit(server: AIbuddyMcpServer) {
  ...
  function handleOpenAuthorization(server: AIbuddyMcpServer) {
