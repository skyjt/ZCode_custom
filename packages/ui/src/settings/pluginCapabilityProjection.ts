...
import { compareDocumentPluginPriority, isPluginCommand, isUserCommand } from "@aibuddy/shared";
...
export function selectBuiltInPlugins(
  ...
  installedPlugins: readonly AIbuddyInstalledPluginSummary[],
): AIbuddyPluginInfo[] {
...
interface PluginSettingsGroups {
  installed: AIbuddyPluginInfo[];
  builtIn: AIbuddyPluginInfo[];
}
...
export function selectPluginsForScope(
  plugins: readonly AIbuddyPluginInfo[],
  _installedPlugins: readonly AIbuddyInstalledPluginSummary[],
  _scope: AIbuddyPluginScope,
): AIbuddyPluginInfo[] {
...
export function selectSkillsForScope(
  ...
  scopedPlugins: readonly Pick<AIbuddyPluginInfo, "id" | "name" | "enabled">[],
...
export function selectCommandsForScope(
  commands: readonly AIbuddyCommand[],
  scopedPlugins: readonly Pick<AIbuddyPluginInfo, "id" | "name" | "enabled">[],
  scope: AIbuddyPluginScope,
): AIbuddyCommand[] {
  const enabledPlugins = scopedPlugins.filter((plugin) => plugin.enabled);
