...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.aibuddyAgentService;
  ...
  const launch = useCallback(
    async (entry: AIbuddySavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: AIbuddySavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );
