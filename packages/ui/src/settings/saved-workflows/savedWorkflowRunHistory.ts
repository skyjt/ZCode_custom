...
import type { AIbuddySavedWorkflowRun, AIbuddySavedWorkflowRunStatus } from "@aibuddy/shared";
...
): Map<string, AIbuddySavedWorkflowRun> {
  const byName = new Map<string, AIbuddySavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: AIbuddySavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: AIbuddySavedWorkflowRun, now: number): number {
