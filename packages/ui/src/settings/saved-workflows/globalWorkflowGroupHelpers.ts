...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type AIbuddySavedWorkflowRun } from "@aibuddy/shared";
import type { AIbuddyAgentSavedWorkflowTarget } from "@aibuddy/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: AIbuddyAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: AIbuddySavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
