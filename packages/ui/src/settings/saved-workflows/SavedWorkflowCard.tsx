...
function SavedWorkflowLastRunBadge({
  ...
  run: AIbuddySavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: AIbuddySavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: AIbuddySavedWorkflowEntry) => void;
  onRun: (entry: AIbuddySavedWorkflowEntry) => void;
  onRevise: (entry: AIbuddySavedWorkflowEntry) => void;
  onCopyPath: (entry: AIbuddySavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: AIbuddySavedWorkflowEntry) => void;
  onDelete: (entry: AIbuddySavedWorkflowEntry) => void;
}
