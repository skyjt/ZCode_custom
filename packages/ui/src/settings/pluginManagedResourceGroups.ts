import type {
  ...
  AIbuddyPluginInfo,
  AIbuddyPluginComponentKind,
  AIbuddyPluginsDescribeResult,
} from "@aibuddy/shared";
import { isPluginCommand, isUserCommand, AIBUDDY_COMMAND_AGENT_SOURCE } from "@aibuddy/shared";
...
const COMPONENT_KIND_ORDER: AIbuddyPluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: AIbuddyPluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<AIbuddyPluginComponentKind, PluginComponentDisplayGroup>();
...
export interface PluginMcpServerItem {
  active: boolean;
  authorization?: AIbuddyMcpServerStatusSnapshot["authorization"];
  error?: string;
  failureKind?: AIbuddyMcpServerStatusSnapshot["failureKind"];
  id: string;
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === AIBUDDY_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: AIbuddyMcpServer[], query: string): AIbuddyMcpServer[] {
  return servers.filter((server) => {
    if (server.source !== "aibuddyagentmcp") {
      return false;
    }
...
function toPluginMcpServerDisplayName(plugin: AIbuddyPluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: AIbuddyPluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: AIbuddyMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
