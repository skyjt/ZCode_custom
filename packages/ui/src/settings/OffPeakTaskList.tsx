...
interface OffPeakTaskListProps {
  ...
  onOpen: (task: AIbuddyOffPeakTask) => void;
  onPause: (task: AIbuddyOffPeakTask) => void;
  onContinue: (task: AIbuddyOffPeakTask) => void;
  onCancel: (task: AIbuddyOffPeakTask) => void;
  onDelete: (task: AIbuddyOffPeakTask) => void;
  onOpenSession: (task: AIbuddyOffPeakTask) => void;
}

// 按状态分组会让任务在运行和终态切换时跳位，破坏用户对已有卡片位置的预期；
// 列表只按不可变的创建时间倒序，状态变化不再影响顺序。
function sortOffPeakTasksByCreatedAt(tasks: readonly AIbuddyOffPeakTask[]): AIbuddyOffPeakTask[] {
