import type { AIbuddyCommand, AIbuddySlashCommand } from "@aibuddy/shared";
...
function commandToSlashCommand(command: AIbuddyCommand): AIbuddySlashCommand | null {
  if (!command.enabled) {
    return null;
  }
...
export function mergeSlashCommandsAfterCommandRefresh(
  currentSlashCommands: readonly AIbuddySlashCommand[],
  refreshedCommands: readonly AIbuddyCommand[],
): AIbuddySlashCommand[] {
  ...
  const customCommands: AIbuddySlashCommand[] = [];
