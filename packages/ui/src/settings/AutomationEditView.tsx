...
import {
  ...
  type AIbuddyAutomationScheduleRule,
} from "@aibuddy/shared";
import {
...
interface AutomationEditViewProps {
  /** null = 新建；否则编辑。 */
  editing: AIbuddyAutomation | null;
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: AIbuddyAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: AIbuddyAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: AIbuddyAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: AIBUDDY_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<AIbuddyAutomationScheduleRule | undefined>(
    () =>
