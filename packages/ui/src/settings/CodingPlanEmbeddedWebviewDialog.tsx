...
import { RENDERER_AIBUDDY_ENDPOINT_URLS } from "@/lib/rendererAIbuddyEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 发送 aibuddy:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const theme = useAIbuddyStoreWithDefault((state) => state.theme, "zai-dark");
  const userId = useAIbuddyStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const computedWebviewUrl = useMemo(() => {
    ...
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_AIBUDDY_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_AIBUDDY_E2E_STORE_BRIDGE === "1",
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                ...
                aibuddyJwtToken: values[1],
              }
            : {
                bigmodelAccessToken: values[0],
                // BigModel OAuth callback 同样会落盘 aibuddy JWT；官网用它在
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // 发回购买完成信号（aibuddy:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:aibuddy-coding-plan"
            src={webviewUrl}
            className={cn(
