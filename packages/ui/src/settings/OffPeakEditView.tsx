...
interface OffPeakEditViewProps {
  editing: AIbuddyOffPeakTask | null;
  ...
  onDeleteHistory?: (task: AIbuddyOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<AIbuddyConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: AIbuddyOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({
