...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<AIbuddyAvailablePluginSummary, "name" | "listing">[],
  name: string,
): AIbuddyPluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: AIbuddyPluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: AIbuddyPluginStoreListing;
  summary?: AIbuddyAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: AIbuddyPluginInfo;
  installedMeta?: AIbuddyInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<AIbuddyInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly AIbuddyPluginMarketplaceSummary[],
  locale: string,
): AIbuddyPluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  marketplaces: AIbuddyPluginMarketplaceSummary[];
  marketplaceAvailabilityKnown: boolean;
  availablePlugins: AIbuddyAvailablePluginSummary[];
  installedPlugins: AIbuddyInstalledPluginSummary[];
  plugins: AIbuddyPluginInfo[];
  restorableBuiltins: AIbuddyAvailablePluginSummary[];
}): StorePluginItem[] {
