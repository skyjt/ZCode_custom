...
export function SettingsPage({
  ...
  const { intl, localePreference, setLocalePreference } = useAIbuddyIntl();
  ...
  >([]);
  const interfaceMode = useAIbuddyStore((state) => state.interfaceMode);
  const setInterfaceMode = useAIbuddyStore((state) => state.setInterfaceMode);
  const theme = useAIbuddyStore((state) => state.theme);
  const setTheme = useAIbuddyStore((state) => state.setTheme);
  const codePreviewSettings = useAIbuddyStore((state) => state.codePreviewSettings);
  const setCodePreviewSettings = useAIbuddyStore((state) => state.setCodePreviewSettings);
  const uiFontSizePx = useAIbuddyStore((state) => state.uiFontSizePx);
  const setUiFontSizePx = useAIbuddyStore((state) => state.setUiFontSizePx);
  const notificationEnabled = useAIbuddyStore((state) => state.notificationEnabled);
  const setNotificationEnabled = useAIbuddyStore((state) => state.setNotificationEnabled);
  const notificationSoundEnabled = useAIbuddyStore((state) => state.notificationSoundEnabled);
  const setNotificationSoundEnabled = useAIbuddyStore((state) => state.setNotificationSoundEnabled);
  ...
  const requestOnboardingDialog = useAIbuddyStore((state) => state.requestOnboardingDialog);
  ...
  const [aibuddyInteractionBehavior, setAIbuddyInteractionBehavior] =
    useState<AIbuddyInteractionBehavior>("queue");
  ...
  useEffect(() => {
    services.settingService
      ...
      .then((settings: AppSettings) => {
        ...
        setAIbuddyInteractionBehavior(settings.aibuddyInteractionBehavior ?? "queue");
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    setAIbuddyInteractionBehavior(sharedSettings.aibuddyInteractionBehavior ?? "queue");
  ...
  const handleAIbuddyInteractionBehaviorChange = useCallback(
    async (behavior: AIbuddyInteractionBehavior) => {
      await runSettingsActionAsync({
        featureId: "settings.conversation",
        action: "change_interaction_behavior",
        trigger: "select",
        operation: () => updateSharedSettings({ aibuddyInteractionBehavior: behavior }),
        completed: { resultSource: "shared_settings", valueAfter: behavior },
      });
      setAIbuddyInteractionBehavior(behavior);
    },
    [updateSharedSettings],
  );
                      ...
                      <div className="space-y-8">
                        {activeSection === "general" ? (
                          <GeneralSectionContent
                            ...
                            aibuddyInteractionBehavior={aibuddyInteractionBehavior}
                            ...
                            onAIbuddyInteractionBehaviorChange={
                              handleAIbuddyInteractionBehaviorChange
                            }
