...
import type { AIbuddySessionEndedSubagent, AIbuddySessionSubagentsResult } from "@aibuddy/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: AIbuddySessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly AIbuddySessionEndedSubagent[],
  incoming: readonly AIbuddySessionEndedSubagent[],
): AIbuddySessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const aibuddyAgentService = services.aibuddyAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<AIbuddySessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof aibuddyAgentService?.listSessionSubagents !== "function") {
      ...
      return aibuddyAgentService.listSessionSubagents({
