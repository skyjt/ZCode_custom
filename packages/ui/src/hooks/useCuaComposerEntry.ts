...
import { isRemoteWorkspaceIdentity, AIBUDDY_CUA_OFFICIAL_PLUGIN_ID } from "@aibuddy/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/aibuddySessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 aibuddySessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 aibuddy-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useAIbuddySessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === AIBUDDY_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === AIBUDDY_CUA_OFFICIAL_PLUGIN_ID,
