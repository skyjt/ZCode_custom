...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly AIbuddyTaskMeta[];
  optimisticTasks: readonly AIbuddyTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareAIbuddyTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useAIbuddySessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useAIbuddySessionStore.getState();
