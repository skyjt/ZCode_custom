...
import { createUuid } from "@aibuddy/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@aibuddy/shared";
...
export function useAppPanels(options: {
  ...
  const { aibuddyAgentService, aibuddySessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await aibuddyAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
