...
import { selectWorkspaceAIbuddyState, useAIbuddySessionStore } from "@/store/aibuddySessionStore.js";
import { buildTaskEntityKey, buildTaskWorkspaceKey } from "@/lib/taskQueryCache.js";
import { mergeTaskWithOptimisticMeta } from "@/lib/aibuddyTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: AIbuddyTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, AIbuddyTaskMeta>();
  const placementTaskByKey = new Map<string, AIbuddyTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, AIbuddyTaskMeta[]>();
  const topMissingTasks: AIbuddyTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: AIbuddyGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: AIbuddyGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: AIbuddyGroupedTaskView["nodes"][number],
  next: AIbuddyGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: AIbuddyGroupedTaskView,
  next: AIbuddyGroupedTaskView,
): AIbuddyGroupedTaskView {
...
function nodeToTopLevelRef(
  node: AIbuddyGroupedTaskView["nodes"][number],
): AIbuddyGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
function viewToOrderInput(params: { view: AIbuddyGroupedTaskView }): AIbuddyGroupedTaskViewOrderInput {
  return {
...
const groupedViewCacheBySignature = new Map<string, AIbuddyGroupedTaskView>();

function readCachedGroupedView(signature: string): AIbuddyGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: AIbuddyGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<AIbuddyGroupedTaskView>(
  ...
  const clearPromotedGroupedDraftTask = useAIbuddySessionStore(
  ...
  const displayedViewRef = useRef<AIbuddyGroupedTaskView>({ nodes: [] });
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.aibuddyTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.aibuddyTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: AIbuddyTaskMeta[] = [];
    const settledRootTasks: AIbuddyTaskMeta[] = [];
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.aibuddyTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.aibuddyTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<AIbuddyTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.aibuddyTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.aibuddyTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: AIbuddyGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.aibuddyTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.aibuddyTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: AIbuddyTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.aibuddyTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: AIbuddyGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.aibuddyTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.aibuddyTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: AIbuddyGroupedTaskView = {
      ...
      try {
        ...
        await services.aibuddyTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.aibuddyTaskService, view],
  );
