...
function buildWorkspaceScopes(workspaceTabs: WorkspaceTabState[]): AIbuddyTaskListWorkspaceScope[] {
  const scopes = new Map<string, AIbuddyTaskListWorkspaceScope>();
...
export function useGlobalTaskList(params: {
  ...
  const taskListVersionSignature = useAIbuddySessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs
        .map((tab) => {
          const workspace = selectWorkspaceAIbuddyState(
            state,
            tab.workspacePath,
