...
function areResolvedTaskMetasEqual(left: AIbuddyTaskMeta | null, right: AIbuddyTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: AIbuddyTaskMeta | null) {
  const stableTaskMetaRef = useRef<AIbuddyTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useAIbuddySessionStore((state) =>
    selectWorkspaceAIbuddyState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 aibuddyTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
