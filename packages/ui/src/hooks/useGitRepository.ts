...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, AIbuddyPersistedFileSnapshot>,
  file: AIbuddyTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: AIbuddyPersistedFileChange | null;
  summary: AIbuddyTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, AIbuddyPersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 AIbuddy Agent 已持久化的单轮文件快照，
