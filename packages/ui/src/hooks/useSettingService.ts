...
import { APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL, type AppSettings } from "@aibuddy/shared";
...
export function useSettings() {
  const { broadcastService, settingService, aibuddyAgentService } = useServices();
  ...
  const update = useCallback(
    async (patch: Partial<AppSettings>) => {
      ...
      if (
        ...
        const syncResults = await Promise.allSettled([
          aibuddyAgentService.syncAppRuntimePreferences(preferences),
        ]);
    ...
    [broadcastService, settingService, settingsStore, aibuddyAgentService, platform, refresh],
  );
