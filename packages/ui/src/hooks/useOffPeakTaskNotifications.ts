...
import type { AIbuddyOffPeakTaskStatus } from "@aibuddy/shared";
import type { IOffPeakTaskService } from "@aibuddy/services";
...
function notifiableStatus(status: AIbuddyOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, AIbuddyOffPeakTaskStatus>());
