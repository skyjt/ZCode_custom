...
import type { AIbuddyTaskSnapshot } from "@aibuddy/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IAIbuddyTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<AIbuddyTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IAIbuddyTaskService["getTaskSnapshotWithEtag"]>[0];

const aibuddyTaskServiceProxyCache = new WeakMap<IAIbuddyTaskService, IAIbuddyTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IAIbuddyTaskService,
  Map<string, { etag: string; snapshot: AIbuddyTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "aibuddy-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: AIbuddyTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: AIbuddyTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: AIbuddyTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createAIbuddyTaskServiceProxy(service: IAIbuddyTaskService): IAIbuddyTaskService {
...
/** 获取 AIbuddy task wrapper 服务实例 */
export function useAIbuddyTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IAIbuddyTaskService {
  // AIbuddy task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.aibuddyTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = aibuddyTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createAIbuddyTaskServiceProxy(rawService);
  aibuddyTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
