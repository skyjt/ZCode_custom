...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@aibuddy/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<AIbuddyTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<AIbuddyTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<AIbuddyTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<AIbuddyTaskMeta | null>;
  ...
  const startDraft = useAIbuddySessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: AIbuddyTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareAIbuddyTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
