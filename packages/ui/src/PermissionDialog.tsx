...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@aibuddy/shared/aibuddy-protocol-v4";
...
function readPermissionRuleScopes(option: AIbuddyPermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: AIbuddyPermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: AIbuddyPermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<AIbuddyProvider, Record<string, string>>
> = {
  glm: {
    // GLM/AIbuddy Agent 通过 AIbuddy Agent 发来的项目级记忆授权文案是英文原文。
...
function getProviderOptionNameMessageIds(
  provider: AIbuddyProvider | undefined,
  name: string,
...
function getPermissionDisplayReason(request: AIbuddyPermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: AIbuddyPermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: AIbuddyPermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  const respondWithOption = useCallback(
    (option: AIbuddyPermissionOption) => {
  ...
  // 当前 AIbuddy Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // AIbuddy Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
