...
type AgentIntl = ReturnType<typeof useAIbuddyIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const aibuddy = meta && isPlainRecord(meta.aibuddy) ? meta.aibuddy : null;
  const aibuddyBackgroundAgent =
    aibuddy && isPlainRecord(aibuddy.backgroundAgent) ? aibuddy.backgroundAgent : null;
  const taskNotification =
    aibuddy && isPlainRecord(aibuddy.taskNotification) ? aibuddy.taskNotification : null;
  const backgroundAgent = aibuddyBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "aibuddy", "taskNotification", "summary"]);
