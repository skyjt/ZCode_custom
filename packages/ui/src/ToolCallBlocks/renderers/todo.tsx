...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@aibuddy/shared";
import type { AIbuddyPlanStep } from "@aibuddy/shared";
...
const todoTextClasses: Record<AIbuddyPlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): AIbuddyPlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: AIbuddyPlanStep["status"] }) {
  if (status === "completed") {
