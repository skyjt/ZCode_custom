...
import type { OnboardingRecordEntry } from "@aibuddy/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useAIbuddyStore((state) => state.newUserOnboardingOpen);
  const setRequested = useAIbuddyStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useAIbuddyStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useAIbuddyStore((state) => state.interfaceMode);
