...
import type { AIbuddyGroupedTaskViewNode } from "@aibuddy/services";
...
function getTopLevelNodeKey(node: AIbuddyGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: AIbuddyGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: AIbuddyGroupedTaskViewNode, index: number) => ReactNode;
}) {
