...
import type { AIbuddyGroupedTaskView, AIbuddyGroupedTaskViewNode } from "@aibuddy/services";
import type { AIbuddyTaskMeta } from "@aibuddy/shared";
import type { GroupedDraftTaskPlacement } from "@/store/aibuddySessionStoreTypes.js";
import { taskKey } from "@/workspace-grouped-tasks/ids.js";

function cloneView(view: AIbuddyGroupedTaskView): AIbuddyGroupedTaskView {
  return {
...
function cloneNodes(view: AIbuddyGroupedTaskView): AIbuddyGroupedTaskViewNode[] {
  return [...view.nodes];
}

function cloneGroupNode(
  node: Extract<AIbuddyGroupedTaskViewNode, { type: "group" }>,
): Extract<AIbuddyGroupedTaskViewNode, { type: "group" }> {
...
function filterGroupedViewByTaskKeys(
  ...
  let changed = false;
  const nodes: AIbuddyGroupedTaskView["nodes"] = [];
...
function getGroupedTaskGroupIds(view: AIbuddyGroupedTaskView): string[] {
...
function getTaskGroupIdInView(view: AIbuddyGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function findTopLevelTaskIndex(view: AIbuddyGroupedTaskView, targetTaskKey: string): number {
  return view.nodes.findIndex(
    (node) => node.type === "task" && taskKey(node.task) === targetTaskKey,
  );
}

function findGroupIndex(view: AIbuddyGroupedTaskView, groupId: string): number {
  return view.nodes.findIndex((node) => node.type === "group" && node.group.id === groupId);
}

function findGroupIdByTaskKey(view: AIbuddyGroupedTaskView, targetTaskKey: string): string | null {
  for (const node of view.nodes) {
    if (node.type !== "group") {
      continue;
    }
...
function getNodeIdAfterGroup(
  view: AIbuddyGroupedTaskView,
  groupId: string,
): AIbuddyGroupedTaskViewNode | null {
...
function insertTaskBeforeTopLevelNode(
  view: AIbuddyGroupedTaskView,
  task: AIbuddyTaskMeta,
  beforeNode: AIbuddyGroupedTaskViewNode | null,
): AIbuddyGroupedTaskView {
  const nodes = cloneNodes(view);
  const insertIndex =
    beforeNode === null
