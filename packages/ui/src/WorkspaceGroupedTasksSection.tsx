...
import type { AIbuddyGroupedTaskView, AIbuddyTaskGroupColor } from "@aibuddy/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type AIbuddyTaskMeta } from "@aibuddy/shared";
...
function getGroupedTaskViewSignature(view: AIbuddyGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const removeTaskState = useAIbuddySessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useAIbuddySessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useAIbuddySessionStore((state) => state.setTaskUnreadIndicator);
  const groupedDraftTask = useAIbuddySessionStore(
    (state) =>
      selectWorkspaceAIbuddyState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useAIbuddySessionStore(
    (state) =>
      selectWorkspaceAIbuddyState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useAIbuddySessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<AIbuddyGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<AIbuddyGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: AIbuddyTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: AIbuddyTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.aibuddyTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: AIbuddyTaskMeta) => {
      ...
      void workspaceServices.services.aibuddyTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: AIbuddyTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: AIbuddyGroupedTaskView) => {
