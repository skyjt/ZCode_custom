...
export function useRootPlatformEffects({
  ...
  initialWorkspacePurpose?: import("@aibuddy/shared").WorkspacePurpose;
  ...
  isDesktop?: boolean;
  locale: ReturnType<typeof import("@/i18n/IntlProvider.js").useAIbuddyIntl>["locale"];
  tabs: WindowTabState[];
  ...
  useEffect(() => {
    ...
    const disposeNotificationClick = platform.onTaskNotificationClick((taskId: string) => {
      ...
      const workspaces = useAIbuddySessionStore.getState().workspaces;
  ...
  useEffect(() => {
    ...
    void baseServices.conversationShareService
      ...
      .then((result) => {
        ...
        const sessionStore = useAIbuddySessionStore.getState();
  ...
  useEffect(() => {
    ...
    syncActiveSession();
    return useAIbuddySessionStore.subscribe(syncActiveSession);
