...
import { getDefaultWorkspaceState } from "@/store/aibuddySessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly AIbuddyTaskMeta[]): AIbuddyTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: AIbuddyPermissionRequest["options"],
  right: AIbuddyPermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: AIbuddyPermissionRequest,
  right: AIbuddyPermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useAIbuddyChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToAIbuddyAgent(params.provider),
