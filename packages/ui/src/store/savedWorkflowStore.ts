...
import {
  ...
  type AIbuddySavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: AIbuddySavedWorkflowEntry[];
  invalid: AIbuddySavedWorkflowInvalidEntry[];
  runs: AIbuddySavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: AIbuddyAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: AIbuddyAgentSavedWorkflowTarget,
  agentService: IAIbuddyAgentService,
): Promise<{ list: AIbuddyWorkflowsListResult; runs: AIbuddySavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as AIbuddySavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: AIbuddyAgentSavedWorkflowTarget | null,
