...
} from "@/store/aibuddySessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/aibuddySessionStoreSelectors.js";

type SetFn = (
  partial:
    | AIbuddySessionStoreState
    | Partial<AIbuddySessionStoreState>
    | ((state: AIbuddySessionStoreState) => AIbuddySessionStoreState | Partial<AIbuddySessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: AIbuddyConfigOption): AIbuddyConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: AIbuddyConfigOption[]): AIbuddyConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly AIbuddyConfigOption[]): AIbuddyConfigOption[] {
...
function isModeConfigOption(option: AIbuddyConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<AIbuddyConfigOption["options"]>;
}): AIbuddyConfigOption {
...
function replaceModeConfigOption(
  currentOptions: AIbuddyConfigOption[] | null,
  nextModeOption: AIbuddyConfigOption | null,
): AIbuddyConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceAIbuddyUIState,
  updater: (options: AIbuddyConfigOption[] | null) => AIbuddyConfigOption[] | null,
): Pick<WorkspaceAIbuddyUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceAIbuddyUIState,
  taskId: string,
): { configOptions: AIbuddyConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: AIbuddyTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as AIbuddyTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToAIbuddyAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToAIbuddyAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToAIbuddyAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
