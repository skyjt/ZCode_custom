...
import type { Hook, HookConfig } from "@aibuddy/shared";
import type { IHooksService } from "@aibuddy/services";
...
function buildAIbuddyHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildAIbuddyHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "aibuddy") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildAIbuddyHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
