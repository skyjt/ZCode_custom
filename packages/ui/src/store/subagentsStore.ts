...
import type { ISubagentsService } from "@aibuddy/services";
...
interface SubagentsStoreState {
  ...
  loadedProvider: AIbuddyProvider | null;
  agents: AgentSummary[];
  ...
  initialize: (
    ...
    providerOrSubagentsService: AIbuddyProvider | ISubagentsService,
...
export const useSubagentsStore = create<SubagentsStoreState>((set, get) => ({
  ...
  async initialize(
    ...
    const provider = normalizeAgentProviderToAIbuddyAgent(
      hasProvider ? providerOrSubagentsService : AIBUDDY_AGENT_PROVIDER,
    );
  ...
  async refresh(subagentsService: ISubagentsService, workspaceIdentity?: string) {
    ...
    const provider = normalizeAgentProviderToAIbuddyAgent(get().provider);
  ...
  async createAgent(
    ...
    try {
      const result = await subagentsService.createAgent({
        config,
        provider: normalizeAgentProviderToAIbuddyAgent(provider),
      });
      await get().refresh(subagentsService, workspaceIdentityFromState);
      return result.agent;
    } catch (error) {
...
export async function refreshLoadedSubagentsStoreForWorkspace(params: {
  ...
  const contextKey = getSubagentsContextKey(workspacePath, AIBUDDY_AGENT_PROVIDER, workspaceIdentity);
