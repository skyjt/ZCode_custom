import type { McpServerConfig, NativeMcpServerRecord } from "@aibuddy/shared";
...
function isAIbuddyAgentUserServer(server: NativeMcpServerRecord): boolean {
  return (
    server.source === "aibuddyagentmcp" &&
    server.scope === "user" &&
    (!server.location || server.location.source === "aibuddy")
  );
}

export async function importLegacyCommonServersToAIbuddyAgent(
  ...
  const existingNames = new Set(
    nativeServers.filter(isAIbuddyAgentUserServer).map((server) => server.name),
  );
  let importedCount = 0;
  let skippedCount = 0;
...
export async function migrateStoredCommonMcpToAIbuddyAgent(
  platform: McpPlatformService | null,
  nativeServers: NativeMcpServerRecord[],
  ...
  const migration = await importLegacyCommonServersToAIbuddyAgent(
    ...
    legacyServers,
    nativeServers,
    "localStorage:aibuddy-mcp-config",
  );
  if (migration.completed) {
    // 只有确认写入 aibuddy agent 目录后才清理旧数据，避免 Web 端没有 desktop bridge 时丢配置。
    clearLegacyCommonMcpServers();
  }
  ...
  logger.info(
    `[mcpStore] migrated ${migration.importedCount} legacy common MCP servers to aibuddy agent config`,
  );
