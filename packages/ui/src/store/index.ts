...
import type { IBroadcastService, BroadcastMessage } from "@aibuddy/services";
import type { OAuthProviderId, UserInfo } from "@aibuddy/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "aibuddy-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "aibuddy-performance-mode";
 ...
 */
export function createAIbuddyStore(
  ...
  const useStore = create<AIbuddyState>()((set, get) => ({
    ...
    theme: normalizeThemePreference((readSafeLocalStorage("aibuddy-theme") as Theme) || "zai-dark"),
    setTheme: (theme: Theme) => {
      const normalizedTheme = normalizeThemePreference(theme);
      writeSafeLocalStorage("aibuddy-theme", normalizedTheme);
    ...
    locale: readSafeLocalStorage("aibuddy-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("aibuddy-locale", locale);
      set({ locale });
    },
...
export type AIbuddyStore = ReturnType<typeof createAIbuddyStore>;
