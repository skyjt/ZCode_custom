...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToAIbuddyAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToAIbuddyAgent(params.provider);
