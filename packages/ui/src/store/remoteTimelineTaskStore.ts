...
import type { AIbuddyTaskListSortBy, IAIbuddyTaskService } from "@aibuddy/services";
...
interface RemoteTimelineTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    aibuddyTaskService: Pick<IAIbuddyTaskService, "listTaskList">;
    sortBy: AIbuddyTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: AIbuddyTaskMeta, sortBy?: AIbuddyTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: AIbuddyTaskListSortBy = "updated",
): AIbuddyTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, aibuddyTaskService, sortBy, limit }) {
    ...
    try {
      const result = await aibuddyTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
