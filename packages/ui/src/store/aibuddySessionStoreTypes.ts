/* oxlint-disable eslint(max-lines) -- AIbuddy Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * AIbuddy Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 aibuddySessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
...
export interface WorkspaceInitState {
  status: AIbuddyWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: AIbuddyTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 AIbuddy Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: AIbuddyBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: AIbuddyPermissionRequest | null;
  pendingPermissionRequests: AIbuddyPermissionRequest[];
  elicitationRequest: AIbuddyElicitationRequest | null;
  pendingElicitationRequests: AIbuddyElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: AIbuddyUiError | null;
}
...
export interface WorkspaceAIbuddyUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * AIbuddy Agent 的 workspace 初始化状态。
   *
   * 单 AIbuddy Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  draftError: AIbuddyUiError | null;
  ...
  taskConfigOptionsByTaskId: Record<string, AIbuddyConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, AIbuddyTaskMeta>;
  ...
  /** 当前 workspace 选中的 AIbuddy Agent provider */
  ...
  /** AIbuddy Agent configOptions（模型、模式、思考级别等） */
  configOptions: AIbuddyConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: AIbuddyTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface AIbuddySessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceAIbuddyUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceAIbuddyUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: AIbuddyPermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: AIbuddyElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 aibuddyTaskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: AIbuddyProvider = AIBUDDY_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceAIbuddyUIState {
  // 单 AIbuddy Agent 迁移后默认 provider 必须收敛到 glm。
