/**
 * AIbuddy Session Store 选择器与内部辅助函数
 *
 * 从 aibuddySessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { AIbuddyTaskRuntimeStatus, AIbuddyTaskMeta } from "@aibuddy/shared";
...
import {
  ...
  type AIbuddySessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceAIbuddyUIState,
} from "./aibuddySessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceAIbuddyUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceAIbuddyUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: AIbuddySessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceAIbuddyUIState) => WorkspaceAIbuddyUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 AIbuddy Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceAIbuddyUIState,
  taskId: string,
): TaskRuntimeState {
...
interface WorkspaceDisplayedTaskState {
  taskStatus: AIbuddyTaskRuntimeStatus;
...
export function getTaskUiState(workspaceState: WorkspaceAIbuddyUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceAIbuddyUIState
    | Partial<Pick<WorkspaceAIbuddyUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, AIbuddyTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<AIbuddyTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceAIbuddyState(
  state: AIbuddySessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceAIbuddyUIState, "workspaceInit">,
): WorkspaceInitState {
