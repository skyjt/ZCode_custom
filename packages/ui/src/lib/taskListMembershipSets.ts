...
> &
  Partial<Pick<IAIbuddyTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<AIbuddyTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: AIbuddyTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: AIbuddyTaskMeta[][]): AIbuddyTaskMeta[] {
  const itemByEntityKey = new Map<string, AIbuddyTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, AIbuddyTaskMeta>();
