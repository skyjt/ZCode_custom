...
function supportsRegistryApiFormat(
  ...
  // 仅剩 glm（AIbuddy Agent）provider；三方 CLI 的 api format 差异已随 provider 下线。
  return isAIbuddyAgentProvider(selectedProvider);
}
...
export function buildRegistryModelSelectGroups(
  ...
  return view.providers.flatMap((provider) => {
    ...
    const accountAccess = aibuddyProviderAccountAccessSchema.safeParse(provider.config.access);
