import type { AIbuddyConfigOption, AIbuddyTaskMeta } from "@aibuddy/shared";
import { getAIbuddyAgentModeSelectOptions } from "@aibuddy/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<AIbuddyTaskMeta, "provider" | "model">,
): string | null {
...
function ensureModelOptionValue(option: AIbuddyConfigOption, modelValue: string): AIbuddyConfigOption {
...
function createRecoveredModelOption(modelValue: string): AIbuddyConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): AIbuddyConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): AIbuddyConfigOption {
  return {
...
function mergeRecoveredTaskModelConfigOptions({
  ...
  taskMeta: Pick<AIbuddyTaskMeta, "provider" | "model">;
  configOptions: readonly AIbuddyConfigOption[];
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<AIbuddyTaskMeta, "provider" | "model"> &
    Partial<Pick<AIbuddyTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly AIbuddyConfigOption[];
}): AIbuddyConfigOption[] {
