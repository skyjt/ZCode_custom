...
import type {
  ...
  AIbuddyPersistedFileChange,
  AIbuddyProvider,
  AIbuddyStreamEvent,
  AIbuddyTimelineStatus,
  AIbuddyUsage,
} from "@aibuddy/shared";
import {
...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly AIbuddyPersistedFileChange[];
  usage?: AIbuddyUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 AIbuddy Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 AIbuddy Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly AIbuddyTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: AIbuddyTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: AIbuddyContextCompactionTimelineMeta;
  provider?: AIbuddyProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // AIbuddy Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: AIbuddyUsage | null, right: AIbuddyUsage): AIbuddyUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly AIbuddyPersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<AIbuddyStreamEvent, { type: "task_network_debug_status" }>,
...
export function recordAgentStepTelemetryEvent(input: {
  ...
  switch (input.event.type) {
    ...
    case "agent_message_chunk":
      if (input.event.aibuddyTimeline) {
        return finalized;
      }
