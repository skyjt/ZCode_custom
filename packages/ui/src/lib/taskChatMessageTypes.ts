// re-home 产物（为删除 aibuddyChatMessages 铺路，模式同 aibuddy-task-types-core）。
// 本文件承载旧消息转换层（aibuddyChatMessages/aibuddyChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（aibuddyChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  AIbuddyAssistantMessagePart,
  AIbuddyAssistantMessageFeedback,
  AIbuddyPromptAttachment,
  AIbuddyTimelineMeta,
  AIbuddyTaskSnapshotBodyRef,
  AIbuddyTaskSnapshotToolFieldRef,
  AIbuddyTaskSnapshotToolSlice,
} from "@aibuddy/shared";
...
export interface TaskChatToolCall {
  ...
  /** AIbuddy 固定工具名；kind 仍保留给旧 AIbuddy Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = AIbuddyAssistantMessagePart;
