import type { FileEntry, AIbuddyTaskMeta, AIbuddyTaskRuntimeStatus } from "@aibuddy/shared";
import type { IFileService } from "@aibuddy/services";
import type { WorkspaceAIbuddyUIState } from "@/store/aibuddySessionStoreTypes.js";
...
export function hasRunningWorkspaceChat(params: {
  workspaceState: Pick<WorkspaceAIbuddyUIState, "draftRuntime" | "taskRuntimeByTaskId">;
  taskItems: Pick<AIbuddyTaskMeta, "taskId">[];
}): boolean {
...
function isWorkspaceRemovalBlockingRuntimeStatus(status: AIbuddyTaskRuntimeStatus): boolean {
