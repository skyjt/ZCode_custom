import type { AIbuddyWorkspaceTaskListChanged } from "@aibuddy/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<AIbuddyWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<AIbuddyWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<AIbuddyWorkspaceTaskListChanged, "reason">,
): boolean {
