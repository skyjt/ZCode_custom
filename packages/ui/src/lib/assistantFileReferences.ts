...
import { extractConversationPreviewFileReferences } from "@aibuddy/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@aibuddy/shared";
import {
  extractAIbuddyFileCitationDirectives,
  resolveAIbuddyFileCitationPreviewKind,
} from "@/lib/aibuddyFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractAIbuddyFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 aibuddy-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
