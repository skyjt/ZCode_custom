...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.aibuddy/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.aibuddy/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isAIbuddySkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.aibuddy/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: AIbuddyProvider,
): T[] {
  return skills.filter(isAIbuddySkill);
}
