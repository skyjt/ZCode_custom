import type {
  ...
  AIbuddyTaskChangedFileSummary,
  AIbuddyTaskMeta,
} from "@aibuddy/shared";
import { computeLineChangeStat } from "@aibuddy/shared";
...
export function getTaskChangeSummary(
  task: Pick<AIbuddyTaskMeta, "changeSummary"> | null | undefined,
): AIbuddyTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: AIbuddyTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly AIbuddyPersistedFileChange[] | undefined,
): AIbuddyTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: AIbuddyTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: AIbuddyPersistedFileChange | null | undefined,
): AIbuddyTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: AIbuddyTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly AIbuddyPersistedFileChange[] | undefined,
): Map<number, AIbuddyTaskChangeSummary> {
  const result = new Map<number, AIbuddyTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
