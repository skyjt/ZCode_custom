...
import { extractAIbuddyFileCitations } from "@/lib/aibuddyFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractAIbuddyFileCitations(value);
...
export function createAIbuddyFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function aibuddyFileCitationRemarkPlugin() {
    return (tree: unknown) => {
