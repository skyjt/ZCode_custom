import {
  getAIbuddyToolFamilyForName,
  isTodoPlanToolName,
  isAIbuddyFileContentWriteToolName,
  normalizeAIbuddyToolName,
  type AIbuddyKnownToolName,
  type AIbuddyToolFamily,
} from "@aibuddy/shared";
...
export interface ToolCallIdentity {
  toolName: AIbuddyKnownToolName | string | null;
...
function identityFromKnownToolName(
  ...
  const family = getAIbuddyToolFamilyForName(toolName);
  if (!family) {
    return null;
  }
  return {
...
function readRawToolNameCandidates(raw: unknown) {
  return {
    ...
    aibuddy: readNestedString(raw, ["_meta", "aibuddy", "toolName"]),
...
function isLegacyAgentTool(
  ...
  const rawAIbuddy = normalizeLegacyToken(rawNames.aibuddy);
...
export function resolveToolCallIdentity(toolCall: ToolIdentityLike): ToolCallIdentity {
  ...
  for (const candidate of [
    ...
    { value: rawNames.aibuddy, source: "raw-aibuddy-meta" as const },
  ]) {
  ...
  if (isLegacyAgentTool(toolCall, rawNames)) {
    return identityFromLegacyFamily(
      rawNames.direct ?? rawNames.aibuddy ?? "Agent",
      "agent",
      "legacy-payload",
    );
  }

  // `Task` 现在是现役 Claude 兼容工具名，但历史 AIbuddy Agent 投影会用
  ...
  if (
    // 当前 AIbuddy Agent 传给 app 的 plan mode 退出工具是 ExitPlanMode，
...
export function isFileContentWriteToolCall(
  ...
  if (isAIbuddyFileContentWriteToolName(identity.toolName)) {
    return true;
  }
