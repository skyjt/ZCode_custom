...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: AIbuddyTaskMeta,
    overrides?: Partial<AIbuddyTaskMeta>,
  ): AIbuddyTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.aibuddyTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.aibuddyTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 aibuddyTaskMetaMerge：归档后只有旧列表状态被更新。
