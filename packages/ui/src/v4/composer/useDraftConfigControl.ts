...
import { AIBUDDY_AGENT_PROVIDER, resolveExecutionState } from "@aibuddy/shared";
...
import { submissionModeSchema } from "@aibuddy/shared/aibuddy-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  ...
  slashCommands: readonly AIbuddySlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const aibuddySessionService = useAIbuddySessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithAIbuddySessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useAIbuddySessionStore.getState();
