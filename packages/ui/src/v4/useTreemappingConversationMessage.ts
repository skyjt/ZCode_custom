...
// 复用 treemappingActivity 的既有解析规则，不复活 aibuddyChatMessages。
//
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(aibuddyAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [aibuddyAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
