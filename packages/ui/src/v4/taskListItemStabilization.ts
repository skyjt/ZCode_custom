...
//
// sessions-index / Controller tasks-index 每个内容帧都会全量重建 AIbuddyTaskMeta[]，
...
export function buildTaskListItemIdentityKey(meta: AIbuddyTaskMeta): string {
...
export function areTaskListItemsEquivalent(left: AIbuddyTaskMeta, right: AIbuddyTaskMeta): boolean {
  return areStabilizedValuesEquivalent(left, right);
}

/** 引用稳定化：等价条目复用旧对象；顺序与内容全等时复用整个旧数组。 */
export function stabilizeTaskListItems<T extends AIbuddyTaskMeta>(previous: T[], next: T[]): T[] {
