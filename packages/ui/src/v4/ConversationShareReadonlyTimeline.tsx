 ...
 * 公开页 bundle 体积与无 Desktop 宿主（window.aibuddy / PlatformProvider / tab store）的运行环境；
...
import { getCompactToolCallStatusMessageId, type Locale } from "@aibuddy/shared";
import type {
  ArtifactRow,
  AssistantTextRow,
  ConversationRow,
...
import { AIbuddyIntlProvider, useAIbuddyIntl } from "@/i18n/IntlProvider.js";
...
function resolveImportedArtifactPath(
  ...
  // 取舍：这里只校验形状（.aibuddy-share/<dir>/shared-artifacts/<file> 四段），刻意不把
  ...
  // .aibuddy-share/*/shared-artifacts/ 内，无越权读放大。
...
export function ConversationShareReadonlyTimeline({
  ...
  const labels: ReadonlyLabels =
    locale === "zh-CN"
      ? {
          ...
          unsupportedRows: "部分内容需要更新 AIbuddy 查看",
        }
      : {
          history: "Reasoning",
          ...
          unsupportedRows: "Some content requires a newer version of AIbuddy",
        };
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <AIbuddyIntlProvider initialLocale={locale}>
