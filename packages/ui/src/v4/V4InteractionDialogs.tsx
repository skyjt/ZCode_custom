...
import type { AIbuddyElicitationRequest, AIbuddyPermissionOption, AIbuddyProvider } from "@aibuddy/shared";
...
import type { ElicitationFormDraft } from "@/store/aibuddySessionStoreTypes.js";
...
function buildV4ElicitationProgressKey(request: AIbuddyElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useAIbuddyStoreWithDefault((state) => state.notificationEnabled, true);
  const localElicitationDraft = useAIbuddySessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useAIbuddySessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: AIbuddyPermissionOption, feedback?: string) => {
