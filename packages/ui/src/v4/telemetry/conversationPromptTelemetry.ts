import { AIBUDDY_AGENT_PROVIDER, type PlanIdentitySnapshot, type AIbuddyProvider } from "@aibuddy/shared";
...
function resolveLegacyConversationModelValue(params: {
  ...
  if (!configProvider || !modelName || configProvider === AIBUDDY_AGENT_PROVIDER) {
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 AIbuddy 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: AIbuddyProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  const agentProvider = params.agentProvider ?? AIBUDDY_AGENT_PROVIDER;
  ...
  return {
    ...
    // agent 表示 AIbuddy 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}
