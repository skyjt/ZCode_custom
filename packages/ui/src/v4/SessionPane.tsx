...
import type { SessionCreateSource } from "@aibuddy/shared";
...
import { localizeConversationShareUrl } from "@aibuddy/shared";
import type {
...
import { prepareWorkspaceWithAIbuddySessionService } from "@/hooks/useWorkspacePrepare.js";
import {
  createCodingPlanFunnelContext,
  resolveCodingPlanEntryPlanState,
} from "@/lib/codingPlanFunnelTelemetry.js";
import { decodeCustomModelValue, encodeCustomModelValue } from "@/lib/aibuddyCustomModelValue.js";
import { parseModelPickerValue } from "@/lib/aibuddySessionProjection.js";
...
import type { GroupedDraftTaskState } from "@/store/aibuddySessionStoreTypes.js";
import {
...
export function SessionPane({
  ...
  const { conversationShareService, modelSelectionService, aibuddySessionService, aibuddyTaskService } =
    useServices();
  ...
  // send_result 的落定信号：用户消息真正画进对话历史。AIbuddy 没有乐观渲染，
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<AIbuddyUiError | null>(null);
  ...
  const workspaceConfigOptions = useAIbuddySessionStore(
  ...
  useEffect(() => {
    ...
    useAIbuddySessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useAIbuddySessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const theme = useAIbuddyStoreWithDefault((state) => state.theme, "system");
  const codePreviewSettings = useAIbuddyStoreWithDefault(
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useAIbuddySessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await aibuddySessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useAIbuddySessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "AIBUDDY_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      try {
        await aibuddyTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithAIbuddySessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          aibuddySessionService,
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 AIbuddy」，不能整块消失。
                importedShare &&
