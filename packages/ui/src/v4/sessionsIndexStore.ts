...
import { isAIbuddyFileLockTimeoutError } from "@aibuddy/shared";
import { AIBUDDY_AGENT_RUNTIME_UNAVAILABLE_CODE } from "@aibuddy/services";
...
function isTransientSubscribeError(error: unknown): boolean {
  if (isAIbuddyFileLockTimeoutError(error)) {
    return true;
  }
  const code =
