...
const RUNTIME_RECYCLED_ERROR = "AIbuddy agent runtime 已被回收，重连未成功";
 ...
 *
 * 三种文案都来自同一次回收：transport 关闭时 AIbuddyProtocolClient.rejectAll 打断在途请求
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("AIbuddy agent transport closed") ||
    message.includes("AIbuddy Protocol client disposed") ||
    message.includes("AIbuddy Protocol client is disposed") ||
    message.includes("AIbuddy Agent runtime is not running")
  );
}
...
export class ConversationProjectionStore {
  ...
  /**
   * 有界退避重连。重订阅走 start-if-needed（aibuddyAgentService.subscribeConversationV4），
