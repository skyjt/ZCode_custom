// sessions-index 的 SessionSummary → 侧栏实时 detail AIbuddyTaskMeta 映射。
...
import type { TraceId, AIbuddyProvider, AIbuddyTaskMeta } from "@aibuddy/shared";
import type { SessionSummary } from "@aibuddy/shared/aibuddy-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): AIbuddyTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: AIbuddyTaskMeta;
}

/**
 * SessionSummary → 实时 detail AIbuddyTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as AIbuddyProvider } : {}),
      ...(status ? { status } : {}),
