...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: AIbuddyProvider | null }) => {
      ...
      const aibuddySessionStore = useAIbuddySessionStore.getState();
      const latestWorkspaceState = aibuddySessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: AIbuddyProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      aibuddySessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        aibuddySessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        aibuddySessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        aibuddySessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.aibuddyTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithAIbuddySessionService({
            ...
            workspaceIdentity,
            provider,
            aibuddySessionService: services.aibuddySessionService,
          });

          const latestAfterPrepare = aibuddySessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = aibuddySessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              aibuddySessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              aibuddySessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              aibuddySessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        aibuddySessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        aibuddySessionStore.setWorkspaceInitState(workspaceAbsPath, "ready", null, workspaceIdentity);
        ...
        if (shouldPrepareWorkspace) {
          aibuddySessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          aibuddySessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
    ...
    [
      ...
      services.aibuddySessionService,
      workspaceAbsPath,
      workspaceIdentity,
    ],
  );
