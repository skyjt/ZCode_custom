...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useAIbuddySessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useAIbuddySessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useAIbuddySessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useAIbuddySessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useAIbuddySessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useAIbuddySessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useAIbuddySessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useAIbuddySessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.aibuddyTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useAIbuddySessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useAIbuddySessionStore
        .getState()
  ...
  const currentWorkspaceState = useAIbuddySessionStore.getState().getWorkspaceState(workspaceAbsPath);
