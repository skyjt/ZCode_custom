...
export function trajectoryToolOutputs(message: AIbuddyModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: AIbuddyModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: AIbuddyModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: AIbuddyModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
