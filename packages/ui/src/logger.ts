import { formatLogPrefix } from "@aibuddy/shared";
  ...
  return (
    (
      globalThis as typeof globalThis & {
        __AIBUDDY_RENDERER_DISABLE_LOGGING__?: boolean;
      }
    ).__AIBUDDY_RENDERER_DISABLE_LOGGING__ === true
  );
}
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.aibuddy 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).aibuddy?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).aibuddy?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
