...
import { AIBUDDY_MODE_OPTION_DESCRIPTION_IDS, AIBUDDY_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: AIbuddyConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: AIbuddyConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  ...
  entry: AIbuddyConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 AIbuddy Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<AIbuddyConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: AIbuddyProvider | undefined,
  entry: Pick<AIbuddyConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return AIBUDDY_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return AIBUDDY_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
