...
import type { ModelSelection } from "@aibuddy/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly aibuddyBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#aibuddyBuiltinSource = dependencies.aibuddyBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#aibuddyBuiltinSource.onDidChange((reason) => this.#emit(`aibuddyBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [aibuddyBuiltin, personal] = await Promise.all([
      this.#aibuddyBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([aibuddyBuiltin.revision, personal.revision]),
      aibuddyBuiltinRevision: aibuddyBuiltin.revision,
      personalRevision: personal.revision,
      aibuddyBuiltinProviders: aibuddyBuiltin.providers,
      aibuddyBuiltinProviderTemplates: aibuddyBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      aibuddyBuiltinModelRules: aibuddyBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const aibuddyBuiltin = await this.#aibuddyBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = aibuddyBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = aibuddyBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 AIbuddy Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(aibuddyBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = aibuddyBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? aibuddyBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...aibuddyBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(aibuddyBuiltin, current.providers);
  ...
  async reorderPersonalProviders(
    ...
    return this.#updatePersonal((current) => ({
      ...
      models: current.models,
      providerOrder: normalizeProviderOrder(aibuddyBuiltin.providers, current.providers, providerIds),
    }));
  }
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        aibuddyBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(aibuddyBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(aibuddyBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(aibuddyBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(aibuddyBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#aibuddyBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _aibuddyBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  aibuddyBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    aibuddyBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(aibuddyBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? aibuddyBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return aibuddyBuiltin.providers.overlay(personal);
}
