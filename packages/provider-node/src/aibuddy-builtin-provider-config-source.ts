...
import {
  decodeAIbuddyBuiltinRelease,
  encodeAIbuddyBuiltinRelease,
  serializeAIbuddyBuiltinRelease,
  type AIbuddyBuiltinRelease,
} from "./aibuddy-builtin-release.js";

export interface NodeAIbuddyBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyAIbuddyBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeAIbuddyBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeAIbuddyBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("AIbuddy Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: AIbuddyBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: AIbuddyBuiltinRelease): Promise<ApplyAIbuddyBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeAIbuddyBuiltinRelease(release) === serializeAIbuddyBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`AIbuddy Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<AIbuddyBuiltinRelease> {
  ...
  async #writeActiveLocked(release: AIbuddyBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeAIbuddyBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeAIbuddyBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeAIbuddyBuiltinProviderConfigSource(
  options: NodeAIbuddyBuiltinProviderConfigSourceOptions,
): NodeAIbuddyBuiltinProviderConfigSource {
  return new NodeAIbuddyBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: AIbuddyBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeAIbuddyBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
...
): AIbuddyBuiltinRelease {
  if (
    bundled?.release &&
    active?.release &&
    bundled.release.revision === active.release.revision &&
    serializeAIbuddyBuiltinRelease(bundled.release) !== serializeAIbuddyBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is AIbuddyBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active AIbuddy Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  release: AIbuddyBuiltinRelease,
  sourceKey: string,
): ProviderConfigLayerSnapshot {
  return Object.freeze({
    revision: `aibuddy-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: AIbuddyBuiltinRelease): string {
  return `${release.revision}:${serializeAIbuddyBuiltinRelease(release)}`;
}
