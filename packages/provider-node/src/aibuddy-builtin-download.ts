import { z } from "zod";
import { decodeAIbuddyBuiltinRelease, type AIbuddyBuiltinRelease } from "./aibuddy-builtin-release.js";
...
export interface AIbuddyBuiltinDownloadOptions {
...
export async function downloadAIbuddyBuiltinRelease(
  options: AIbuddyBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeAIbuddyBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`AIbuddy Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
