...
import type { NodeAIbuddyBuiltinProviderConfigSource } from "./aibuddy-builtin-provider-config-source.js";
import type { AIbuddyBuiltinRelease } from "./aibuddy-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type AIbuddyBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface AIbuddyBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: AIbuddyBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface AIbuddyBuiltinRefreshEvent {
  readonly result: AIbuddyBuiltinRefreshResult;
...
export class AIbuddyBuiltinRemoteSynchronizer {
  readonly #options: AIbuddyBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<AIbuddyBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: AIbuddyBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<AIbuddyBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<AIbuddyBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("AIbuddy Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: AIbuddyBuiltinRefreshEvent): void {
