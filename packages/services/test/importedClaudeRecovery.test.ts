...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "aibuddy-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["aibuddyAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: AIbuddySessionStateSnapshot | undefined;
    ...
    const service = createAIbuddyTaskServiceAdapter({
      ...
      aibuddyAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = aibuddySessionStateSnapshotSchema.parse({
