...
import { readLegacyAIbuddyConfigProviders } from "../src/model-provider/legacyAIbuddyConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyAIbuddyConfigProviders();
    },
...
test("startup migrates published AIbuddy config into personal config without changing the source", async () => {
