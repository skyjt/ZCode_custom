...
import { createAIbuddyTaskServiceAdapter } from "../src/aibuddy-agent/aibuddyTaskServiceAdapter.js";
import {
...
test("current Project Memory catalog and files remain readable", async () => {
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-current-memory-"));
  setDataBaseDir(dir);
  try {
...
test("opening the task index leaves retired ACP IDs and user rows untouched", async () => {
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-acp-index-"));
...
test("missing sessions report the owner error even when a valid ACP snapshot exists", async () => {
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-acp-snapshot-"));
  setDataBaseDir(dir);
  ...
  type Options = Parameters<typeof createAIbuddyTaskServiceAdapter>[0];
  const service = createAIbuddyTaskServiceAdapter({
    aibuddyAgentService: {
      async resumeSession() {
        throw ownerError;
      },
      disposeAll() {},
    } as unknown as Options["aibuddyAgentService"],
    taskIndexRepo: {
      async getTaskMeta() {
        return meta;
      },
...
test("current session recovery preserves Desktop and replayable projections", async () => {
  const snapshot = aibuddySessionStateSnapshotSchema.parse({
    protocol: { name: AIBUDDY_PROTOCOL_NAME, version: AIBUDDY_PROTOCOL_VERSION },
    session: {
...
test("current Provider configuration starts without an old config migration callback", async () => {
  const dir = await mkdtemp(join(tmpdir(), "aibuddy-provider-current-"));
  const runtime = createProviderConfigRuntime({
    aibuddyBuiltinFilePath: fileURLToPath(
      new URL("../../../config/provider/aibuddy-builtin.json", import.meta.url),
    ),
  ...
  try {
    ...
    assert.ok(config.aibuddyBuiltinRevision);
