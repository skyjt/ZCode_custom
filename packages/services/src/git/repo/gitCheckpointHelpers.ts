...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@aibuddy/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/aibuddy/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "AIbuddy Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@aibuddy.local",
    GIT_COMMITTER_NAME: "AIbuddy Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@aibuddy.local",
  };
}
