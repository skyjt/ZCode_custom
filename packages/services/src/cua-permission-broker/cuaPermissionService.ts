...
// to @aibuddy/aibuddy-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @aibuddy/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @aibuddy/services root
...
// at compile time; Vite never resolves @aibuddy/aibuddy-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@aibuddy/aibuddy-cua/broker";
...
} from "@aibuddy/aibuddy-cua/broker/ports";
