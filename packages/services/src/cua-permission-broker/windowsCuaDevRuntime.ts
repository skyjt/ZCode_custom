...
const DEV_ROOT_ENV = "AIBUDDY_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@aibuddy/aibuddy-cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.aibuddyCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid aibuddyCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
