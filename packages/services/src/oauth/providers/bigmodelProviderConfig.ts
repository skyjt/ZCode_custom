...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: buildBigModelApiUrl({ AIBUDDY_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "aibuddy",
  redirectUri: "aibuddy://oauth/callback",
};
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildAIbuddyApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 aibuddy OAuth token 路由，显式 appSecret 仅保留给
