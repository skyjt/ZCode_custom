...
} from "#src/session/aibuddyTaskService.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import {
  readTrajectoryFileTail,
  resolveModelIODirs,
  sanitizeSessionSegment,
} from "#src/aibuddy-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/aibuddy-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.aibuddy/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - AIbuddy Agent 把 taskId 当作 sessionId（见 aibuddyTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): AIbuddyModelTrajectoryRecord {
  ...
  const mapped: AIbuddyModelTrajectoryRecord = {
...
function mapMessages(value: unknown): AIbuddyModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): AIbuddyModelTrajectoryContentPart {
...
function mapUsage(value: unknown): AIbuddyModelTrajectoryUsage | undefined {
