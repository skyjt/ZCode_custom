/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 AIbuddy session 的协议适配。 */
...
import {
  coalesceConsecutiveAIbuddyAssistants,
  ...
  generateTraceId,
  getAIbuddyGoalActiveIterationCount,
  getAIbuddyGoalIterationByAssistantMessageId,
  getAIbuddyUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeAIbuddyApiRetryStatus,
  attachAIbuddyBackgroundTaskNotificationToRaw,
  collectAIbuddyBackgroundTaskNotificationsByToolUseId,
  mergeAIbuddyBackgroundTaskControlItems,
  parseAIbuddyBackgroundTaskControlItems,
  parseAIbuddyBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveAIbuddyVisibleSessionTitle,
  textFromAIbuddyMessageParts,
  AIBUDDY_AGENT_PROVIDER,
  aibuddyBackgroundTaskNotificationToolUpdateStatus,
  appendAIbuddyStreamingToolInputDelta,
  buildAIbuddyStreamingToolInputPreview,
  createAIbuddyToolProjectionMemory,
  finalizeAIbuddyToolProjectionInput,
  forgetAIbuddyToolProjectionMetadata,
  isAIbuddyModelRetryRecoveryProgressPayload,
  markAIbuddyStreamingToolInputPreviewMaterialized,
  resolveAIbuddyToolProjectionMetadata,
  aibuddyApiRetryFromModelNetworkStatusPayload,
  aibuddyApiRetryFromStreamRecoveryPayload,
  aibuddyTaskNetworkDebugStatusFromPayload,
  shouldMaterializeAIbuddyStreamingToolInputPreview,
  type AIbuddyApiRetryStatus,
  type AIbuddyAssistantMessageFeedback,
  type AIbuddyBackgroundTaskNotificationInfo,
  type AIbuddyBackgroundTaskControlItem,
  type AIbuddyBackgroundTurnAttribution,
  type AIbuddyCancelTaskCommandResult,
  type AIbuddyConfigOption,
  type AIbuddyEnqueueTaskCommandResult,
  type AIbuddyError,
  type AIbuddyGoalVerificationTimelineMeta,
  type AIbuddyImportSessionsResult,
  type AIbuddyImportableSessionCandidate,
  type AIbuddyUsage,
  type AIbuddyPersistedMessage,
  type AIbuddyPersistedMessagePart,
  type AIbuddyPersistedToolCall,
  type AIbuddyPromptAttachment,
  type AIbuddyProvider,
  type AIbuddySessionFile,
  type AIbuddyTaskGoal,
  type AIbuddyTaskGoalStats,
  type AIbuddyTaskMode,
  type AIbuddyPlanStep,
  type AIbuddySlashCommand,
  type AIbuddyStreamEvent,
  type AIbuddyTaskCreateResult,
  type AIbuddyTaskMeta,
  type AIbuddyTaskClientMode,
  type AIbuddyTaskRuntimeCommand,
  type AIbuddyTaskSnapshot,
  type AIbuddyTaskSnapshotBody,
  type AIbuddyTaskSnapshotRefContent,
  type AIbuddyTaskSnapshotToolCallsSlice,
  type AIbuddyTaskTokenUsageResult,
  type AIbuddyTodoGroup,
  type AIbuddyTurnSteerCommandKind,
  type AIbuddyTurnSteerSource,
  type AIbuddyWorkspaceEvent,
  type AIbuddyWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  aibuddyContextUsageBreakdownSchema,
  aibuddySessionSettingsStateSchema,
  type AIbuddyDeliveryKind,
  type AIbuddyMessagePart,
  type AIbuddyMessageWithParts,
  type ModelSelection,
  type AIbuddyPermissionOption,
  type AIbuddyPermissionRequestParams,
  type AIbuddyPermissionRequest,
  type AIbuddySessionEvent,
  type AIbuddySessionMode,
  type AIbuddySessionSettingsState,
  type AIbuddySessionStateSnapshot,
  type AIbuddyStateUpdatedNotification,
  type AIbuddyContextCompactionTimelineMeta,
  type AIbuddyTimelineMeta,
  type AIbuddyTimelineStatus,
  type AIbuddyTimelineTrigger,
  type AIbuddyToolProjectionMemory,
  type AIbuddyUserInputRequestParams,
  type AIbuddyUserInputResponse,
  type AIbuddyAgentMcpServer,
} from "@aibuddy/shared";
import type {
  AIbuddyTaskListQuery,
  AIbuddyTaskListResult,
  AIbuddyWorkspaceEventSubscriptionParams,
  IAIbuddyTaskService,
  AIbuddyArchivedTaskDeletionResult,
  AIbuddyTaskReadyOutcome,
  AIbuddyTaskTerminalOutcome,
} from "../session/aibuddyTaskService.js";
...
import type {
  AIbuddyTaskIndexReadyEvent,
  AIbuddyTaskIndexSyncer,
  AIbuddyTaskIndexTerminalEvent,
} from "./aibuddyTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@aibuddy/shared/aibuddy-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./aibuddyV4HostCommand.js";
...
import {
  ...
  normalizeAvailableAIbuddyMode,
  settingsToConfigOptions,
} from "./aibuddyConfigOptions.js";
...
interface CreateAIbuddyTaskServiceAdapterOptions {
  aibuddyAgentService: IAIbuddyAgentService;
  ...
  taskIndexSyncer: AIbuddyTaskIndexSyncer;
...
type WorkspaceEventInput = string | AIbuddyWorkspaceEventSubscriptionParams;
type AIbuddySendPromptRuntimeCommand = Extract<AIbuddyTaskRuntimeCommand, { type: "send_prompt" }>;
type AIbuddyTerminalStreamEvent =
  | Extract<AIbuddyStreamEvent, { type: "task_complete" }>
  | Extract<AIbuddyStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: AIbuddyProvider = AIBUDDY_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: AIbuddySlashCommand[] = [];
const logger = createServiceLogger("aibuddy-task-service");
...
function formatAIbuddyAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveAIbuddyAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.AIBUDDY_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".aibuddy", "cli", "log");
  return join(logDir, `aibuddy-${formatAIbuddyAgentLogDate(now)}.jsonl`);
}

export function createAIbuddyTaskServiceAdapter(
  options: CreateAIbuddyTaskServiceAdapterOptions,
): IAIbuddyTaskService {
  const errorEmitter = new Emitter<AIbuddyError>();
  const taskEmitters = new Map<string, Emitter<AIbuddyStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<AIbuddyStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, AIbuddyTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, AIbuddyApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, AIbuddyBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, AIbuddyToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`AIbuddy task service adapter does not support IAIbuddyTaskService.${name} yet.`),
      {
        code: "AIBUDDY_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: AIbuddyBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: AIbuddyAgentMcpServer[] | undefined,
  ): Promise<AIbuddyAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    params: {
      ...
      messageId?: string;
      content: string;
      attachments?: AIbuddyPromptAttachment[];
      toolDenylist?: string[];
      clientId?: string;
    ...
    // AIbuddy task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 AIbuddy session 事件回投 AIbuddy Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "AIbuddy task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.aibuddyAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.aibuddyAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "AIbuddy task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "AIbuddy task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: AIbuddyTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: AIbuddySendPromptRuntimeCommand,
  ): AIbuddySendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: AIbuddySendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: AIbuddySendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "AIbuddy task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: AIbuddyTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is AIbuddySendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "AIbuddy task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "AIbuddy task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "AIbuddy task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<AIbuddyStreamEvent, { type: "tool_call_update" }>["status"],
  ): AIbuddyPersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: AIbuddyStreamEvent): void {
    ...
    const nextTool: AIbuddyPersistedToolCall =
  ...
  function persistedToolId(tool: AIbuddyPersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: AIbuddyPersistedToolCall | undefined,
    liveTool: AIbuddyPersistedToolCall,
  ...
  function findToolLocation(
    messages: readonly AIbuddyPersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly AIbuddyPersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly AIbuddyPersistedMessagePart[] | undefined,
    toolIndex: number,
  ): AIbuddyPersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: AIbuddyPersistedMessage,
    toolIndex: number,
    tool: AIbuddyPersistedToolCall,
  ): AIbuddyPersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "AIbuddy snapshot 合并 live tool projection", {
        event: "aibuddy_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`AIbuddy session target is not loaded: ${taskId}`), {
        code: "AIBUDDY_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): AIbuddyToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createAIbuddyToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<AIbuddyWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<AIbuddyStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<AIbuddyStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<AIbuddyStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: AIbuddyStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: AIbuddyTaskMeta | undefined,
    reason: AIbuddyWorkspaceTaskListChanged["reason"],
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.aibuddyAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.aibuddyAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 AIbuddySessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.aibuddyAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.aibuddyAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: AIbuddySessionStateSnapshot): AIbuddyTaskMeta {
    const target = {
  ...
  function rememberIndexedTaskMeta(meta: AIbuddyTaskMeta): AIbuddyTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 AIbuddy session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: AIbuddyTaskMeta): Promise<AIbuddyTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 AIbuddy session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: AIbuddyStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: AIbuddyStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToAIbuddy(
    ...
    const goalIterationByAssistantMessageId = getAIbuddyGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectAIbuddyBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveAIbuddyAssistants(
              getAIbuddyUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: AIbuddyPermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<AIbuddyStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseAIbuddyBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: AIbuddyAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 AIbuddy snapshot 到 task index 失败", error);
      });
      const snapshotEvent: AIbuddyStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: AIbuddyTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: AIbuddyTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeAIbuddyAgentServiceAndWait(): Promise<void> {
    const agentService = options.aibuddyAgentService as IAIbuddyAgentService & {
  ...
  const service: IAIbuddyTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.aibuddyAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.aibuddyAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<AIbuddyTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: AIbuddySessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.aibuddyAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.aibuddyAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.aibuddyAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.aibuddyAgentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toAIbuddyMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<AIbuddyEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: AIbuddyTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<AIbuddyEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<AIbuddyCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "AIbuddy task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "AIbuddy task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "AIbuddy task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "AIbuddy task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "AIbuddy task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.aibuddyAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[aibuddy-task-service] goalSession start", {
      ...
      const result = await options.aibuddyAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[aibuddy-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[aibuddy-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.aibuddyAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<AIbuddyTaskMeta> {
    ...
    async listTasks(params): Promise<AIbuddyTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<AIbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: AIbuddyTaskListQuery): Promise<AIbuddyTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<AIbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<AIbuddyTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<AIbuddyTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<AIbuddyTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const aibuddySnapshot = limitTaskSnapshotMessages(
        snapshotToAIbuddy(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(aibuddySnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[aibuddy-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(aibuddySnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...aibuddySnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[aibuddy-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[aibuddy-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[aibuddy-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<AIbuddyTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<AIbuddyTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<AIbuddyTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<AIbuddyTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<AIbuddyConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.aibuddyAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<AIbuddySessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as AIbuddyAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<AIbuddyImportableSessionCandidate[]> {
      ...
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.aibuddyAgentService.createSession({
          ...
          // 导入后的任务必须是真实 AIbuddy session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toAIbuddyMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<AIbuddyConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.aibuddyAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toAIbuddyMode(params.value as AIbuddyTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<AIbuddyConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.aibuddyAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<AIbuddyConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveAIbuddyAgentCurrentLogFilePath();
      // 返回 AIbuddy Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // AIbuddy Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[AIbuddyTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<AIbuddyTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 AIbuddy Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.aibuddyAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.aibuddy-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<AIbuddyArchivedTaskDeletionResult> {
      const result: AIbuddyArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<AIbuddyTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[AIbuddyTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[AIbuddyTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[AIbuddyTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[AIbuddyTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[AIbuddyTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<AIbuddyTaskMeta> {
    ...
    async setTaskUnread(params): Promise<AIbuddyTaskMeta> {
    ...
    async archiveTask(params): Promise<AIbuddyTaskMeta> {
    ...
    async unarchiveTask(params): Promise<AIbuddyTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<AIbuddyTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<AIbuddyStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<AIbuddyTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<AIbuddyTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<AIbuddyStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 AIbuddyStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.aibuddyAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toAIbuddyDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<AIbuddyWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.aibuddyAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 aibuddy-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeAIbuddyAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: AIbuddyTaskMeta, overlay: TaskOverlay): AIbuddyTaskMeta {
  return {
    ...meta,
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<AIbuddyTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: AIbuddyTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 aibuddy-cli。
...
function toAIbuddyMode(mode: AIbuddyTaskMode | undefined): AIbuddySessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `aibuddy-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<AIbuddyPersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<AIbuddyGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `aibuddy-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `aibuddy-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: AIbuddyPersistedMessage[],
  timelineMessages: AIbuddyPersistedMessage[],
): AIbuddyPersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: AIbuddySessionStateSnapshot): number {
  ...
  return getAIbuddyGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toAIbuddyDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<AIbuddyStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseAIbuddyBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = aibuddyBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachAIbuddyBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: AIbuddyMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, AIbuddyBackgroundTaskNotificationInfo>,
): AIbuddyPersistedMessage {
  const tools: AIbuddyPersistedToolCall[] = [];
  const parts: AIbuddyPersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: AIbuddyTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly AIbuddyMessagePart[],
): AIbuddyPromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<AIbuddyMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is AIbuddyPromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<AIbuddyMessagePart, { type: "file" }>,
): AIbuddyPromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<AIbuddyMessagePart, { type: "text" }>,
): AIbuddyTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<AIbuddyMessagePart, { type: "compaction" }>,
): AIbuddyTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<AIbuddyMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, AIbuddyBackgroundTaskNotificationInfo>,
): AIbuddyPersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // AIbuddy Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachAIbuddyBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? aibuddyBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly AIbuddyMessagePart[]): string {
  return textFromAIbuddyMessageParts(parts);
}

function reasoningFromParts(parts: readonly AIbuddyMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<AIbuddyMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function sessionTodosToPlanSteps(
  todos: AIbuddySessionStateSnapshot["todos"],
): AIbuddyPlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: AIbuddySessionStateSnapshot["goalStats"],
): AIbuddyTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: AIbuddySessionStateSnapshot["todoGroups"],
): AIbuddyTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = aibuddySessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableAIbuddyMode(parsedSettings.data.mode.current),
      availableModes: getAIbuddyAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: AIbuddyToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, AIbuddyBackgroundTaskControlItem[]>,
  ...
  // AIbuddy Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 AIbuddy prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: AIbuddyStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: AIbuddyStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "AIbuddy compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "AIbuddy session failed",
...
): Extract<AIbuddyStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isAIbuddyModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendAIbuddyStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeAIbuddyStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildAIbuddyStreamingToolInputPreview(state.rawInput);
    markAIbuddyStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildAIbuddyStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeAIbuddyToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "AIbuddy streaming tool input projected", {
    ...details,
    event: "aibuddy.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<AIbuddyStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<AIbuddyMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): AIbuddyContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<AIbuddyStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveAIbuddyToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeAIbuddyToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: AIbuddyStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // AIbuddy Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 AIbuddy Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetAIbuddyToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // AIbuddy Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: AIbuddyPermissionRequestParams,
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: AIbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): AIbuddyPermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: AIbuddyUserInputRequestParams,
): AIbuddyStreamEvent {
  const questions =
...
): Extract<AIbuddyStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: AIbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<AIbuddyStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: AIbuddySessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<AIbuddyStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, AIbuddyBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: AIbuddyBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): AIbuddyBackgroundTaskControlItem[] | null {
  const parsedJobs = parseAIbuddyBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeAIbuddyBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): AIbuddyPermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as AIbuddyPermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<AIbuddyStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = aibuddyTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromAIbuddyGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromAIbuddyGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseAIbuddyBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<AIbuddyStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): AIbuddyApiRetryStatus | null | undefined {
...
function recordAgentModelNetworkTelemetry(event: AIbuddySessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `aibuddy_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<AIbuddyStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: AIbuddySessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: AIbuddySessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // AIbuddy Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = aibuddyContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): AIbuddyUsage | undefined {
...
function timelineStatusValue(value: unknown): AIbuddyTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): AIbuddyTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): AIbuddyTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): AIbuddyTurnSteerCommandKind | undefined {
