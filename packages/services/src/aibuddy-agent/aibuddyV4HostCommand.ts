...
class AIbuddyV4CommandRejectedError extends Error {
  readonly code = "AIBUDDY_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "AIbuddyV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new AIbuddyV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new AIbuddyV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
