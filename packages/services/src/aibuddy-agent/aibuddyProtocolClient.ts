import { AIbuddyStorageStartupGate } from "#src/aibuddy-agent/aibuddyStorageStartupGate.js";
...
import type {
  ...
  AIbuddyProtocolTrace,
} from "@aibuddy/shared";
import type { V4Method } from "@aibuddy/shared/aibuddy-protocol-v4";
import type { z } from "zod";
import type { AIbuddyProtocolTransport } from "./aibuddyProtocolTransport.js";

/** 客户端可发的方法名：旧 aibuddyProtocolMethods + v4/*（并存，收敛为 v4）。 */
type AIbuddyProtocolClientMethod = AIbuddyProtocolMethod | V4Method;

interface AIbuddyProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface AIbuddyProtocolRequestTimeoutEvent {
  method: AIbuddyProtocolClientMethod;
  requestId: AIbuddyProtocolRequestId;
  timeoutMs: number;
}
...
interface AIbuddyProtocolClientRequestOptions {
...
const DEFAULT_AIBUDDY_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class AIbuddyProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "AIbuddyProtocolClientError";
  }
}

export class AIbuddyProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: AIbuddyProtocolClientMethod,
    readonly requestId: AIbuddyProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`AIbuddy Protocol request timed out: ${method}`);
    this.name = "AIbuddyProtocolRequestTimeoutError";
  }
}

export class AIbuddyProtocolClient implements IDisposable {
  readonly storageStartup: AIbuddyStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<AIbuddyProtocolNotification>();
  private readonly requestEmitter = new Emitter<AIbuddyProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<AIbuddyProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "AIbuddy Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: AIbuddyProtocolTransport,
    options?: AIbuddyProtocolClientOptions,
  ) {
    this.storageStartup = new AIbuddyStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_AIBUDDY_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`AIbuddy agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new AIbuddyProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: AIbuddyProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: AIbuddyProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: AIbuddyProtocolRequestId; result: unknown }).id,
    ...
    if ("error" in message && "id" in message) {
      ...
      this.rejectResponse(
        errorMessage.id,
        new AIbuddyProtocolClientError(
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as AIbuddyProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as AIbuddyProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as AIbuddyProtocolNotification);
    }
  }

  private resolveResponse(id: AIbuddyProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`AIbuddy Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: AIbuddyProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("AIbuddy Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("AIbuddy Protocol client is disposed");
    }
  }
}
