import { resolveWorkspaceKey, AIBUDDY_AGENT_PROVIDER } from "@aibuddy/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IAIbuddyAgentService, AIbuddyAgentWorkspaceTarget } from "#src/aibuddy-agent/aibuddyAgent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IAIbuddyAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: AIBUDDY_AGENT_PROVIDER });
  const ids = rows
