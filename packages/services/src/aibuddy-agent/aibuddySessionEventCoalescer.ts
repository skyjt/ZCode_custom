import type { AIbuddySessionEvent } from "@aibuddy/shared";
import type { AIbuddyAgentServiceEvent } from "#src/aibuddy-agent/aibuddyAgent.js";
...
type SessionServiceEvent = Extract<AIbuddyAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: AIbuddyAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: AIbuddySessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: AIbuddySessionEvent,
  next: AIbuddySessionEvent,
): AIbuddySessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: AIbuddyAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
