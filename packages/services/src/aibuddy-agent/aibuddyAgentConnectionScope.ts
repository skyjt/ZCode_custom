...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@aibuddy/rpc";
import {
...
export type AIbuddyAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface AIbuddyAgentV4ConnectionContext {
  connectionId: string;
  clientMode: AIbuddyAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__aibuddyTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__aibuddyTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: AIbuddyAgentV4ConnectionContext;
};

interface TrustedAIbuddyAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedAIbuddyAgentV4UnsubscribeRoute;
};

export function readTrustedAIbuddyAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedAIbuddyAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedAIbuddyAgentV4Connection(
  value: unknown,
): AIbuddyAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: AIbuddyAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedAIbuddyAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: AIbuddyAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: AIbuddyAgentWorkspaceTarget): AIbuddyAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: AIbuddyAgentWorkspaceTarget;
  connection: AIbuddyAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: AIbuddyAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: AIbuddyAgentV4ConnectionContext): HelloMessage {
...
export interface AIbuddyAgentConnectionScope {
  readonly service: IAIbuddyAgentService;
...
export function createAIbuddyAgentConnectionScope(
  base: IAIbuddyAgentService,
  context: AIbuddyAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): AIbuddyAgentV4ConnectionContext => {
    const downstream = readTrustedAIbuddyAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: AIbuddyAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: AIbuddyAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: AIbuddyAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies AIbuddyAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: AIbuddyAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedAIbuddyAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedAIbuddyAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: AIbuddyAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: AIbuddyAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: AIbuddyAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IAIbuddyAgentService> = {
