...
import { repairSubagentTaskIndex } from "#src/aibuddy-agent/repairSubagentTaskIndex.js";
import {
  deriveAIbuddyTaskStatusFromSessionSnapshot,
  generateTraceId,
  getAIbuddyUserVisibleMessages,
  isAIbuddyGoalContinuationReminderText,
  isAIbuddyModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@aibuddy/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { AIbuddyWorkspaceEventSubscriptionParams } from "#src/session/aibuddyTaskListTypes.js";
import type {
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./aibuddyConfigOptions.js";

const logger = createServiceLogger("aibuddy-task-index-syncer");
...
export interface AIbuddyTaskIndexTerminalEvent {
...
export interface AIbuddyTaskIndexReadyEvent {
  target: AIbuddyAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface AIbuddyTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: AIbuddyAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，aibuddySessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: AIbuddyWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: AIbuddyWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: AIbuddyAgentSessionTarget, model: string): Promise<AIbuddyTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: AIbuddyTaskMeta | undefined,
    reason: AIbuddyWorkspaceTaskListChanged["reason"],
    options?: Pick<AIbuddyWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): AIbuddyWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): AIbuddyTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: AIbuddyAgentWorkspaceTarget,
...
export function createAIbuddyTaskIndexSyncer(
  options: CreateAIbuddyTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<AIbuddyWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<AIbuddyTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<AIbuddyTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === AIBUDDY_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<AIbuddyWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<AIbuddyWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: AIbuddyAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: AIbuddyAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 aibuddySessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: AIbuddyAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 aibuddySessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 aibuddySessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 aibuddySessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useAIbuddyConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 AIbuddyConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useAIbuddyConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: AIbuddyAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: AIbuddySessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isAIbuddyGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isAIbuddyModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: AIbuddySessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getAIbuddyUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<AIbuddyMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromAIbuddyGoal(
  goal: NonNullable<AIbuddySessionStateSnapshot["projection"]["target"]>,
): AIbuddyTaskGoal {
  return {
