import { resolveAIbuddyAgentSpawnCwd } from "#src/aibuddy-agent/aibuddyAgentSpawnCwd.js";
import type { AIbuddyAgentStorageStartupSnapshot } from "#src/aibuddy-agent/aibuddyAgent.js";
/* eslint-disable max-lines -- aibuddyAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseAIbuddyProcessDiagnostic,
  AIBUDDY_AGENT_LIFECYCLE_LOG_MARKER,
  AIBUDDY_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  AIBUDDY_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  AIBUDDY_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@aibuddy/shared/process-diagnostic";
import {
  AIBUDDY_AGENT_RUNTIME,
  AIBUDDY_AGENT_PROVIDER,
  AIBUDDY_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveAIbuddyRuntimeEnv,
  sanitizeAIbuddyRuntimeEnv,
} from "@aibuddy/shared";
import {
  findAIbuddyAgentRuntimeBinary,
  findAIbuddyAgentRuntimeNodeBundle,
...
import { AIbuddyProtocolClient } from "./aibuddyProtocolClient.js";
import { AIbuddyStdioTransport } from "./aibuddyStdioTransport.js";
import { readAIbuddyStdioTapDevState } from "./aibuddyStdioTapDevConfig.js";
import type { AIbuddyAgentPresentationSurface } from "./aibuddyAgentPresentationSurface.js";
...
export interface AIbuddyAgentCommand {
...
export interface AIbuddyAgentCommandResolverContext {
  presentationSurface?: AIbuddyAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type AIbuddyAgentCommandResolver = (
  context: AIbuddyAgentCommandResolverContext,
) => Promise<AIbuddyAgentCommand | null> | AIbuddyAgentCommand | null;

export interface AIbuddyAgentProcessManagerOptions {
  commandResolver?: AIbuddyAgentCommandResolver;
  presentationSurface?: AIbuddyAgentPresentationSurface;
...
interface ManagedAIbuddyAgentProcess {
  ...
  runtimeIdentity: AIbuddyAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.AIBUDDY_E2E_ARTIFACT_DIR?.trim();
  if (env.AIBUDDY_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "aibuddy-e2e-coverage-preload.cjs");
 ...
 */
interface AIbuddyAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: AIbuddyAgentRuntimeIdentity;
}

export interface AIbuddyAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: AIbuddyAgentRuntimeIdentity;
...
interface AIbuddyAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface AIbuddyAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("aibuddy-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("AIBUDDY_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildAIbuddyAgentSpawnPreflight(
  command: AIbuddyAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<AIbuddyAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveAIbuddyAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceAIbuddyAgentCommand(
  context: AIbuddyAgentCommandResolverContext,
): AIbuddyAgentCommand | null {
  const distEntrypoint = findUpward("apps/aibuddy-cli/packages/cli/dist/aibuddy.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.AIBUDDY_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "aibuddy.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 aibuddy-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/aibuddy-cli/packages/cli/src/main.ts");
...
function resolveDeployedAIbuddyAgentBinaryCommand(
  context: AIbuddyAgentCommandResolverContext,
): AIbuddyAgentCommand | null {
  // 旧 resolver 只识别 AIBUDDY_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 aibuddy-server.cjs 单文件部署到 ~/.aibuddy/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 aibuddy-agent 已经部署到 ~/.aibuddy/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "AIbuddy agent server command is not configured"。
  // 这里复用 findAIbuddyAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.aibuddy/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findAIbuddyAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: AIBUDDY_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeAIbuddyAgentCommand(
  context: AIbuddyAgentCommandResolverContext,
): AIbuddyAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 aibuddy-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 aibuddy.cjs，
  ...
  const bundlePath = findAIbuddyAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...AIBUDDY_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultAIbuddyAgentCommand(
  context: AIbuddyAgentCommandResolverContext,
): AIbuddyAgentCommand | null {
  const command = process.env.AIBUDDY_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.AIBUDDY_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.AIBUDDY_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 aibuddy.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceAIbuddyAgentCommand(context) ??
    resolveElectronRuntimeAIbuddyAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedAIbuddyAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: AIbuddyAgentCommand | null,
  presentationSurface: AIbuddyAgentCommandResolverContext["presentationSurface"],
): AIbuddyAgentCommand | null {
...
function wrapAIbuddyAgentCommandWithStdioTapDevProxy(
  command: AIbuddyAgentCommand,
  workspaceKey: string,
): AIbuddyAgentCommand {
  const tapState = readAIbuddyStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/aibuddy-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("AIbuddy stdio tap proxy enabled but script not found");
    return command;
  }
...
export class AIbuddyAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedAIbuddyAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedAIbuddyAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<AIbuddyProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: AIbuddyAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): AIbuddyAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: AIbuddyAgentCommandResolver;
  private readonly presentationSurface: AIbuddyAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: AIbuddyAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: AIbuddyAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<AIbuddyAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<AIbuddyAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 AIbuddyAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: AIbuddyAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultAIbuddyAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("AIbuddy agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedAIbuddyAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedAIbuddyAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("AIbuddy agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedAIbuddyAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedAIbuddyAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedAIbuddyAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("AIbuddy agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("AIbuddy agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedAIbuddyAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`AIbuddy agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<AIbuddyProtocolClient> {
    if (this.disposed) {
      throw new Error("AIbuddy agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("AIbuddy agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("AIbuddy agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): AIbuddyProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "AIbuddy agent server command is not configured. Set AIBUDDY_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("AIbuddy agent process start was cancelled.");
    }
    const effectiveCommand = wrapAIbuddyAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("AIbuddy agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("AIbuddy agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildAIbuddyAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 AIBUDDY_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 AIbuddy 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveAIbuddyRuntimeEnv(process.env);
    log("AIbuddy agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeAIbuddyRuntimeEnv(process.env),
        [AIBUDDY_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new AIbuddyStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseAIbuddyProcessDiagnostic(line);
    ...
    const client = new AIbuddyProtocolClient(transport, {
    ...
    const runtimeIdentity: AIbuddyAgentRuntimeIdentity = {
    ...
    const managed: ManagedAIbuddyAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("AIbuddy agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `AIbuddy agent process error${this.processLifecycleReporter?.onError ? ` ${AIBUDDY_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("AIbuddy agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `AIbuddy agent process exited unexpectedly${this.processLifecycleReporter ? ` ${AIBUDDY_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "AIbuddy agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("AIbuddy agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("AIbuddy agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<AIbuddyAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("AIbuddy agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "AIBUDDY_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("AIbuddy agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("AIbuddy agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，aibuddy-cli 会残留为孤儿进程。
