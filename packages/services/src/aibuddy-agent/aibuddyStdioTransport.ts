...
import { aibuddyProtocolMessageSchema } from "@aibuddy/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  AIbuddyProtocolTransport,
  AIbuddyProtocolTransportClosedEvent,
} from "./aibuddyProtocolTransport.js";
import {
...
interface AIbuddyStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("aibuddy-agent-process-tree");

export class AIbuddyStdioTransport implements AIbuddyProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<AIbuddyProtocolMessage>();
  private readonly closeEmitter = new Emitter<AIbuddyProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: AIbuddyStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // AIbuddy Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: AIbuddyProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("AIbuddy agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.AIBUDDY_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = aibuddyProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // aibuddy-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: AIbuddyProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
