...
const PLUGIN_MANIFEST_PATHS = [
  join(".aibuddy-plugin", "plugin.json"),
...
async function discoverPluginAgents(params: {
  ...
  const storageRoot = await resolveAIbuddyStorageRoot(params.storageOptions);
...
async function readPluginConfig(options?: SubagentStorageOptions): Promise<PluginConfigSummary> {
  try {
    const configPath = join(resolveUserHomeDir(options), ".aibuddy", "cli", "config.json");
...
async function readEnabledPluginRecords(
  ...
  const records = installed.filter(
    (record) =>
      ...
      !(
        record.id.endsWith(`@${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID}`) &&
  ...
  for (const cacheRoot of await scanOfficialPluginCacheRoots(pluginStorageRoot)) {
    const id = `${cacheRoot.name}@${AIBUDDY_OFFICIAL_PLUGIN_MARKETPLACE_ID}`;
...
export function createSubagentsService(options?: SubagentsServiceOptions): ISubagentsService & {
  ...
  return {
    ...
    async getPrimaryUserAgentsDirectory(_params: { provider: AIbuddyProvider }): Promise<{
      path: string;
    }> {
