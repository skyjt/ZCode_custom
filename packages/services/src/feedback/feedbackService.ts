...
import type { ApiClient, FeedbackDeviceInfo } from "@aibuddy/shared";
import {
...
function resolveApiBaseUrl(explicit?: string): string {
  return (
    explicit?.trim() ||
    process.env.AIBUDDY_FEEDBACK_API_BASE?.trim() ||
    buildRuntimeAIbuddyApiUrl(process.env, "/api/v1")
  );
}
...
function buildDeviceSnapshot(): FeedbackDeviceInfo {
  return {
    ...
    buildCommitId: AIBUDDY_COMMIT,
    buildTime: AIBUDDY_BUILD_TIME,
...
export function createFeedbackService(options: CreateFeedbackServiceOptions): IFeedbackService {
  ...
  async function getAIbuddyJwtToken(): Promise<string | undefined> {
    return (await options.credentialService.load(AIBUDDY_JWT_TOKEN_KEY))?.trim() || undefined;
  }

  async function hasAIbuddyJwtToken(): Promise<boolean> {
    return Boolean(await getAIbuddyJwtToken());
  }
  ...
  const httpClient = new FeedbackHttpClient({
    ...
    getAuthHeaders: async () => {
      ...
      const jwtToken = await getAIbuddyJwtToken();
      if (jwtToken) {
