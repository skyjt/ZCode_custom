...
import { redactFeedbackText, AIBUDDY_VERSION, AIBUDDY_COMMIT } from "@aibuddy/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "aibuddy-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "AIbuddy diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${AIBUDDY_VERSION}`,
          `commit: ${AIBUDDY_COMMIT}`,
