...
export {
  ...
  AIBUDDY_E2E_FS_FAULTS_ALLOW_ENV,
  AIBUDDY_E2E_FS_FAULTS_ENV,
...
export {
  ...
  AIBUDDY_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createAIbuddyAgentService } from "./aibuddy-agent/aibuddyAgentService.js";
export { createAIbuddyTaskServiceAdapter } from "./aibuddy-agent/aibuddyTaskServiceAdapter.js";
export { createAIbuddySessionService } from "./aibuddy-session/aibuddySessionService.js";
export {
  resolveDefaultAIbuddyAgentCommand,
  AIbuddyAgentProcessManager,
} from "./aibuddy-agent/aibuddyAgentProcessManager.js";
export type {
  AIbuddyAgentCommand,
  AIbuddyAgentCommandResolver,
  AIbuddyAgentCommandResolverContext,
  AIbuddyAgentProcessManagerOptions,
} from "./aibuddy-agent/aibuddyAgentProcessManager.js";
export { AIbuddyProtocolClient } from "./aibuddy-agent/aibuddyProtocolClient.js";
export type { AIbuddyProtocolTransport } from "./aibuddy-agent/aibuddyProtocolTransport.js";
export { AIbuddyStdioTransport } from "./aibuddy-agent/aibuddyStdioTransport.js";
export {
  getAIbuddyStdioTapDevLogDir,
  readAIbuddyStdioTapDevState,
  setAIbuddyStdioTapDevEnabled,
} from "./aibuddy-agent/aibuddyStdioTapDevConfig.js";
export type { AIbuddyStdioTapDevState } from "@aibuddy/shared";
export {
...
import { IAIbuddyTaskService } from "./session/aibuddyTaskService.js";
import { IAIbuddyAgentService } from "./aibuddy-agent/aibuddyAgent.js";
import type { CuaOperationStateReporter } from "./aibuddy-agent/cuaOperationTurnTracker.js";
import { IAIbuddySessionService } from "./aibuddy-session/aibuddySession.js";
import {
...
import { createAIbuddyAgentService } from "./aibuddy-agent/aibuddyAgentService.js";
import type { AIbuddyAgentCommandResolver } from "./aibuddy-agent/aibuddyAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./aibuddy-agent/agentTelemetryEnv.js";
import { resolveAIbuddyAgentPresentationSurface } from "./aibuddy-agent/aibuddyAgentPresentationSurface.js";
import { createAIbuddyTaskServiceAdapter } from "./aibuddy-agent/aibuddyTaskServiceAdapter.js";
import { createAIbuddySessionService } from "./aibuddy-session/aibuddySessionService.js";
import { createAIbuddyTaskIndexSyncer } from "./aibuddy-agent/aibuddyTaskIndexSyncer.js";
...
import { readLegacyAIbuddyConfigProviders } from "./model-provider/legacyAIbuddyConfigProviderReader.js";
...
import { fetchAIbuddyBuiltinRemoteRelease } from "./model-provider/aibuddyBuiltinRemoteConfig.js";
import {
  createProviderRuntimeFromConfigRuntime,
...
import { buildHelperOpenArgs, isCuaLocalDevelopmentRuntime } from "@aibuddy/aibuddy-cua/broker/server";
...
import {
  ...
  isPotentialAIbuddyCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@aibuddy/aibuddy-cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@aibuddy/aibuddy-cua/broker/socketPath";
import {
  DEFAULT_AIBUDDY_MODEL_CONTEXT_BUDGET_STRATEGY,
  resolveSafeEndpointHostname,
  AIBUDDY_JWT_INVALID_BROADCAST_CHANNEL,
  formatLogPrefix,
  ...
  type BrowserCommand,
  isAIbuddyCuaMcpCommand,
  isAIbuddyCuaMcpPackageArg,
  isAIbuddyCuaInternalFeatureEnabled,
  AIBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type AIbuddyAutomation,
  type AIbuddyAutomationRun,
  getCapturedAIbuddyAgentTelemetryEnv,
  AIBUDDY_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  ZAI_PROVIDER_ID,
  aibuddyAccountAccessSchema,
  aibuddyProviderAccountAccessSchema,
  AIBUDDY_VERSION,
  AIBUDDY_ENV,
  buildRuntimeAIbuddyApiUrl,
} from "@aibuddy/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @aibuddy/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isAIbuddyCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isAIbuddyCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialAIbuddyCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("AIbuddy Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const AIBUDDY_CUA_BUNDLED_HELPER_APP_PATH_ENV = "AIBUDDY_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[AIBUDDY_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliAIbuddyCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const home = env.HOME?.trim() || homedir();
  const configPath = join(home, ".aibuddy", "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliAIbuddyCuaServer(name, config),
  );
}

function isGlobalCliAIbuddyCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @aibuddy/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 aibuddy-cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isAIbuddyCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isAIbuddyCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 aibuddy-cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [AIBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [AIBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [AIBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace aibuddy-cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  aibuddyAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 AIbuddy Built-in Provider Config 物理路径。 */
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 AIbuddy Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 AIbuddy Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[AIBUDDY_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentAIbuddyEndpointOrigin = async () =>
    resolveRuntimeAIbuddyEndpointOrigin(process.env, {
      overrideOrigin: (await settingService.get()).aibuddyEndpointOrigin,
    });
  ...
  const aibuddyJwtLogoutHandlerRef: {
  ...
  const apiClient = createNodeApiClient({
    ...
    onAIbuddyJwtInvalid: (input, headers) => aibuddyJwtLogoutHandlerRef.current?.(input, headers),
    isAIbuddyJwtRequest: (input, headers) =>
      isCurrentOAuthCredentialRequest({ input, headers, credentialService }),
    resolveAIbuddyEndpointOrigin: resolveCurrentAIbuddyEndpointOrigin,
  });
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    aibuddyBuiltinFilePath: options.aibuddyBuiltinProviderConfigFilePath,
    aibuddyBuiltinEnvironment: {
      environmentConfigRoot: resolveAppConfigDir(),
      platform: clientConfigPlatform,
      appVersion: AIBUDDY_VERSION,
      resolveEndpointOrigin: resolveCurrentAIbuddyEndpointOrigin,
      onRefreshResult: (event) => {
        if (event.result === "updated")
          providerConfigLog.info(undefined, "AIbuddy Built-in CDN 配置已更新", event);
        else providerConfigLog.debug(undefined, "AIbuddy Built-in 刷新检查", event);
      },
      fetchRelease: (endpointOrigin, signal) =>
        fetchAIbuddyBuiltinRemoteRelease({
          apiClient,
          endpointOrigin,
          signal,
          appVersion: AIBUDDY_VERSION,
    ...
    onAIbuddyBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "AIbuddy Built-in Config 远端刷新失败", { error });
    },
    ...
    // 已发布 config.json 保存的是 AIbuddy 用户配置；清理第三方 ACP 不能移除这条升级路径。
    // Repository 仅在新 Personal 配置不存在时导入，并保留旧文件以便回滚。
    readLegacyProviders: () => readLegacyAIbuddyConfigProviders(),
  });
  ...
  let providerConnectivityAgentService:
    | Pick<IAIbuddyAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 aibuddyAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => aibuddyAgentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => aibuddyAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 aibuddyAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 aibuddyAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（AIBUDDY_CUA_DEV_MODE=1 或
    // AIBUDDY_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isAIbuddyCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isAIbuddyCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，aibuddyAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@aibuddy/aibuddy-cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // AIBUDDY_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 AIBUDDY_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __AIBUDDY_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.AIBUDDY_HOME?.trim() || join(homedir(), ".aibuddy");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（AIbuddy 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "AIbuddy Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "AIbuddy Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@aibuddy/aibuddy-cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "AIbuddy Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "AIbuddy Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart AIbuddy Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const aibuddyAgentService = createAIbuddyAgentService({
    ...
    commandResolver: options?.aibuddyAgentCommandResolver,
    presentationSurface: resolveAIbuddyAgentPresentationSurface({
    ...
    // AIbuddyAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.aibuddyAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @aibuddy/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentAIbuddyEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveAIbuddyApiOrigin: resolveCurrentAIbuddyEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // AIbuddy 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [AIBUDDY_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedAIbuddyAgentTelemetryEnv();
      ...
      return {
        ...
        ...buildAgentEndpointOriginEnv(await resolveCurrentAIbuddyEndpointOrigin()),
        // broker 凭据（socket/token）注入 agent spawn env，让内置 aibuddy-cua plugin 的
        // computer-use MCP server 经 __aibuddy-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          aibuddyBuiltinFilePath: await providerConfigRuntime.resolveAIbuddyBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_AIBUDDY_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = aibuddyAgentService;
  ...
  hasActiveTurnRef = () => aibuddyAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 aibuddySessionService，绕开 AIbuddy task adapter 的
  ...
  const aibuddyTaskIndexSyncer = createAIbuddyTaskIndexSyncer({
    agentService: aibuddyAgentService,
    taskIndexRepo,
  });
  ...
  const aibuddySessionService = createAIbuddySessionService({
    agentService: aibuddyAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await aibuddyAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 AIbuddy task service adapter 提供；核心 session 状态由 AIbuddy agent server 维护。
  const aibuddyTaskService = createAIbuddyTaskServiceAdapter({
    aibuddyAgentService,
    taskIndexRepo,
    taskIndexSyncer: aibuddyTaskIndexSyncer,
    settingService,
  ...
  const aibuddyJwtLogoutLogger = createServiceLogger("aibuddy-jwt-logout");
  aibuddyJwtLogoutHandlerRef.current = (input, headers) => {
      ...
      .then((invalidated) => {
        ...
        if (invalidated) {
          void broadcastService.send({
            channel: AIBUDDY_JWT_INVALID_BROADCAST_CHANNEL,
            payload: {},
          });
        }
      })
      .catch((error) => {
        aibuddyJwtLogoutLogger.warn("AIbuddy JWT logout failed", { error });
      });
  };
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = aibuddyProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: aibuddyAccountAccessSchema.parse(access),
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    apiClient,
    baseUrl: buildRuntimeAIbuddyApiUrl(process.env, "/api/v1"),
    tokenProvider: async (): Promise<string | null> => {
      ...
      return tokenSet?.aibuddyJwtToken ?? tokenSet?.accessToken ?? null;
    },
  });
  ...
  const services = new ServiceCollection()
    ...
    .register(IAIbuddyTaskService, aibuddyTaskService)
    .register(IAIbuddyAgentService, aibuddyAgentService)
    .register(IAIbuddySessionService, aibuddySessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentAIbuddyEndpointOrigin(),
          appVersion: AIBUDDY_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          stopRunningTask: async (params) => {
            await aibuddyTaskService.stopGeneration({
        ...
        // 回写前向引用，供 aibuddyAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ aibuddyAgentService }))
...
/** 仅给同一事件账号返回当前 AIbuddy JWT；不缓存、不修改登录凭据。 */
...
): () => Promise<import("@aibuddy/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(IAIbuddyTaskService),
    services.getOptional(IAIbuddyAgentService),
    services.getOptional(IAIbuddySessionService),
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 aibuddy-cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(ITerminalService),
    services.getOptional(IAIbuddyTaskService),
    services.getOptional(IAIbuddyAgentService),
