...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@aibuddy/shared";

let _dataBaseDir: string | null = null;
export const AIBUDDY_WINDOWS_APP_INSTALL_DIR_ENV = "AIBUDDY_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.AIBUDDY_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env AIBUDDY_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.aibuddy */
export function getAIbuddyDataRootDir(): string {
  return join(getDataBaseDir(), ".aibuddy");
}

/** 非项目对话共享的真实工作目录；默认 ~/.aibuddy/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getAIbuddyDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.aibuddy/v2 */
export function getAppConfigDir(): string {
  return join(getAIbuddyDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, AIBUDDY_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "AIbuddy") : null,
    programFilesX86 ? win32.join(programFilesX86, "AIbuddy") : null,
    programW6432 ? win32.join(programW6432, "AIbuddy") : null,
    localAppData ? win32.join(localAppData, "Programs", "AIbuddy") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getAIbuddyDataRootDir(), "export-log-stage");
}

export function getExportLogDir(): string {
  return join(getAIbuddyDataRootDir(), "export-log");
}

export function getFeedbackRootDir(): string {
  return join(getAIbuddyDataRootDir(), "feedback");
}
...
export function getGitCheckpointIndexRootDir(): string {
  return join(getAIbuddyDataRootDir(), "git-checkpoint-index");
}

/** ~/.aibuddy/v2/tasks-index.sqlite */
...
/** 与 AIbuddy session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.aibuddy/v2/sessions/{workspaceHash} */
...
/** ~/.aibuddy/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.aibuddy/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .aibuddy/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".aibuddy", "v2");
  const newDir = join(newBaseDir, ".aibuddy", "v2");
