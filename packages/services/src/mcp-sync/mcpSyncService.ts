...
const AIBUDDY_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "aibuddy",
  directorySource: "aibuddy",
  userConfigDirSegments: [".aibuddy", "cli"],
  workspaceConfigDirSegments: [".aibuddy"],
...
export function createMcpSyncService(
  ...
  return {
    ...
    async checkRemoteUserMcpWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(dirname(getUserAIbuddyMcpConfigPath()));
    },
...
function getUserAIbuddyMcpConfigPath(): string {
  return buildUserConfigPath(AIBUDDY_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const aibuddyRecords = await readUserMcpRecordsFromFile(AIBUDDY_MCP_DESCRIPTOR);
  if (aibuddyRecords.length > 0) {
    return sortMcpRecords(aibuddyRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(AIBUDDY_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeAIbuddyServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const aibuddyServers = await readDirectoryServersFromFile(
    AIBUDDY_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (aibuddyServers.length > 0) {
    return aibuddyServers;
  }
...
async function writeAIbuddyServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(AIBUDDY_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, AIBUDDY_MCP_DESCRIPTOR.configKeyName, servers);
  await writeTextAtomic(filePath, `${JSON.stringify(next, null, 2)}\n`);
}

async function readUserCliConfig(): Promise<Record<string, unknown>> {
  return (await readJsonObject(getUserAIbuddyMcpConfigPath())) ?? {};
}

async function writeUserCliConfig(config: Record<string, unknown>): Promise<void> {
  await writeTextAtomic(getUserAIbuddyMcpConfigPath(), `${JSON.stringify(config, null, 2)}\n`);
}
...
async function importMcpServers(params: {
  ...
  const targetPath = getUserAIbuddyMcpConfigPath();
  const current = (await readJsonObject(targetPath)) ?? {};
  const targetServers = readServerMapFromJson(current, AIBUDDY_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, AIBUDDY_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
