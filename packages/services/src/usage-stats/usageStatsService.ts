...
import type { OfficialMcpCredentialSource } from "./providers/aibuddyMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 AIbuddy Protocol 读取 agent 数据库真实统计。 */
  aibuddyAgentService: Pick<IAIbuddyAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 AIbuddy Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.aibuddyAgentService.getAppUsageStats({
        range: request.range,
