import { BIGMODEL_PROVIDER_ID, resolveBigModelApiOrigin, ZAI_PROVIDER_ID } from "@aibuddy/shared";
import { ZAI_API_HOST } from "../providers/api/apiEndpoints.js";
import {
  DEFAULT_ORG_NAME,
  DEFAULT_PROJECT_NAME,
  AIBUDDY_API_KEY_NAME,
...
export class AccountProviderApiKeyResolver {
  ...
  private async resolveBizApiKey(
    ...
    let apiKeyEntry = apiKeys.find((item) => item.name === AIBUDDY_API_KEY_NAME) ?? null;

    if (!apiKeyEntry) {
      apiKeyEntry = await this.fetchRemoteData<RemoteApiKeySummary>(listUrl, {
        method: "POST",
        headers: createBizAuthHeaders(authorization),
        body: JSON.stringify({ name: AIBUDDY_API_KEY_NAME }),
      });
    }
