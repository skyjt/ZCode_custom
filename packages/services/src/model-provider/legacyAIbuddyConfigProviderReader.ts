/* oxlint-disable eslint(max-lines) -- 已发布旧 AIbuddy config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeAIbuddyEnv(env) === "production"
  ...
  if (resolveRuntimeAIbuddyEnv(env) === "production") {
...
function getAIbuddyConfigFilePath(): string {
...
interface AIbuddyConfigFile {
  $schema?: string;
  provider?: Record<string, AIbuddyOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface AIbuddyOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, AIbuddyOpenCodeModelConfig>;
  aibuddy?: AIbuddyProviderConfigExtension;
  [key: string]: unknown;
}

interface AIbuddyOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | AIbuddyReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  aibuddy?: AIbuddyModelConfigExtension;
  [key: string]: unknown;
}

interface AIbuddyReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface AIbuddyProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface AIbuddyModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const aibuddyModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const aibuddyProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const aibuddyReasoningConfigExtensionSchema = z
  .object({
...
const aibuddyModelConfigExtensionSchema = z
  .object({
...
const aibuddyOpenCodeModelConfigSchema: z.ZodType<AIbuddyOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), aibuddyReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(aibuddyModelProviderModalitySchema).optional(),
        output: z.array(aibuddyModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    aibuddy: aibuddyModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const aibuddyProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: aibuddyProviderModelMappingsSchema.optional(),
...
const aibuddyOpenCodeProviderConfigSchema: z.ZodType<AIbuddyOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), aibuddyOpenCodeModelConfigSchema).optional(),
    aibuddy: aibuddyProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const aibuddyConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), aibuddyOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: AIbuddyOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: AIbuddyOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/aibuddy 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.aibuddy?.endpoints ?? {};
}

function stripAIbuddyPrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("aibuddy"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: AIbuddyOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.aibuddy?.reasoning) {
    return stripModelProviderReasoningPatches(model.aibuddy.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const aibuddy = options.preferOpenCodeFields ? undefined : model.aibuddy;
  const aibuddyExtensions = model.aibuddy;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(aibuddyExtensions?.priority),
    disabledReason: model.disabledReason ?? aibuddy?.disabledReason,
    supportsTools: model.supportsTools ?? aibuddy?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? aibuddy?.supportsStructuredOutput,
    modified: model.modified ?? aibuddyExtensions?.modified,
    deleted: model.deleted ?? aibuddyExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  const aibuddy = provider.aibuddy;
  ...
  for (const deletedModelId of normalizeDeletedModelIds(aibuddy?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? aibuddy?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripAIbuddyPrefixedProviderMappings(provider.providerMappings ?? aibuddy?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : aibuddy?.enabled !== undefined
        ? { enabled: aibuddy.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? aibuddy?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? aibuddy?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? aibuddy?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? aibuddy?.source)
      ? { source: provider.source ?? aibuddy?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? aibuddy?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? aibuddy?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? aibuddy?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? aibuddy?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? aibuddy?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? aibuddy?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? aibuddy?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? aibuddy?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? aibuddy?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? aibuddy?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? aibuddy?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? aibuddy?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? aibuddy?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? aibuddy?.updatedAt) : undefined) ?? now,
  });
}

async function readRawAIbuddyConfigFile(): Promise<AIbuddyConfigFile | null> {
  const filePath = getAIbuddyConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = aibuddyConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readAIbuddyConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawAIbuddyConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripAIbuddyPrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 AIbuddy Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyAIbuddyConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readAIbuddyConfigProviders();
