...
import { resolveBigModelStartPlanAIbuddyJwt } from "./bigmodelStartPlanAIbuddyJwt.js";
import {
...
function buildZaiSubscriptionListUrl(): string {
  ...
  // availability 校验必须和 OAuth business login 共用同一套 AIBUDDY_ENV 域名分流，只允许 origin 不同。
...
async function validateSelectedTeamPlanAvailability(
  ...
  const aibuddyJwtToken = (await context.credentialService?.load(AIBUDDY_JWT_TOKEN_KEY))?.trim();
...
async function resolveStartPlanAuthorization(
  ...
  if (provider.family === "bigmodel") {
    const aibuddyJwtToken = await resolveBigModelStartPlanAIbuddyJwt({
      credentialService: context.credentialService,
      provider,
    });
    return {
      value: aibuddyJwtToken ? `Bearer ${aibuddyJwtToken}` : "",
      missingReason: "coding_plan_not_authenticated",
    };
  }

  const credentialJwt = await loadZaiProviderConnectionAIbuddyJwtToken(context);
...
async function loadZaiProviderConnectionAIbuddyJwtToken(
  context: CodingPlanAvailabilityContext,
): Promise<string> {
  return (await context.credentialService?.load(AIBUDDY_JWT_TOKEN_KEY))?.trim() || "";
}
...
function hasActiveStartPlan(plans: ZaiStartPlanPlan[] | undefined): boolean {
  return Boolean(
    plans?.some((plan) => {
      ...
      // billing/balance 的 plans 真实返回的是 `plan_id=aibuddy-v3-start-plan`
      // 和 `name=AIbuddy V3 Start Plan`；只认 `name === "start plan"` 的话，
