import { formatLogPrefix, type TraceId } from "@aibuddy/shared";
...
export function createServiceLogger(
  ...
  function write(
    ...
    // 服务层日志过去常被复用到 AIbuddy Agent 命名 logger，导致新 AIbuddy 路径继续依赖 AIbuddy Agent 目录。
    // 这里把通用分级日志抽到独立模块，后续删除 AIbuddy Agent runtime 时不会牵连非 AIbuddy Agent 服务。
