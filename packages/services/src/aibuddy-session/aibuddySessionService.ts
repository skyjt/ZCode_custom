/* eslint-disable max-lines -- aibuddySessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IAIbuddyAgentService } from "#src/aibuddy-agent/aibuddyAgent.js";
import type { AIbuddyTaskIndexSyncer } from "#src/aibuddy-agent/aibuddyTaskIndexSyncer.js";
...
import type {
  ...
  AIbuddySessionEventsParams,
  AIbuddySessionListParams,
  AIbuddySessionMessagesParams,
  AIbuddySessionReadParams,
  AIbuddySessionReadWorkspacePresentationParams,
  AIbuddySessionResumeParams,
  AIbuddySessionSetModeParams,
  AIbuddySessionSetModelParams,
  AIbuddySessionSetThoughtLevelParams,
  AIbuddyTaskTarget,
  AIbuddySessionWorkspaceTarget,
} from "#src/aibuddy-session/aibuddySession.js";
import { formatModelPickerValue } from "#src/aibuddy-agent/aibuddyConfigOptions.js";
import { createAIbuddySessionApiRetryRuntimeTracker } from "#src/aibuddy-session/aibuddySessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/aibuddy-session/importedClaudeSessionRepair.js";
import { createAIbuddyDeferredDraftRegistry } from "#src/aibuddy-session/aibuddySessionDraftRegistry.js";
...
interface CreateAIbuddySessionServiceOptions {
...
export function createAIbuddySessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateAIbuddySessionServiceOptions): IAIbuddySessionService {
  const { withApiRetryRuntime } = createAIbuddySessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createAIbuddyDeferredDraftRegistry();

  function notifySyncer(
    target: AIbuddyTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: AIbuddySessionStateSnapshot) {
  ...
  // aibuddySessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[aibuddy-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: AIbuddySessionStateSnapshot,
    params: AIbuddySessionResumeParams | AIbuddySessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends AIbuddySessionCreateParams | AIbuddySessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: AIbuddySessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: AIbuddySessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: AIbuddySessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: AIbuddySessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[aibuddy-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[aibuddy-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 AIbuddy task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[aibuddy-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: AIbuddySessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[aibuddy-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[aibuddy-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[aibuddy-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: AIbuddySessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: AIbuddySessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[aibuddy-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: AIbuddySessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: AIbuddySessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: AIbuddyTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[aibuddy-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: AIbuddyTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: AIbuddyTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[aibuddy-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: AIbuddySessionSetModelParams) {
    ...
    async setThoughtLevel(params: AIbuddySessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: AIbuddySessionSetModeParams) {
