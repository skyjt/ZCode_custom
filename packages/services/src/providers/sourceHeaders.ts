...
import {
  ...
  normalizeAIbuddySourceHeaderValue,
  AIBUDDY_ENV,
...
export { AIBUDDY_SOURCE_HEADERS };

interface AIbuddySourceHeaderOptions {
  appVersion?: string;
  arch?: string;
...
function normalizePrintableHeaderValue(value: string | undefined): string | undefined {
  return normalizeAIbuddySourceHeaderValue(value);
}
...
export function buildAIbuddySourceHeaders(
  options: AIbuddySourceHeaderOptions = {},
): Record<string, string> {
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  const appVersion = normalizePrintableHeaderValue(options.appVersion ?? AIBUDDY_VERSION);
  const releaseChannel = normalizePrintableHeaderValue(options.releaseChannel ?? AIBUDDY_ENV);
  ...
  return buildAIbuddySourceHeadersFromContext({
    appVersion,
    arch,
    clientLanguage,
    clientTimezone,
