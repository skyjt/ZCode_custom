import type {
  ...
  AIbuddyOffPeakTask,
  AIbuddyOffPeakTaskCreateParams,
  ModelSelection,
} from "@aibuddy/shared";
...
export interface IOffPeakTaskService {
  ...
  createTask(params: AIbuddyOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<AIbuddyOffPeakTask | null>;
  list(): Promise<AIbuddyOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null>;
}
