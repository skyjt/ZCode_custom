import type {
  AIbuddyAutomation,
  AIbuddyAutomationCreateParams,
  AIbuddyAutomationRun,
  AIbuddyAutomationScheduleRule,
  AIbuddyAutomationUpdateParams,
} from "@aibuddy/shared";
...
function assertValidAutomationScheduleRule(rule: AIbuddyAutomationScheduleRule): void {
...
export class AutomationService {
  ...
  async create(params: AIbuddyAutomationCreateParams): Promise<AIbuddyAutomation> {
  ...
  async update(
    ...
    // 同时顺手修复历史上已经存在的同类脏数据。
    const normalizedParams: AIbuddyAutomationUpdateParams = hasIntervalCarrier
    ...
    const options: {
      ...
      lifecycleStatus?: AIbuddyAutomation["lifecycleStatus"];
      resetRetry?: boolean;
    } = {};
    ...
    const effectiveDirectScheduleRule = hasDirectScheduleRule
      ? directScheduleRule
      : carrierScheduleRule;
    const normalizedWithSchedule: AIbuddyAutomationUpdateParams =
  ...
  async runNow(
    automationId: string,
    scope?: AutomationWorkspaceScope,
  ): Promise<{ automation: AIbuddyAutomation; run: AIbuddyAutomationRun } | null> {
    const workspaceKey = resolveScopeKey(scope);
    const existing = await this.repo.get(automationId, workspaceKey);
    if (!existing) return null;
