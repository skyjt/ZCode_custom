...
import type {
  ...
  AIbuddyTaskListItem,
} from "#src/session/aibuddyTaskListTypes.js";
import type {
  AIbuddyGroupedTaskRef,
  AIbuddyGroupedTaskView,
  AIbuddyGroupedTaskViewNode,
  AIbuddyGroupedTaskViewOrderInput,
  AIbuddyGroupedTaskViewQuery,
  AIbuddyGroupedTaskViewStructure,
  AIbuddyGroupedTaskViewStructureMember,
  AIbuddyGroupedTaskViewStructureTopOrder,
  AIbuddyGroupedTaskViewTopLevelNodeRef,
  AIbuddyTaskGroup,
  AIbuddyTaskGroupColor,
} from "#src/session/aibuddyTaskListTypes.js";
...
function appendAIbuddyAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: AIbuddyTaskMeta["status"];
  lastError?: AIbuddyTaskMeta["lastError"];
  target?: AIbuddyTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__aibuddy_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: AIbuddyTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies AIbuddyTaskGroupColor[];
...
function shouldPreserveNewerTerminalStatus(
  existingMeta: AIbuddyTaskMeta | null,
  incomingMeta: AIbuddyTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): AIbuddyTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = aibuddyTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as AIbuddyTaskMeta),
  ...
  return {
    ...
    mode: row.mode as AIbuddyTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === AIBUDDY_AGENT_PROVIDER ? AIBUDDY_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as AIbuddyTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as AIbuddyTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: AIbuddyTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@aibuddy/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): AIbuddyTaskListItem {
...
function isTaskGroupColor(value: string): value is AIbuddyTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): AIbuddyTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): AIbuddyTaskGroupColor {
...
function groupedTopNodeOrderRef(node: AIbuddyGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareCronGroupTasks(left: AIbuddyTaskListItem, right: AIbuddyTaskListItem): number {
...
function compareGroupTasks(
  left: AIbuddyTaskListItem,
  right: AIbuddyTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendAIbuddyAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): AIbuddyTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: AIbuddyTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: AIbuddyTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 AIbuddy session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 AIbuddy 任务。
   ...
   */
  private ensureCronGroupMembership(meta: AIbuddyTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<AIbuddyTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: AIbuddyTaskMeta): Promise<AIbuddyTaskMeta> {
    await this.ensureReady();
  ...
  async clearTaskUnreadIfMatches(params: {
  ...
  }): Promise<{ meta: AIbuddyTaskMeta; cleared: boolean }> {
    await this.ensureReady();
    return this.enqueueWrite(params, () => {
      ...
      try {
        ...
        const nextMeta: AIbuddyTaskMeta = {
          ...current,
          unreadAt: undefined,
        };
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<AIbuddyTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: AIbuddyTaskListQuery & { provider?: AIbuddyProvider },
  ): Promise<AIbuddyTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<AIbuddyTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: AIbuddyGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: AIbuddyGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: AIbuddyGroupedTaskViewQuery & { provider?: AIbuddyProvider },
  ): Promise<AIbuddyGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 AIbuddy Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendAIbuddyAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
    ...
    const nodes: AIbuddyGroupedTaskViewNode[] = groups.map((group) => {
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<AIbuddyGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: AIbuddyGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: AIbuddyGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: AIbuddyGroupedTaskViewOrderInput & { provider?: AIbuddyProvider },
  ): Promise<AIbuddyGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: AIbuddyGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 AIbuddy Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: AIbuddyGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: AIbuddyGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: AIbuddyGroupedTaskRef[] = [];
