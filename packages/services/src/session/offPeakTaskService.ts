...
import {
  ...
  type AIbuddyOffPeakTaskCreateParams,
} from "@aibuddy/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@aibuddy/provider";
...
function isValidCreateParams(params: AIbuddyOffPeakTaskCreateParams): boolean {
  return (
...
> {
  ...
  if (failureStage === "client_validation") {
    ...
    errorCode = error.biaibuddy === undefined ? "" : String(error.biaibuddy);
    if (error.biaibuddy === 3101) errorCategory = "eligibility_3101";
    else if (error.biaibuddy === 3103) errorCategory = "quota_3103";
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: AIbuddyOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: AIbuddyOffPeakTaskCreateParams = {
      ...params,
    ...
    let created: AIbuddyOffPeakTask;
    try {
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null> {
  ...
  ): Promise<AIbuddyOffPeakTask | null> {
    const existing = await this.deps.repo.get(offPeakTaskId);
  ...
  async list(): Promise<AIbuddyOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<AIbuddyOffPeakTask | null> {
  ...
  ): Promise<AIbuddyOffPeakTask[]> {
    const repaired: AIbuddyOffPeakTask[] = [];
  ...
  private async settleOne(task: AIbuddyOffPeakTask): Promise<void> {
