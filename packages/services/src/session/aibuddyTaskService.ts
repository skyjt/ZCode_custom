/* eslint-disable max-lines -- AIbuddy task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@aibuddy/rpc";
import { ServiceChannels } from "@aibuddy/shared";
import type { CommandPayloadMap } from "@aibuddy/shared/aibuddy-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  AIbuddyImportSessionsResult,
  AIbuddyImportableSessionCandidate,
  AIbuddySessionCompactResult,
  AIbuddySessionGoalAction,
  AIbuddySessionGoalResult,
  AIbuddyTaskCreateResult,
  AIbuddyTaskMeta,
  AIbuddyStreamEvent,
  AIbuddyTaskMode,
  AIbuddyConfigOption,
  AIbuddyError,
  AIbuddyAssistantMessageFeedback,
  AIbuddyPromptAttachment,
  AIbuddyProvider,
  AIbuddyWorkspaceEvent,
  TraceId,
  AIbuddySessionFile,
  AIbuddyAgentMcpServer,
  AIbuddyTaskSnapshot,
  AIbuddyTaskSnapshotBody,
  AIbuddyTaskSnapshotToolCallsSlice,
  AIbuddyTaskSnapshotRefContent,
  AIbuddyEnqueueTaskCommandResult,
  AIbuddyCancelTaskCommandResult,
  AIbuddyTaskClientMode,
  AIbuddyTaskTokenUsageResult,
...
import type {
  ...
  AIbuddyTaskListSortBy,
  AIbuddyTaskListWorkspaceScope,
...
export interface AIbuddyTaskSnapshotWithEtagResult {
  snapshot: AIbuddyTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface AIbuddyArchivedTaskDeletionResult {
...
export type AIbuddyModelTrajectoryContentPart =
...
export interface AIbuddyModelTrajectoryMessage {
  role: string;
  parts: AIbuddyModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface AIbuddyModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type AIbuddyModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface AIbuddyModelTrajectoryCallSource {
  kind: AIbuddyModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface AIbuddyModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: AIbuddyModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: AIbuddyModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: AIbuddyModelTrajectoryContentPart[];
    usage?: AIbuddyModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface AIbuddyModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 AIbuddy Agent 落盘 model-io）。 */
  available: boolean;
  records: AIbuddyModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface AIbuddyTaskTerminalOutcome {
  taskId: string;
...
export interface AIbuddyTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IAIbuddyTaskService — AIbuddy task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * AIbuddy Agent server 维护，新功能应优先走 IAIbuddySessionService。
 */
export interface IAIbuddyTaskService {
  // ---- 生命周期 ----
  ...
  /** 创建 AIbuddy session 并同步 task 索引。 */
  createTask(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    provider?: AIbuddyProvider;
  ...
  }): Promise<AIbuddyTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<AIbuddyEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: AIbuddyTaskClientMode;
  }): Promise<AIbuddyCancelTaskCommandResult>;
  ...
  }): Promise<AIbuddySessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<AIbuddySessionGoalResult>;
   ...
   */
  listTaskList(params: AIbuddyTaskListQuery): Promise<AIbuddyTaskListResult>;
  ...
  applyGroupedTaskViewOrder(params: AIbuddyGroupedTaskViewOrderInput): Promise<AIbuddyGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<AIbuddyTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<AIbuddyTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<AIbuddyTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<AIbuddyTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<AIbuddyTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<AIbuddyConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: AIbuddyAssistantMessageFeedback | null;
  }): Promise<AIbuddySessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: AIbuddyTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.aibuddy/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 AIbuddy Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<AIbuddyModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<AIbuddyTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 AIbuddy Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  onDynamicWorkspaceEvent(
    workspace: string | AIbuddyWorkspaceEventSubscriptionParams,
  ): Event<AIbuddyWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<AIbuddyError>;
}

export const IAIbuddyTaskService = createServiceDescriptor<IAIbuddyTaskService>(
  ServiceChannels.AIbuddyTask,
);
