...
export function buildSearchableTextFromMessages(messages: AIbuddySessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: AIbuddyTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.aibuddy/v2/sessions/{hash}/{taskId}.json。现在真实 AIbuddy session 承担续聊，legacy snapshot
