...
export function createSkillSyncService(options?: { maxArchiveBytes?: number }): ISkillSyncService {
  ...
  return {
    ...
    async listRemoteUserSkillStatuses(params): Promise<SkillSyncRemoteStatusResult> {
      const root = getUserAIbuddySkillRoot();
    ...
    async checkRemoteUserSkillWriteAccess() {
      return checkRemoteSyncDirectoryWriteAccess(getUserAIbuddySkillRoot());
    },
...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const userAIbuddySkillRoot = getUserAIbuddySkillRoot();
  const rawAIbuddyCandidates = await collectUserSkillCandidatesInRoot(userAIbuddySkillRoot);
  const aibuddyDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userAIbuddySkillRoot);
  const aibuddySkillNameKeys = new Set(
    rawAIbuddyCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const aibuddyCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawAIbuddyCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !aibuddyDirectoryNames.has(candidate.directoryName) &&
        !aibuddySkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...aibuddyCandidates, ...agentsCandidates].sort((left, right) =>
...
async function collectUserSkillDirectoryPathByName(): Promise<Map<string, string>> {
  const result = await collectSkillDirectoryPathByName(getUserAIbuddySkillRoot());
...
async function importArchive(
  ...
  const targetRoot = getUserAIbuddySkillRoot();
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .aibuddy/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .aibuddy 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
