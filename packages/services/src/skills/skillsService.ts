...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".aibuddy", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".aibuddy", "cli");
...
/** 对齐 apps/aibuddy-cli/packages/adapters/src/skills/index.ts:19 */
...
async function isUserAgentsSkillCoveredByAIbuddy(params: {
  skillPath: string;
  rootPath: string;
  userAIbuddySkillNameKeys: Set<string>;
}): Promise<boolean> {
  const { skillPath, rootPath, userAIbuddySkillNameKeys } = params;
  if (rootPath !== getUserAgentsSkillRoot()) {
    return false;
  }
  if (await exists(join(getUserAIbuddySkillRoot(), basename(dirname(skillPath)), SKILL_FILE_NAME))) {
    return true;
  }
  return userAIbuddySkillNameKeys.has(await readSkillNameKey(skillPath));
}

/**
 * 从 workspacePath 向上走到 worktree 根（含 .git 标记），把每一层的
 * `.aibuddy/skills` 与 `.agents/skills` 都收集起来。
 * 对齐 apps/aibuddy-cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 `.aibuddy` 的 fallback，导致同层 `.aibuddy` 只要有一个技能，`/`、`$` 和设置页
    // 就会整根漏掉 `.agents` 技能，形成“模型可执行但 UI 无法引用”的发现语义分裂。
    roots.push(getWorkspaceAIbuddySkillRoot(dir));
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，`.aibuddy/skills` 里只要存在一个技能就截断
    // `.agents/skills` 会导致外部 Agent 的全局技能在导入后从设置页消失。
    roots.push({
      scope: "user" as const,
      rootPath: getUserAIbuddySkillRoot(),
    });
    roots.push({
      scope: "user" as const,
  ...
  const userAIbuddySkillNameKeys = params.includeUserSkills
    ? await collectSkillNameKeysInRoot(getUserAIbuddySkillRoot())
    : new Set<string>();
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      if (
        await isUserAgentsSkillCoveredByAIbuddy({
          skillPath,
          rootPath: root.rootPath,
          userAIbuddySkillNameKeys,
        })
      ) {
        continue;
      }
      ...
      // 这些字段不影响 AIbuddy 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.AIBUDDY_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async copyToCommon(params: {
      ...
      const commonRoot =
        skill.scope === "workspace"
          ? getWorkspaceAIbuddySkillRoot(params.workspacePath)
    ...
    async removeFromCommon(params: {
      ...
      const userCommonRoot = getUserAIbuddySkillRoot().replaceAll("\\", "/").toLowerCase();
      const workspaceCommonRoot = getWorkspaceAIbuddySkillRoot(params.workspacePath)
        .replaceAll("\\", "/")
        .toLowerCase();
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.aibuddy/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 .aibuddy/skills 与 .agents/skills，外加用户级两根。
      const allowedRootCandidates = await resolveAncestorWorkspaceRoots(params.workspacePath);
      allowedRootCandidates.push(getUserAIbuddySkillRoot());
