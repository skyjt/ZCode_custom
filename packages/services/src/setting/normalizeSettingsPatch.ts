...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "aibuddyEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.aibuddyEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedAIbuddyEndpointOrigin = normalizedPatch.aibuddyEndpointOrigin.trim();
    normalizedPatch.aibuddyEndpointOrigin =
      trimmedAIbuddyEndpointOrigin.length > 0 ? trimmedAIbuddyEndpointOrigin : undefined;
  }
