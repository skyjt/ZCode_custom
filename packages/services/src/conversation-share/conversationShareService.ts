...
import {
  ...
  readAIbuddyAttachmentFaultCode,
...
import type { IAIbuddySessionService } from "#src/aibuddy-session/aibuddySession.js";
...
interface ConversationShareServiceOptions {
  aibuddyAgentService: ConversationShareAgentService;
  ...
  aibuddySessionService?: Pick<IAIbuddySessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^aibuddy-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 aibuddy-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readAIbuddyAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readAIbuddyAttachmentFaultCode(error);
  return (
    faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === AIBUDDY_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly aibuddyAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly aibuddySessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.aibuddyAgentService = options.aibuddyAgentService;
    ...
    this.aibuddySessionService = options.aibuddySessionService;
    ...
    // 配置的 AIbuddy origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeAIbuddyApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > AIBUDDY_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.AIBUDDY_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeAIbuddyEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".aibuddy-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.aibuddyAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".aibuddy-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".aibuddy-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.aibuddySessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.aibuddy-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.aibuddySessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .aibuddy-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".aibuddy-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.aibuddyAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.aibuddyAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      aibuddyAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.aibuddySessionService.listSessions({
