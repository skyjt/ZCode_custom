...
import {
  ...
  buildAIbuddySourceHeadersFromContext,
...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["aibuddy:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".aibuddy", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".aibuddy", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? AIBUDDY_VERSION;
  ...
  async function sendReport(
    ...
    if (!AIBUDDY_TELEMETRY_ENABLED || !AIBUDDY_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteAIbuddyEndpointUrl(
        AIBUDDY_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveAIbuddyEndpointOrigin?.()) ?? AIBUDDY_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildAIbuddySourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? AIBUDDY_ENV,
