...
interface StageOptions {
  ...
  distDir: string;
  /** 既有 CLI/Agent bundle（aibuddy.cjs，自包含 CJS） */
  ...
  /** 未被 pnpm 链接到 node_modules 的 workspace 包（例如 @aibuddy/tui）。 */
...
# 由 aibuddy-server staging 生成：定位发行根后用随包 Node 启动 Server CLI。
...
/** 组装 `aibuddy-server-<os>-<arch>/` 发行目录；只做本地组装，不上传、不发布。 */
export async function stageRelease(options: StageOptions): Promise<StagedRelease> {
  ...
  const releaseName = `aibuddy-server-${options.target}`;
  ...
  await cp(options.agentBundlePath, join(runtimeDir, "aibuddy.cjs"), { dereference: true });
  ...
  const launcherPath = join(
    releaseDir,
    "bin",
    options.target.startsWith("win32-") ? "aibuddy.cmd" : "aibuddy",
  );
  ...
  const componentSpecs = [
    ...
    { id: "agent-runtime", paths: ["runtime/aibuddy.cjs", "runtime/licenses/agent"] },
    ...(plugins.length > 0
      ? [
          {
            id: "official-plugins",
