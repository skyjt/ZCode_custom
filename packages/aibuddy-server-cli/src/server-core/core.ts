...
import { IAIbuddyAgentService } from "@aibuddy/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitAIbuddyBuiltinProviderConfigFilePath =
    process.env[AIBUDDY_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const aibuddyBuiltinProviderConfigFilePath = explicitAIbuddyBuiltinProviderConfigFilePath
    ? explicitAIbuddyBuiltinProviderConfigFilePath
    : typeof __AIBUDDY_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeAIbuddyBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __AIBUDDY_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!aibuddyBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 AIbuddy Built-in Provider Config，且未设置 ${AIBUDDY_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IAIbuddyAgentService));
