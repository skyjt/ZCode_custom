...
import { createServiceLogger } from "@aibuddy/services/node";
import {
...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IAIbuddyAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "aibuddy-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(AIBUDDY_RPC_HOST_CAPABILITY_HEADER);
