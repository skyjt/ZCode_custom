...
function getDefaultServerDataRoot(): string {
  const configured = process.env.AIBUDDY_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".aibuddy", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\aibuddy-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".aibuddy") {
