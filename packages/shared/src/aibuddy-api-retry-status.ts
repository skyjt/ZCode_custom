import type { AIbuddyApiRetryStatus } from "./aibuddy-task-types-core.js";

export function normalizeAIbuddyApiRetryStatus(
  value: unknown,
...
export function aibuddyApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): AIbuddyApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeAIbuddyApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = aibuddyApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isAIbuddyModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function aibuddyApiRetryFromStreamRecoveryPayload(
  value: unknown,
): AIbuddyApiRetryStatus | undefined {
  const record = asRecord(value);
