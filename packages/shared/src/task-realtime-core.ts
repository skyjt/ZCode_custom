...
import { z } from "zod";
import type { AIbuddyTaskMigrationSource, AIbuddyTaskMode } from "./aibuddy-task-types-core.js";
import { aibuddyAgentProviderSchema } from "./aibuddy-agent-policy.js";
import { aibuddyPermissionResponseSchema } from "./aibuddy-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./aibuddy-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./aibuddy-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const aibuddyTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly AIbuddyTaskMode[];
const aibuddyTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly AIbuddyTaskMigrationSource[];
const aibuddyTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 AIbuddyTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(aibuddyTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: aibuddyAgentProviderSchema.optional(),
  migrationSource: z.enum(aibuddyTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: aibuddyTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const aibuddyPromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const aibuddyTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const aibuddyTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  aibuddyTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: aibuddyTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: aibuddyTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
