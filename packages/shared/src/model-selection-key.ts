...
export function buildNativeSupplierKey(aibuddyProvider: AIbuddyProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${aibuddyProvider}`;
}
...
export function buildGhostSupplierKey(
  aibuddyProvider: AIbuddyProvider,
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(aibuddyProvider);
}
