export type AIbuddyAgentBinaryKind = "native-binary";

export interface AIbuddyAgentRuntimeDescriptor {
  binaryKind: AIbuddyAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（aibuddy.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const AIBUDDY_AGENT_RUNTIME: AIbuddyAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".aibuddy/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[AIbuddy Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("aibuddy-agent", platform)],
  nodeBundleEntryFile: "aibuddy.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getAIbuddyAgentRuntime(): AIbuddyAgentRuntimeDescriptor {
  return AIBUDDY_AGENT_RUNTIME;
}
