import type { AIbuddyPersistedMessage, AIbuddyPersistedMessagePart } from "./aibuddy-task-types-core.js";

// AIbuddy runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveAIbuddyAssistants(
  messages: readonly AIbuddyPersistedMessage[],
): AIbuddyPersistedMessage[] {
  const merged: AIbuddyPersistedMessage[] = [];
...
function mergeAssistantPair(
  first: AIbuddyPersistedMessage,
  next: AIbuddyPersistedMessage,
): AIbuddyPersistedMessage {
  ...
  const shiftedNextParts: AIbuddyPersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
