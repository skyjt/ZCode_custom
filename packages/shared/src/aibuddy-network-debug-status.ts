import type {
  ...
  TraceId,
  AIbuddyStreamEvent,
  AIbuddyTaskNetworkDebugStatusType,
} from "./aibuddy-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<AIbuddyTaskNetworkDebugStatusType>([
...
export function aibuddyTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<AIbuddyStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): AIbuddyTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as AIbuddyTaskNetworkDebugStatusType)
    : undefined;
}
