...
import { z } from "zod";
import { aibuddyProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { aibuddyProviderSchema } from "./providers.js";
import { aibuddyAgentProviderSchema } from "./aibuddy-agent-policy.js";
...
} from "./aibuddy-protocol/index.js";
import { aibuddyTaskModeSchema } from "./aibuddy-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./aibuddy-protocol-v4/core.js";
import { errorAttributionSchema } from "./aibuddy-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./aibuddy-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { aibuddyTaskModeSchema } from "./aibuddy-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("aibuddy-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  aibuddyBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 AIbuddyTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { aibuddyProviderSchema };

export const aibuddyTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessSpawnedResponseSchema = z.object({
  ...
  provider: aibuddyProviderSchema,
  workspacePath: nonEmptyStringSchema,
  command: z.string(),
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: aibuddyProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = aibuddyProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `aibuddyProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: aibuddyMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: aibuddyToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: aibuddyMcpTelemetryEventSchema,
  })
  .strict();
...
export const aibuddyTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const aibuddyPromptImageAttachmentSchema = z.object({
...
export const aibuddyPromptVideoAttachmentSchema = z.object({
...
export const aibuddyPromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const aibuddyPromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const aibuddyPromptAttachmentSchema = z.discriminatedUnion("kind", [
  aibuddyPromptImageAttachmentSchema,
  aibuddyPromptVideoAttachmentSchema,
  aibuddyPromptPdfAttachmentSchema,
  aibuddyPromptFileAttachmentSchema,
]);

export const aibuddyPersistedToolCallSchema = z.object({
...
const aibuddyPersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const aibuddyPersistedMessageSchema = z.object({
  ...
  attachments: z.array(aibuddyPromptAttachmentSchema).optional(),
  tools: z.array(aibuddyPersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(aibuddyPersistedMessagePartSchema).optional(),
...
export const aibuddyTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const aibuddyTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const aibuddyTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const aibuddyTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: aibuddyTaskGoalStatusSchema,
...
export const aibuddyTaskGoalChangedPatchSchema = z.object({
  action: aibuddyTaskTargetChangedActionSchema,
  source: aibuddyTaskTargetChangedSourceSchema,
  target: aibuddyTaskGoalSchema.nullable(),
  previousTarget: aibuddyTaskGoalSchema.nullable().optional(),
});

export const aibuddyTaskTargetStatusSchema = aibuddyTaskGoalStatusSchema;
export const aibuddyTaskTargetSchema = aibuddyTaskGoalSchema;
export const aibuddyTaskTargetChangedPatchSchema = aibuddyTaskGoalChangedPatchSchema;

export const aibuddyTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: aibuddyTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: aibuddyAgentProviderSchema.optional(),
  migrationSource: aibuddyTaskMigrationSourceSchema.optional(),
  ...
  status: aibuddyTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: aibuddyTaskGoalSchema.nullable().optional(),
});

export const aibuddyTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const aibuddyPinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(aibuddyTaskIndexEntrySchema),
});

const aibuddyPersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const aibuddyPersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(aibuddyPersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const aibuddyPersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const aibuddySessionFileSchema = z.object({
  meta: aibuddyTaskMetaSchema,
  messages: z.array(aibuddyPersistedMessageSchema),
  fileChanges: z.array(aibuddyPersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(aibuddyPersistedTurnCheckpointSchema).optional(),
});
