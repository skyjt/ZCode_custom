...
import { normalizeAIbuddyEndpointOrigin } from "./aibuddyEndpoint.js";
import {
...
const aibuddyInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const aibuddyEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeAIbuddyEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeAIbuddyEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("aibuddyEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = aibuddyEndpointOriginSchema.safeParse(raw.aibuddyEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, aibuddyEndpointOrigin: parsed.data };
  }
  const { aibuddyEndpointOrigin: _aibuddyEndpointOrigin, ...next } = raw;
...
const appSettingsObjectSchema = z.object({
  ...
  aibuddyInteractionBehavior: aibuddyInteractionBehaviorSchema.default("queue"),
  ...
  aibuddyEndpointOrigin: aibuddyEndpointOriginSchema.optional(),
});
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeAIbuddyEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
...
export const appSettingsPatchSchema = z.object({
  ...
  aibuddyInteractionBehavior: aibuddyInteractionBehaviorSchema.optional(),
