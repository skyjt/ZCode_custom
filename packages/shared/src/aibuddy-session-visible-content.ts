...
// 消费者：aibuddyTaskServiceAdapter/aibuddySessionProjection/aibuddyTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromAIbuddyMessageParts 已迁
// aibuddy-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { AIbuddySessionGoal } from "./aibuddy-protocol/index.js";
...
export function isAIbuddyGoalContinuationReminderText(text: string): boolean {
...
export function isAIbuddyGoalContinuationReminderMessage(message: AIbuddyMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isAIbuddyGoalContinuationReminderText(textFromAIbuddyMessageParts(message.parts)))
  );
}

export function isAIbuddyGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isAIbuddyGoalModelOnlyReminderMessage(message: AIbuddyMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isAIbuddyGoalContinuationReminderMessage(message) ||
      isAIbuddyGoalStateReminderText(textFromAIbuddyMessageParts(message.parts)))
  );
}

export function isAIbuddyModelOnlySyntheticUserMessage(message: AIbuddyMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isAIbuddyCompactSummaryMessage(message: AIbuddyMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getAIbuddyUserVisibleMessages(
  messages: readonly AIbuddyMessageWithParts[],
  _options: { target?: AIbuddySessionGoal | null } = {},
): AIbuddyMessageWithParts[] {
  const visibleMessages: AIbuddyMessageWithParts[] = [];
  for (const message of messages) {
    if (isAIbuddyModelOnlySyntheticUserMessage(message) || isAIbuddyCompactSummaryMessage(message)) {
...
export function getAIbuddyGoalIterationByAssistantMessageId(
  messages: readonly AIbuddyMessageWithParts[],
  options: { maxGoalIteration?: number; target?: AIbuddySessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareAIbuddyMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isAIbuddyGoalContinuationReminderMessage(message)) {
...
export interface AIbuddyGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getAIbuddyGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly AIbuddyGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareAIbuddyMessagesByCreatedTime(
  left: AIbuddyMessageWithParts,
  right: AIbuddyMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: AIbuddyMessageWithParts, target: AIbuddySessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveAIbuddyVisibleSessionTitle(input: {
  title?: string;
  messages: readonly AIbuddyMessageWithParts[];
  target?: AIbuddySessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isAIbuddyGoalContinuationReminderText(normalizedTitle) &&
    !isAIbuddyGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isAIbuddyGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromAIbuddyMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
