// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 1 个导出：deriveAIbuddyTaskStatusFromSessionSnapshot。
// 消费者：aibuddyTaskServiceAdapter/aibuddyTaskIndexSyncer/aibuddySessionProjection（旧投影栈）。
import type { AIbuddySessionStateSnapshot } from "./aibuddy-protocol/index.js";
import { getAIbuddyUserVisibleMessages } from "./aibuddy-session-visible-content.js";
import type { AIbuddyTaskMeta } from "./aibuddy-task-types-core.js";
type AIbuddyTaskStatus = AIbuddyTaskMeta["status"];

function statusFromAIbuddySession(
  status: AIbuddySessionStateSnapshot["session"]["status"],
): AIbuddyTaskStatus {
...
function hasBlockingActiveSnapshotRuntime(snapshot: AIbuddySessionStateSnapshot): boolean {
...
function hasActiveSnapshotRuntime(snapshot: AIbuddySessionStateSnapshot): boolean {
...
function hasCompletedVisibleAssistantTurn(snapshot: AIbuddySessionStateSnapshot): boolean {
  const visibleMessages = getAIbuddyUserVisibleMessages(snapshot.messages, {
...
export function deriveAIbuddyTaskStatusFromSessionSnapshot(
  ...
  const status = statusFromAIbuddySession(snapshot.session.status);
