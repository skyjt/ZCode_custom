// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 5 个导出：旧 configOptions 投影函数（formatModelPickerValue/normalizeAvailableAIbuddyMode/
// getAIbuddyAgentModeSelectOptions/getAIbuddyAgentAvailableModes/
// aibuddySessionSettingsToAIbuddyConfigOptions）。
// 消费者：services aibuddyConfigOptions、UI aibuddySessionProjection 等旧栈。
import { formatModelPickerValue } from "./model-selection.js";
import type { AIbuddySessionMode, AIbuddySessionSettingsState } from "./aibuddy-protocol/index.js";
import type { AIbuddyConfigOption, AIbuddyTaskModeInfo } from "./aibuddy-task-types-core.js";
...
export function getAIbuddyAgentModeSelectOptions(): NonNullable<AIbuddyConfigOption["options"]> {
...
export function getAIbuddyAgentAvailableModes(): AIbuddyTaskModeInfo[] {
  return AIBUDDY_AGENT_MODE_OPTIONS.map((mode) => ({ ...mode }));
}

export function aibuddySessionSettingsToAIbuddyConfigOptions(
