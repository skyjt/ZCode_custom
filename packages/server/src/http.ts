...
import {
  ...
  AIBUDDY_RPC_HOST_CAPABILITY_HEADER,
  AIBUDDY_VERSION,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("aibuddy-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("AIBUDDY_SERVER_ID") || hostname() || "aibuddy-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("AIBUDDY_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("AIBUDDY_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("AIBUDDY_SERVER_NAME") }
      : {}),
    version: AIBUDDY_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("AIBUDDY_SERVER_TOKEN")),
...
const aibuddyLiteTokenCookieName = "aibuddy_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${aibuddyLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(aibuddyLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(AIBUDDY_RPC_HOST_CAPABILITY_HEADER);
