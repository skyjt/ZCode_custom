...
import { REMOTE_BASE } from "@aibuddy/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `aibuddy-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `aibuddy-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
