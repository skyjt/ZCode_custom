import { AIBUDDY_AGENT_PROVIDER } from "@aibuddy/shared";
import type { IRemoteBackend } from "@aibuddy/server/remote/backend.js";
import { type DeployLoggers, waitForClose } from "@aibuddy/server/remote/deployShared.js";
...
export async function repairLegacyRemoteOfficialPluginDirectoryPermissions(params: {
  ...
  params.loggers.logWarn(
    `[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 检查并修复旧 builtin plugin 目录权限 ${params.remoteOfficialPluginDir}`,
  );
  ...
  try {
    ...
    params.loggers.logWarn(
      `[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 修复旧 builtin plugin 目录权限失败，将继续尝试替换 packages: ${
