...
import { AIBUDDY_AGENT_PROVIDER, resolveAIbuddyRuntimeEnv } from "@aibuddy/shared";
import { createHash, randomUUID } from "node:crypto";
import { existsSync } from "node:fs";
import { cp, lstat, mkdir, mkdtemp, readdir, readFile, readlink, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { dirname, join, resolve } from "node:path";
import type { IRemoteBackend } from "@aibuddy/server/remote/backend.js";
import {
...
import {
  ...
  quotePosixPathArg,
} from "@aibuddy/server/remote/posixShell.js";
import { createTarGzArchive } from "@aibuddy/server/remote/localTarGz.js";
import {
  buildRemoteAgentBundleWrapper,
  isRemoteAgentBundleWrapperCurrent,
  REMOTE_AGENT_BUNDLE_NAME,
} from "@aibuddy/server/remote/aibuddyAgentBundleWrapper.js";
import {
  deployRemoteAgentWrapper,
  isWslBackend,
} from "@aibuddy/server/remote/aibuddyAgentWrapperDeploy.js";
import {
...
} from "@aibuddy/server/remote/aibuddyAgentOfficialPluginAssets.js";
import { repairLegacyRemoteOfficialPluginDirectoryPermissions } from "@aibuddy/server/remote/aibuddyAgentOfficialPluginPermissionRepair.js";

const DEV_AGENT_BUNDLE_RELATIVE_PATH = "apps/aibuddy-cli/packages/cli/dist/aibuddy.cjs";
const DEV_AGENT_BUNDLE_ENV = "AIBUDDY_REMOTE_DEV_AGENT_BUNDLE";
const REMOTE_DEV_AGENT_BUNDLE_NAME = REMOTE_AGENT_BUNDLE_NAME;
const REMOTE_DEV_AGENT_VERSION_FILE_NAME = ".dev-version";

interface DeployDevelopmentAIbuddyAgentRuntimeParams {
...
function shouldUseDevelopmentAgentBundle(): boolean {
  if (resolveAIbuddyRuntimeEnv(process.env) !== "development") {
    return false;
  }
...
async function hashDevelopmentOfficialPluginPackage(
  ...
  const manifestPath = join(packageRoot, ".aibuddy-plugin", "plugin.json");
  if (!existsSync(manifestPath)) {
    throw new Error(`[aibuddy-agent-deploy] missing official plugin manifest: ${manifestPath}`);
  }
...
function assertDevelopmentOfficialPluginRequiredAssets(
  ...
  for (const requiredRelativePath of REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS) {
    ...
    if (!existsSync(requiredPath)) {
      throw new Error(
        `[aibuddy-agent-deploy] missing official plugin required asset: ${requiredPath}`,
      );
    }
  }
}
...
async function shouldSkipDevelopmentAIbuddyAgentDeploy(params: {
  backend: IRemoteBackend;
  remoteVersionFile: string;
  ...
  params.loggers.log(`[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 开发态 aibuddy.cjs 未变化，跳过`);
  return true;
}
...
async function stageDevelopmentOfficialPluginPackages(params: {
  ...
  for (const packageName of REMOTE_AGENT_OFFICIAL_PLUGIN_PACKAGE_NAMES) {
    const sourceRoot = join(params.repoRoot, "apps", "aibuddy-cli", "packages", packageName);
...
async function uploadDevelopmentOfficialPluginPackages(params: {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "aibuddy-remote-agent-packages-"));
    ...
    const cleanupRemoteStaging = async (): Promise<void> => {
      try {
        ...
        params.loggers.logWarn(
          `[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 清理 owner staging 失败 (${ownerSuffix}): ${String(error)}`,
        );
      }
    };
    params.loggers.log(`[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 开发态上传官方插件资源`);
    try {
...
export async function deployDevelopmentAIbuddyAgentRuntime(
  backend: IRemoteBackend,
  params: DeployDevelopmentAIbuddyAgentRuntimeParams,
  loggers: DeployLoggers,
): Promise<boolean> {
  ...
  if (
    await shouldSkipDevelopmentAIbuddyAgentDeploy({
      backend,
  ...
  // 开发态 SSH 远端过去只会部署本地 aibuddy.cjs，不会携带 packages/*-plugin。
  // builtin plugin seed 依赖 agent 包旁边的官方插件源资源，所以 dev 部署需要同步 bundle 与插件资源。
  // 本地修改 apps/aibuddy-cli 后，远端测试仍运行滞后的发布包。这里改为上传 dev 启动时刚构建的
  // dist/aibuddy.cjs，并用远端已部署的 node 包一层 wrapper 启动，保证 agent 仍运行在目标机器内。
  loggers.log(
    `[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 开发态上传本地 aibuddy.cjs ${devVersion.slice(0, 12)}`,
  );
  ...
  loggers.log(
    `[aibuddy-agent-deploy] ${AIBUDDY_AGENT_PROVIDER}: 开发态部署完成 ${devVersion.slice(0, 12)}`,
  );
  return true;
}
