import type { HelloMessage, HelloAckMessage } from "@aibuddy/shared";
import { AIBUDDY_VERSION, formatZodError, helloMessageSchema } from "@aibuddy/shared";
 ...
 * 1. Read lines from stdout until we find a aibuddy-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a aibuddy-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no aibuddy-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "aibuddy-hello"
            ) {
              rejectWithDiagnostics(`Invalid aibuddy-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
