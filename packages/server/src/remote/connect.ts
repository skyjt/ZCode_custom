import { SocketProtocol, ChannelClient } from "@aibuddy/rpc";
import type { IServiceAccessor } from "@aibuddy/services";
import { RemoteServiceAccess } from "@aibuddy/client";
import {
...
/**
 * Connect to a remote aibuddy server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 aibuddy-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'AIBUDDY_SERVER_RUNTIME_ROOT="$HOME/.aibuddy/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${AIBUDDY_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${AIBUDDY_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${AIBUDDY_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${AIBUDDY_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.aibuddy/server/node ~/.aibuddy/server/aibuddy-server.cjs`;
}
