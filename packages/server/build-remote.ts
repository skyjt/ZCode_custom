...
const { content: aibuddyBuiltinProviderConfigJson } = await loadBuiltinProviderConfig();
...
const buildResult = await build({
  ...
  bundle: true,
  outfile: "dist/remote/aibuddy-server.cjs",
  platform: "node",
  format: "cjs",
  target: "node22",
  ...
  define: {
    ...
    __AIBUDDY_BUILTIN_PROVIDER_CONFIG_JSON__: JSON.stringify(aibuddyBuiltinProviderConfigJson),
  },
  metafile: true,
});

const remoteBundleSource = readFileSync("dist/remote/aibuddy-server.cjs", "utf-8");
...
console.log("Built dist/remote/aibuddy-server.cjs");
