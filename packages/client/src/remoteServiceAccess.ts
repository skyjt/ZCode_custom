import { ProxyChannel, type IChannelClient } from "@aibuddy/rpc";
import {
  IFileService,
...
export class RemoteServiceAccess implements IServiceAccessor {
  ...
  constructor(channelClient: IChannelClient) {
    ...
    this.aibuddyTaskService = ProxyChannel.toService<IAIbuddyTaskService>(
      channelClient.getChannel(IAIbuddyTaskService.channelName),
    );
    ...
    this.aibuddyAgentService = ProxyChannel.toService<IAIbuddyAgentService>(
      channelClient.getChannel(IAIbuddyAgentService.channelName),
    );
    this.aibuddySessionService = ProxyChannel.toService<IAIbuddySessionService>(
      channelClient.getChannel(IAIbuddySessionService.channelName),
    );
