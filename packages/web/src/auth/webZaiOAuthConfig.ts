...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const aibuddyEndpointUrls = buildAIbuddyEndpointUrls(
    env.VITE_AIBUDDY_BASE_URL?.trim() ||
      env.VITE_AIBUDDY_ENDPOINT_ORIGIN?.trim() ||
      DEFAULT_AIBUDDY_ENDPOINT_ORIGIN,
  );
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "aibuddy"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "aibuddy",
    redirectUri: aibuddyEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: aibuddyEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),
