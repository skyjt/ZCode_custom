...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const aibuddyJwtToken = tokenSet.aibuddyJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (aibuddyJwtToken) {
      this.localStorage.setItem(AIBUDDY_JWT_TOKEN_KEY, aibuddyJwtToken);
    } else {
      this.localStorage.removeItem(AIBUDDY_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const aibuddyJwtToken = this.localStorage.getItem(AIBUDDY_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(aibuddyJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(aibuddyJwtToken, this.now()).kind === "expired") {
  ...
  loadAIbuddyJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(AIBUDDY_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(AIBUDDY_JWT_TOKEN_KEY) ||
