...
export function createElectronRuntimeMirrorEnv(mirror) {
  return {
    AIBUDDY_ELECTRON_RUNTIME_MIRROR: mirror,
...
function resolveDefaultElectronBuilderBinariesMirror(env = process.env) {
  return env.AIBUDDY_DEPS_BASE_URL?.trim() || env.INTRANET_MACHINE_HOST?.trim()
...
环境变量:
  AIBUDDY_TARGET_OS              与 --os 等价
  AIBUDDY_TARGET_ARCH            与 --arch 等价
`);
}
...
function parseArgs(argv) {
  const options = {
    os: process.env.AIBUDDY_TARGET_OS ?? null,
    arch: process.env.AIBUDDY_TARGET_ARCH ?? null,
    skipPrepare: process.env.AIBUDDY_SKIP_PREPARE === "1",
    skipBuild: process.env.AIBUDDY_SKIP_BUILD === "1",
    dryRun: false,
    positionals: [],
  };
...
async function main() {
  ...
  const buildEnv = {
    AIBUDDY_TARGET_OS: os,
    AIBUDDY_TARGET_ARCH: arch,
