...
contextBridge.exposeInMainWorld("__AIBUDDY_DEVICE_ID__", parseDeviceIdFromArgs());
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__aibuddyFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatAIbuddyRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 aibuddy-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("aibuddy", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@aibuddy/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  /** 打开 AIbuddy Computer Use 完整权限引导 */
  ...
  reportLocalTtftBatch: (batch: import("@aibuddy/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.aibuddy/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@aibuddy/shared").ChromeBrowserDataImportOptions) =>
  ...
  getAIbuddyStdioTapDevState: (): Promise<AIbuddyStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetAIbuddyStdioTapDevState),
