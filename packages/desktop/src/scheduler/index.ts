...
function resolveScheduledAt(automation: AIbuddyAutomation, now: number): number {
...
async function handleClaimed(automation: AIbuddyAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: AIbuddyAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: AIbuddyOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: AIbuddyAutomationTrigger =
