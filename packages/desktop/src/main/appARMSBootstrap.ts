...
import { AIBUDDY_AGENT_LIFECYCLE_LOG_MARKER } from "@aibuddy/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 aibuddy-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapAIbuddyEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
    .init({
      ...
      endpoint: AIBUDDY_ARMS_RUM_ENDPOINT,
      env: armsRumEnv,
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(AIBUDDY_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${AIBUDDY_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  AIBUDDY_TELEMETRY_ENABLED && AIBUDDY_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
