...
import { formatTimestamp } from "@aibuddy/shared";
import { cleanupExpiredLogFiles, LOG_RETENTION_DAYS } from "./logRetention.js";
import { getAppConfigDir, maybeThrowInjectedFsFault } from "@aibuddy/services/node";

function getLogDir() {
  const e2eLogDir =
    process.env.AIBUDDY_ENV === "test" ? process.env.AIBUDDY_E2E_RUNTIME_LOG_DIR?.trim() : undefined;
  if (e2eLogDir) {
    return e2eLogDir;
  }
...
/**
 * main 进程日志，默认写入 ~/.aibuddy/v2/logs/YYYY-MM-DD.log；E2E 测试使用 worker 专属目录。
