...
const AIBUDDY_CLIENT_CONFIG_API_PATH = "/api/v1/client/configs";
...
function resolveForceUpdateClientConfigUrl(endpointOrigin = DEFAULT_AIBUDDY_ENDPOINT_ORIGIN): string {
  const url = new URL(
    `${buildAIbuddyEndpointUrls(endpointOrigin).origin}${AIBUDDY_CLIENT_CONFIG_API_PATH}`,
  );
...
function getForceUpdateMinimalVersionFromClientConfig(config: unknown): string | undefined {
  ...
  if (typeof envelope.code === "number" && envelope.code !== 0) {
    ...
    throw new Error(`AIbuddy client config failed: ${envelope.code}`);
  }
...
async function resolveDesktopForceUpdateRequirement(options: {
  ...
  const resolveFromConfig = (config: unknown) =>
    resolveForceUpdateRequirement({
      currentVersion: AIBUDDY_VERSION,
      forceUpdate: {
        minimalVersion:
