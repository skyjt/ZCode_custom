...
import {
  ...
  DEFAULT_LOCALE,
  AIBUDDY_VERSION,
  AIBUDDY_TELEMETRY_ENABLED,
  AIBUDDY_ARMS_RUM_ENDPOINT,
...
import { resolveAIbuddyBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveAIbuddyEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopAIbuddyDataSizeTelemetry,
  stopDesktopAIbuddyDataSizeTelemetry,
} from "./desktopAIbuddyDataSizeTelemetry.js";
...
import { mapAIbuddyEnvToArmsRumEnv } from "@aibuddy/shared";
import {
...
// 会和开发态已打开的 AIbuddy Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.AIBUDDY_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".aibuddy", "v2", "setting.json");
...
async function resolveCurrentAIbuddyEndpointOrigin() {
  return resolveAIbuddyEndpointOrigin({
    env: AIBUDDY_ENV,
    envBaseOrigin: resolveAIbuddyEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).aibuddyEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 AIbuddy 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: AIBUDDY_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: AIBUDDY_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    AIBUDDY_TELEMETRY_ENABLED && AIBUDDY_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，aibuddy-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.AIBUDDY_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return AIBUDDY_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${AIBUDDY_ENV}`);
    return true;
  }
  ...
  const dialogOptions = {
    ...
    message: isZh ? "确认退出 AIbuddy?" : "Quit AIbuddy?",
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onAIbuddyEndpointChanged: handleAIbuddyEndpointChanged,
    aibuddyEndpointEnvBaseOrigin: resolveAIbuddyEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveAIbuddyEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (AIBUDDY_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentAIbuddyEndpointOrigin();
  if (origin === DEFAULT_AIBUDDY_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleAIbuddyEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveAIbuddyEndpointSelection(), mainSettingService.get()]).then(
    ([aibuddyEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        aibuddyEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          aibuddyBuiltinProviderConfigFilePath: resolveAIbuddyBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(AIBUDDY_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 AIbuddy 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: AIBUDDY_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapAIbuddyEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (AIBUDDY_TELEMETRY_ENABLED && AIBUDDY_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: AIBUDDY_VERSION,
  ...
  registerDesktopAIbuddyDataSizeTelemetry({
    ...
    isAIbuddyBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getAIbuddyDataRootDir(),
    stateFile: join(app.getPath("userData"), "aibuddy-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 AIBUDDY_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    AIBUDDY_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
