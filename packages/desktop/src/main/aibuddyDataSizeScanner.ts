...
export type AIbuddyDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type AIbuddyDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: AIbuddyDataSizePartialReason }
);

export interface AIbuddyDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface AIbuddyDataSizeScanOptions extends AIbuddyDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("AIbuddy data size scan aborted", "AbortError");
}
 ...
 */
export async function scanAIbuddyDataDirectory(
  options: AIbuddyDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<AIbuddyDataSizePartialReason, "io_error"> | null = null;
...
export function isAIbuddyDataSizeScanResult(value: unknown): value is AIbuddyDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<AIbuddyDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
