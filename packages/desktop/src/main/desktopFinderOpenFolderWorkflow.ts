...
const WORKFLOW_NAME = "Open in AIbuddy.workflow";
const WORKFLOW_BUNDLE_ID = "dev.aibuddy.app.finder-open-workflow";
const WORKFLOW_VERSION = "5";
const SERVICES_MENU_LABELS: Record<Locale, string> = {
  "zh-CN": "在AIbuddy中打开",
  "en-US": "Open in AIbuddy",
};
...
if [ -n "$first" ]; then
  ...
  /usr/bin/open "aibuddy://workspace/open?path=\${encoded}"
fi
`;
