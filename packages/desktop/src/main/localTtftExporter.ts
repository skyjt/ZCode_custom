...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@aibuddy/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "aibuddy-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "aibuddy.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "aibuddy.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@aibuddy/local-ttft", "1");
  const duration = meter.createHistogram("aibuddy.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("aibuddy.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("aibuddy.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "aibuddy.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("aibuddy.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("aibuddy.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("aibuddy.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("aibuddy.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("aibuddy.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("aibuddy.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("aibuddy.local_ttft.records");
  const drops = meter.createCounter("aibuddy.local_ttft.dropped");
