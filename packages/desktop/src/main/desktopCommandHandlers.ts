...
import {
  ...
  buildAIbuddyEndpointUrls,
  getCommunityUrlFromConfigs,
  getFeedbackUrlFromConfig,
  resolveHelpAppConfig,
  normalizeAIbuddyEndpointOrigin,
  resolveAIbuddyEndpointOrigin,
} from "@aibuddy/shared";
import { readAIbuddyStdioTapDevState, setAIbuddyStdioTapDevEnabled } from "@aibuddy/services/node";
...
export const HELP_TOGGLE_AIBUDDY_STDIO_TAP_MENU_ID = "help.toggle-aibuddy-stdio-tap";
const AIBUDDY_ENDPOINT_PROMPT_WIDTH = 460;
const AIBUDDY_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:aibuddy-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.aibuddy/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.aibuddy/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.aibuddy/v2:", error);
  }
...
async function promptCustomAIbuddyEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showAIbuddyEndpointPromptWindow({
    currentValue,
...
function buildAIbuddyEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>AIbuddy Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">AIbuddy endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "aibuddy-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "aibuddy-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showAIbuddyEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: AIBUDDY_ENDPOINT_PROMPT_WIDTH,
      height: AIBUDDY_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "AIbuddy Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "aibuddy-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("aibuddy-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("aibuddy-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildAIbuddyEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setAIbuddyEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { aibuddyEndpointOrigin?: string | undefined }): Promise<void> };
  onAIbuddyEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (AIBUDDY_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeAIbuddyEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ aibuddyEndpointOrigin: normalized });
  await options.onAIbuddyEndpointChanged();
}
...
function toggleAIbuddyStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateAIbuddyStdioTapDevMenuState: () => void;
}) {
  const current = readAIbuddyStdioTapDevState();
  const next = setAIbuddyStdioTapDevEnabled(!current.enabled);
  options.updateAIbuddyStdioTapDevMenuState();
...
function resolveChangelogUrl(
  ...
  const origin = buildAIbuddyEndpointUrls(endpointOrigin).origin;
...
async function resolveCurrentAIbuddyEndpointOrigin(settingService: {
  get(): Promise<{ aibuddyEndpointOrigin?: string }>;
  ...
  return resolveAIbuddyEndpointOrigin({
    ...
    overrideOrigin: settings.aibuddyEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "aibuddyEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "aibuddyEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  aibuddyEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentAIbuddyEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.aibuddyEndpointEnvBaseOrigin,
        }),
      );
      return;
    case DesktopCommandIds.CheckForUpdates:
      // 按产品身份而不是后端环境放行：生产后端的 Preview 同样没有更新器。
      if (AIBUDDY_PRODUCT_FLAVOR === "production") {
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleAIbuddyStdioTapDevProxy:
      toggleAIbuddyStdioTapDevProxy({
        logger: options.logger,
        updateAIbuddyStdioTapDevMenuState: options.updateAIbuddyStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetAIbuddyEndpointProduction:
      await setAIbuddyEndpointOverride({
        value: DEFAULT_AIBUDDY_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onAIbuddyEndpointChanged: options.onAIbuddyEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetAIbuddyEndpointTest:
      await setAIbuddyEndpointOverride({
        value: options.aibuddyEndpointEnvBaseOrigin ?? resolveRuntimeAIbuddyEndpointOrigin(),
        settingService: options.settingService,
        onAIbuddyEndpointChanged: options.onAIbuddyEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetAIbuddyEndpointCustom: {
      const current =
        (await options.settingService.get()).aibuddyEndpointOrigin ?? DEFAULT_AIBUDDY_ENDPOINT_ORIGIN;
      const value = await promptCustomAIbuddyEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetAIbuddyEndpoint:
      await setAIbuddyEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onAIbuddyEndpointChanged: options.onAIbuddyEndpointChanged,
