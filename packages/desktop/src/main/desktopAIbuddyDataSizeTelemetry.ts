...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@aibuddy/shared";

import type { AIbuddyDataSizeScanResult } from "./aibuddyDataSizeScanner.js";
import {
  readAIbuddyDataSizeTelemetryState,
  writeAIbuddyDataSizeTelemetryState,
  type AIbuddyDataSizeTelemetryState,
} from "./aibuddyDataSizeTelemetryState.js";
import { scanAIbuddyDataDirectoryInWorker } from "./aibuddyDataSizeWorkerClient.js";

export type { AIbuddyDataSizeTelemetryState } from "./aibuddyDataSizeTelemetryState.js";

const AIBUDDY_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const AIBUDDY_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const AIBUDDY_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const AIBUDDY_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const AIBUDDY_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const AIBUDDY_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const AIBUDDY_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const AIBUDDY_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const AIBUDDY_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const AIBUDDY_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const AIBUDDY_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const AIBUDDY_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface AIbuddyDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: AIbuddyDataSizeTelemetryTiming = {
  abortedRetryMs: AIBUDDY_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: AIBUDDY_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: AIBUDDY_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: AIBUDDY_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: AIBUDDY_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: AIBUDDY_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: AIBUDDY_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: AIBUDDY_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: AIBUDDY_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: AIBUDDY_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface AIbuddyDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface AIbuddyDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isAIbuddyBusy: () => boolean;
  logger: AIbuddyDataSizeTelemetryLogger;
  readState: () => Promise<AIbuddyDataSizeTelemetryState | null>;
  report: (result: AIbuddyDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<AIbuddyDataSizeScanResult>;
  timing?: Partial<AIbuddyDataSizeTelemetryTiming>;
  writeState: (state: AIbuddyDataSizeTelemetryState) => Promise<void>;
}
...
function buildAIbuddyDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: AIbuddyDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_aibuddy_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      metric_kind: "aibuddy_data_bytes",
      ...
      schema_version: 1,
      aibuddy_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / AIBUDDY_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createAIbuddyDataSizeTelemetryScheduler(
  dependencies: AIbuddyDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: AIbuddyDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isAIbuddyBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[aibuddy-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<AIbuddyDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: AIbuddyDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[aibuddy-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("AIbuddy data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("AIbuddy data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[aibuddy-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[aibuddy-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: AIbuddyDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[aibuddy-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createAIbuddyDataSizeTelemetryScheduler> | null = null;

export function registerDesktopAIbuddyDataSizeTelemetry(options: {
  context: Parameters<typeof buildAIbuddyDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isAIbuddyBusy: () => boolean;
  logger: AIbuddyDataSizeTelemetryLogger;
  ...
  stopDesktopAIbuddyDataSizeTelemetry();
  desktopScheduler = createAIbuddyDataSizeTelemetryScheduler({
    ...
    isAIbuddyBusy: options.isAIbuddyBusy,
    logger: options.logger,
    readState: () => readAIbuddyDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildAIbuddyDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanAIbuddyDataDirectoryInWorker(
        {
          maxDurationMs: AIBUDDY_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: AIBUDDY_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeAIbuddyDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[aibuddy-data-size] scheduler start failed", error);
  });
}

export function stopDesktopAIbuddyDataSizeTelemetry(): void {
