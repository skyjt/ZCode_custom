...
import { formatAIbuddyRendererProcessName } from "@aibuddy/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatAIbuddyRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatAIbuddyRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatAIbuddyRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
