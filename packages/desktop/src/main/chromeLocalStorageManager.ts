...
import type { ChromeBrowserDataImportError } from "@aibuddy/shared";
...
export async function readChromeCookiesWithHelper(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "aibuddy-chrome-cookie-helper-"));
...
export async function importChromeLocalStorage(options: {
  ...
  const tempRoot = await mkdtemp(join(tmpdir(), "aibuddy-chrome-local-storage-"));
  const userDataPath = join(tempRoot, "User Data");
  const targetProfilePath = join(userDataPath, "Default");
  let originCount = 0;
  try {
