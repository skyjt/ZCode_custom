...
type PatchedIpcRenderer = IpcRenderer & { __aibuddyArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __aibuddyArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__aibuddyArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__aibuddyArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__aibuddyArmsIpcPatched) {
    return;
  }
  ...
  patched.__aibuddyArmsIpcPatched = true;
}
