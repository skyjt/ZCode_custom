...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: IAIbuddyTaskService;
...
function taskKey(task: Pick<AIbuddyTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: AIbuddyTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@aibuddy/shared/aibuddy-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: AIbuddyTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: AIbuddyTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IAIbuddyTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IAIbuddyTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IAIbuddyTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IAIbuddyAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: AIbuddyTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@aibuddy/shared").WindowHostAttachmentScope;
