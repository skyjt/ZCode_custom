...
export function createRemoteWorkspaceServiceCollection(params: {
  ...
  createReportingRemoteAIbuddyTaskService: <T extends object>(service: T) => T;
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    tokenProvider: async () =>
      (await localCredentialService.load(AIBUDDY_JWT_TOKEN_KEY))?.trim() || null,
  });
  const conversationShareService = new ConversationShareService({
    aibuddyAgentService: params.connectionServices.aibuddyAgentService,
  ...
  const reportingRemoteAIbuddyTaskService = params.createReportingRemoteAIbuddyTaskService(
    params.connectionServices.aibuddyTaskService,
  );
  ...
  const remoteAIbuddyTaskService = params.createRemotePromptAttachmentTaskService(
    reportingRemoteAIbuddyTaskService,
  );
  const remoteAIbuddySessionService = params.createRemotePromptAttachmentSessionService(
    params.connectionServices.aibuddySessionService,
  );
  ...
  params.connectionServices.aibuddyAgentService.onDynamicSessionRuntimePreferencesRequest()(
    (request) => {
      const startedAt = Date.now();
      const requestContext = {
        event: "aibuddy_protocol.runtime_preferences.host_request_received",
      ...
      void (async () => {
        ...
        await params.connectionServices.aibuddyAgentService.respondSessionRuntimePreferences({
          requestId: request.requestId,
          resolution,
        });
  ...
  // 因此这里为 remote workspace host 补齐本地全局 channel；文件、终端、AIbuddy Agent 仍来自远端，
  ...
  const services = new ServiceCollection()
    ...
    .register(IAIbuddyTaskService, remoteAIbuddyTaskService)
    .register(IAIbuddyAgentService, params.connectionServices.aibuddyAgentService)
    .register(IAIbuddySessionService, remoteAIbuddySessionService)
