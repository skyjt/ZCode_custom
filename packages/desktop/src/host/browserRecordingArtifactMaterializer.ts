...
import type { IRemoteBackend } from "@aibuddy/server/remote";
import type { BrowserRecordingArtifact } from "@aibuddy/shared";
...
export async function materializeBrowserRecordingArtifact(input: {
  ...
  const stagingPath = `${targetPath}.aibuddy-recording-${randomUUID()}.tmp`;
  try {
