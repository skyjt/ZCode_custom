import type { AIbuddyAutomationRunOutcome, AIbuddyAutomationTrigger } from "@aibuddy/shared";
...
interface CronRunLifecycleRepo {
  ...
  markRunOutcome(runId: string, outcome: AIbuddyAutomationRunOutcome, error?: string): Promise<void>;
...
export async function settleCronRunTerminalOutcome(
  params: CronRunLifecycleIdentity & {
    repo: CronRunLifecycleRepo;
    outcome: Exclude<AIbuddyAutomationRunOutcome, "running">;
    error?: string;
    logWarn: LogWarn;
  },
): Promise<void> {
