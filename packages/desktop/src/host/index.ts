...
import {
  ...
  createAIbuddyAgentConnectionScope,
  type AIbuddyAgentV4ClientMode,
...
import { createRemoteConnectionProgressContext } from "@aibuddy/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["AIBUDDY_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 aibuddy-* title，方便系统进程列表过滤。
process.title = formatAIbuddyHostProcessName(process.env["AIBUDDY_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("aibuddy-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 aibuddy-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  aibuddyTaskService: IAIbuddyTaskService;
  taskId: string;
  workspacePath: string;
  workspaceIdentity?: string;
}): Promise<number | undefined> {
  try {
    const snapshot = await params.aibuddyTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  void params.aibuddyTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.aibuddyTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        aibuddyTaskService: params.aibuddyTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const aibuddyTaskService = activeServices?.getOptional(IAIbuddyTaskService);
  if (!aibuddyTaskService) {
    throw new Error("AIbuddy task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        aibuddyTaskService.listDeletedTaskIds(workspaceScope),
        aibuddyTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      const task = await aibuddyTaskService.createTask({
        workspacePath: request.workspacePath,
        workspaceIdentity: request.workspaceIdentity,
        // 空 Session 沿用普通初始化；idle Selection 只在下方执行中注入。
        // 在此写入会让闲时轮结束后的普通消息继续使用无票的隐藏 Provider。
        mode: request.permissionMode as AIbuddyTaskMode,
    ...
    await aibuddyTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const aibuddyTaskService = targetServices.getOptional(IAIbuddyTaskService);
  if (!aibuddyTaskService) {
    throw new Error("AIbuddy task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await aibuddyTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isAIbuddyTaskMeta(value: unknown): value is AIbuddyTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteAIbuddyTaskService<T extends object>(
  ...
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: AIbuddyPromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: AIbuddyTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // aibuddySessionStore 的整条消费链词表都是 AIbuddyStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→AIbuddyStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 aibuddy-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: AIbuddyTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: AIbuddyTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote AIbuddy task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote AIbuddy task ready event is not subscribable");
    }
  ...
  // remote workspace 的 AIbuddy Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: AIbuddyTaskMeta; started: boolean } | undefined;
...
function warmUpAIbuddyAgent(
  ...
  const aibuddySessionService = services.getOptional(IAIbuddySessionService);
  if (!aibuddySessionService) {
    return;
  }
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `AIbuddy agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `AIbuddy agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `AIbuddy agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`AIbuddy agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IAIbuddyAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteAIbuddyTaskService: (service) =>
      createReportingRemoteAIbuddyTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 aibuddy-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IAIbuddyTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IAIbuddyTaskService),
        agentService: services.getOptional(IAIbuddyAgentService),
    ...
    const taskService = activeServices?.getOptional(IAIbuddyTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IAIbuddyAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IAIbuddyTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IAIbuddyTaskService["archiveTask"]>[0]
            | Parameters<IAIbuddyTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IAIbuddyTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IAIbuddyTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IAIbuddyTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IAIbuddyTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: AIbuddyAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IAIbuddyAgentService);
  const connectionScope = agentService
    ? createAIbuddyAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IAIbuddyTaskService);
  if (taskService) {
    overrides.set(
      IAIbuddyTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IAIbuddyAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as AIbuddyTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!aibuddyTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "AIbuddy task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    const aibuddyTaskService = activeServices?.getOptional(IAIbuddyTaskService);
    if (!aibuddyTaskService) {
      logger.warn("session message delivery result received before AIbuddy task service initialized");
      return;
    }
    void aibuddyTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              aibuddyAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              aibuddyBuiltinProviderConfigFilePath: msg.aibuddyBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 aibuddyAgentService 转到这个 executor，
        ...
        const aibuddyTaskService = services.getOptional(IAIbuddyTaskService);
        if (aibuddyTaskService) {
          const reportingAIbuddyTaskService = createReportingRemoteAIbuddyTaskService(
            aibuddyTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IAIbuddyTaskService, reportingAIbuddyTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@aibuddy/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 AIBUDDY_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: AIBUDDY_VERSION,
    // 远端 aibuddy-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
