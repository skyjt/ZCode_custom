...
import {
  ...
  aibuddyStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@aibuddy/shared";
import { resolveDefaultAIbuddyAgentCommand } from "@aibuddy/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultAIbuddyAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = aibuddyStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
