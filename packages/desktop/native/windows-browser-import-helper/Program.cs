...
{
    private const string Protocol = "AIBUDDY_BROWSER_IMPORT_V1";
        ...
        try
        {
            if (HasArgument(args, "--version"))
            {
                ...
                Console.Out.WriteLine(
                    "AIBUDDY_BROWSER_IMPORT_HELPER\t" + HelperVersion + "\t" + GetProcessArchitecture() +
    ...
    private static void RunBrokerCore()
    {
        ...
        try
        {
            ...
            pipeName = "aibuddy-browser-import-broker-" + Guid.NewGuid().ToString("N");
            token = CreateToken();
    ...
    private static void RunTemporarySystemService(
        ...
        int brokerPid)
    {
        string serviceName = "AIbuddyBrowserImport_" + Guid.NewGuid().ToString("N");
        string pipeName = "aibuddy-browser-import-system-" + Guid.NewGuid().ToString("N");
